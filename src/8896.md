  RFC 8896              ALTO Cost Calendar   November 2020
  --------------------- -------------------- ---------------
...
The ALTO protocol in \[[RFC7285](#RFC7285){.xref}\] specifies a network
...
In the rest of this document, [Section 3](#sect-3){.xref} provides the
...
-   [Attribute \"cost-type-names\" is associated with
    ...
    attribute because they share the same values for
...
This section provides an example ALTO Server IRD that supports various
...
The design of the Calendar capabilities allows some Calendars with the
...
For the Endpoint Cost Service:[¶](#section-4.3-11){.pilcrow}
...
-   [the \"CalendarResponseAttributes\" object that applies to a cost
    ...
    \"time-interval-size\" and \"number-of-intervals\". The members of
...
The Application Client has the choice to trade content or resources with
...
Section [1](https://www.rfc-editor.org/rfc/rfc8446#section-1){.relref}
...
\[SELFISH_RTG_2002\]
:   [Roughgarden, T.]{.refAuthor}, [\"Selfish Routing\"]{.refTitle},
    [Dissertation Thesis, Cornell]{.refContent}, May 2002.
:   

\[SELFISH_RTG_2003\]
:   [Qiu, L.]{.refAuthor}[, Yang, Y.]{.refAuthor}[,
    Zhang, Y.]{.refAuthor}[, and S. Shenker]{.refAuthor}, [\"Selfish
    Routing in Internet-Like Environments\"]{.refTitle}, [Proceedings of
    SIGCOMM \'03]{.refContent}, [DOI
    10.1145/863955.863974]{.seriesInfo}, August 2003,
    \<<https://doi.org/10.1145/863955.863974>\>.
:   

\[SENSE\]
:   [Department of Energy Office of Science Advanced Scientific
    Computing Research (ASCR) Program]{.refAuthor}, [\"SDN for
    End-to-End Networked Science at the Exascale (SENSE)\"]{.refTitle},
    \<<http://sense.es.net/overview>\>.
:   

\[UNICORN-FGCS\]
:   [Xiang, Q.]{.refAuthor}[, Wang, T.]{.refAuthor}[,
    Zhang, J.]{.refAuthor}[, Newman, H.]{.refAuthor}[,
    Yang, Y.]{.refAuthor}[, and Y. Liu]{.refAuthor}, [\"Unicorn: Unified
    resource orchestration for multi-domain, geo-distributed data
    analytics\"]{.refTitle}, [Future Generation Computer Systems (FGCS),
    Vol. 93, Pages 188-197]{.refContent}, [DOI
    10.1016/j.future.2018.09.048]{.seriesInfo}, [ISSN
    0167-739X]{.seriesInfo}, March 2019,
...
The authors would like to thank [Fred Baker]{.contact-name}, [Li
Geng]{.contact-name}, [Diego Lopez]{.contact-name}, [He
Peng]{.contact-name}, and [Haibin Song]{.contact-name} for fruitful
discussions and feedback on earlier draft versions. [Dawn
Chan]{.contact-name}, [Kai Gao]{.contact-name}, [Vijay
Gurbani]{.contact-name}, [Yichen Qian]{.contact-name}, [Jürgen
Schönwälder]{.contact-name}, [Brian Weis]{.contact-name}, and [Jensen
Zhang]{.contact-name} provided substantial review feedback and
suggestions to the protocol design.[¶](#section-appendix.a-1){.pilcrow}
:::
:::
...
::: {.left dir="auto"}
[New Haven]{.locality}, [CT]{.region} [06520]{.postal-code}
:::
...
::: {.left dir="auto"}
[Nico Schwan]{.fn .nameRole}
:::

::: {.left dir="auto"}
[Thales Deutschland]{.org}
:::

::: {.left dir="auto"}
[Lorenzstrasse 10]{.street-address}
:::

::: {.left dir="auto"}
[70435]{.postal-code} [Stuttgart]{.locality}
:::
