  RFC 8681       RLC FEC Scheme    January 2020
  -------------- ----------------- --------------
...
This document describes two fully specified Forward Erasure Correction
...
For each incoming ADU, an ADUI [MUST]{.bcp14} be created as follows.
...
Any implementation of this PRNG [MUST]{.bcp14} have the same output as
...
The two RLC FEC schemes require the computation of a linear combination
...
[Figure 6](#figure-6){.selfRef}: [Source FEC Payload ID Encoding
...
Packet can then be passed to the transport layer for transmission. The
...
::: {#fig_tinymt32_out_truncated_256}
...
::: {#table_tinymt32_out_truncated_16_huge_nb_small_seq}
  ...
  : [Table 1](#table-1){.selfRef}: [tinymt32_rand16() Occurrence
  Statistics](#name-tinymt32_rand16-occurrence-){.selfRef}
:::
...
sequence), increasing the seed value for each sequence, and perform
statistics on the number of occurrences of each of the 16 possible
values across the 20,000,000,000 numbers of all sequences. In this first
test, we consider 32-bit seed values in order to assess the PRNG quality
after output truncation to 4 bits.[¶](#section-appendix.b-5){.pilcrow}

The results ([Table
1](#table_tinymt32_out_truncated_16_huge_nb_small_seq){.xref}) show that
all possible values are almost equally represented, or said differently,
that the tinymt32_rand16() output converges to a uniform distribution
where each of the 16 possible values would appear exactly 1 / 16 \* 100
= 6.25% of times.[¶](#section-appendix.b-6){.pilcrow}
...
with value occurrences ranging between 6.2121% (i.e., 81,423 occurrences
out of a total of 65536\*20=1,310,720) and 6.2948% (i.e., 82,507
occurrences). However, we do not believe it significantly impacts the
RLC FEC scheme behavior.[¶](#section-appendix.b-7){.pilcrow}

Other types of biases may exist that may be visible with smaller tests,
for instance to evaluate the convergence speed to a uniform
distribution. We therefore perform 200 tests, each of them producing 200
sequences, keeping only the first value of each sequence. We use
non-overlapping repair keys for each sequence, starting with value 0 and
increasing it after each use.[¶](#section-appendix.b-8){.pilcrow}

[]{#name-tinymt32_rand16-occurrence-s}

::: {#table_tinymt32_out_truncated_16_small_nb_small_seq}
  ...
  : [Table 2](#table-2){.selfRef}: [tinymt32_rand16() Occurrence
  Statistics](#name-tinymt32_rand16-occurrence-s){.selfRef}
:::

[Table 2](#table_tinymt32_out_truncated_16_small_nb_small_seq){.xref}
shows across all 200 tests, for each of the 16 possible pseudorandom
number values, the minimum (resp. maximum) number of times it appeared
in a test, as well as the average number of occurrences across the 200
tests. Although the distribution is not perfect, there is no major bias.
On the contrary, in the same conditions, the Park-Miller linear
congruential PRNG of \[[RFC5170](#RFC5170){.xref}\] with a result scaled
down to 4-bit values, using seeds in sequence starting from 1,
systematically returns 0 as the first value during some time. Then,
after a certain repair key value threshold, it systematically returns 1,
etc.[¶](#section-appendix.b-10){.pilcrow}
...
times.[¶](#section-appendix.b-11){.pilcrow}
:::
:::

::: {#possible_param_derivation}
::: {#section-appendix.c .section}
## [Appendix C.](#section-appendix.c){.section-number .selfRef} [Possible Parameter Derivation (Informational)](#name-possible-parameter-derivati){.section-name .selfRef} {#name-possible-parameter-derivati}

[Section 3.1](#CommonProc_rlcParameters){.xref} defines several
parameters to control the encoder or decoder. This annex proposes
techniques to derive these parameters according to the target use-case.
This annex is informational, in the sense that using a different
derivation technique will not prevent the encoder and decoder to
interoperate: a decoder can still recover an erased source symbol
without any error. However, in case of a real-time flow, an
inappropriate parameter derivation may lead to the decoding of erased
source packets after their validity period, making them useless to the
target application. This annex proposes an approach to reduce this risk,
among other things.[¶](#section-appendix.c-1){.pilcrow}

The FEC schemes defined in this document can be used in various manners,
depending on the target use-case:[¶](#section-appendix.c-2){.pilcrow}

-   [the source ADU flow they protect may or may not have real-time
    constraints;[¶](#section-appendix.c-3.1){.pilcrow}]{#section-appendix.c-3.1}
-   [the source ADU flow may be a Constant Bitrate (CBR) or Variable
    Bitrate (VBR)
    flow;[¶](#section-appendix.c-3.2){.pilcrow}]{#section-appendix.c-3.2}
-   [with a VBR source ADU flow, the flow\'s minimum and maximum
    bitrates may or may not be
    known;[¶](#section-appendix.c-3.3){.pilcrow}]{#section-appendix.c-3.3}
-   [and the communication path between encoder and decoder may be a CBR
    communication path (e.g., as with certain LTE-based broadcast
    channels) or not (general case, e.g., with
    Internet).[¶](#section-appendix.c-3.4){.pilcrow}]{#section-appendix.c-3.4}

The parameter derivation technique should be suited to the use-case, as
described in the following sections.[¶](#section-appendix.c-4){.pilcrow}

::: {#param_derivation_cbr_realtime}
::: {#section-c.1 .section}
## [C.1.](#section-c.1){.section-number .selfRef} [Case of a CBR Real-Time Flow](#name-case-of-a-cbr-real-time-flo){.section-name .selfRef} {#name-case-of-a-cbr-real-time-flo}

In the following, we consider a real-time flow with max_lat latency
budget. The encoding symbol size, E, is constant. The code rate, cr, is
also constant, its value depending on the expected communication loss
model (this choice is out of scope of this
document).[¶](#section-c.1-1){.pilcrow}
...
value is known by the FECFRAME sender. It follows that the transmission
bitrate at the output of the FECFRAME sender will be higher, depending
on the added repair flow overhead. In order to comply with the maximum
FEC-related latency budget, we have:[¶](#section-c.1-2){.pilcrow}

-   [dw_max_size = (max_lat \* br_in) / (8 \*
    E)[¶](#section-c.1-3.1){.pilcrow}]{#section-c.1-3.1}

assuming that the encoding and decoding times are negligible with
respect to the target max_lat. This is a reasonable assumption in many
situations (e.g., see [Section 8.1](#oprecom_ff_considerations){.xref}
in case of small window sizes). Otherwise the max_lat parameter should
be adjusted in order to avoid the problem. In any case, interoperability
will never be compromised by choosing a too large
value.[¶](#section-c.1-4){.pilcrow}

In a second configuration, the FECFRAME sender generates a fixed bitrate
flow, equal to the CBR communication path bitrate equal to br_out (in
bits/s), and this value is known by the FECFRAME sender, as in
\[[Roca17](#Roca17){.xref}\]. The maximum source flow bitrate needs to
be such that, with the added repair flow overhead, the total
transmission bitrate remains inferior or equal to br_out. We
have:[¶](#section-c.1-5){.pilcrow}

-   [dw_max_size = (max_lat \* br_out \* cr) / (8 \*
    E)[¶](#section-c.1-6.1){.pilcrow}]{#section-c.1-6.1}

assuming here also that the encoding and decoding times are negligible
with respect to the target max_lat.[¶](#section-c.1-7){.pilcrow}

For decoding to be possible within the latency budget, it is required
that the encoding window maximum size be smaller than or at most equal
to the decoding window maximum size. The ew_max_size is the main
parameter at a FECFRAME sender, but its exact value has no impact on the
FEC-related latency budget. The ew_max_size parameter is computed as
follows:[¶](#section-c.1-8){.pilcrow}

-   [ew_max_size = dw_max_size \* WSR /
    255[¶](#section-c.1-9.1){.pilcrow}]{#section-c.1-9.1}

In line with \[[Roca17](#Roca17){.xref}\], WSR = 191 is considered as a
reasonable value (the resulting encoding to decoding window size ratio
is then close to 0.75), but other values between 1 and 255 inclusive are
possible, depending on the use-case.[¶](#section-c.1-10){.pilcrow}
...
received FEC Repair Packets ([Section
4.1.3](#ArbitraryFlows_repair_fpi){.xref}). A receiver can then easily
compute dw_max_size:[¶](#section-c.1-11){.pilcrow}

-   [dw_max_size = max_NSS_observed \* 255 /
    WSR[¶](#section-c.1-12.1){.pilcrow}]{#section-c.1-12.1}

A receiver can then choose an appropriate linear system maximum
size:[¶](#section-c.1-13){.pilcrow}

-   [ls_max_size >=
    dw_max_size[¶](#section-c.1-14.1){.pilcrow}]{#section-c.1-14.1}

It is good practice to use a larger value for ls_max_size as explained
in [Appendix D](#decodingBeyondMaxLatency){.xref}, which does not impact
maximum latency nor interoperability.[¶](#section-c.1-15){.pilcrow}

In any case, for a given use-case (i.e., for target encoding and
decoding devices and desired protection levels in front of communication
impairments) and for the computed ew_max_size, dw_max_size and
ls_max_size values, it is [RECOMMENDED]{.bcp14} to check that the
maximum encoding time and maximum memory requirements at a FECFRAME
sender, and maximum decoding time and maximum memory requirements at a
FECFRAME receiver, stay within reasonable bounds. When assuming that the
encoding and decoding times are negligible with respect to the target
max_lat, this should be verified as well, otherwise the max_lat
[SHOULD]{.bcp14} be adjusted accordingly.[¶](#section-c.1-16){.pilcrow}

The particular case of session start needs to be managed appropriately
since the ew_size, starting at zero, increases each time a new source
ADU is received by the FECFRAME sender, until it reaches the ew_max_size
value. Therefore, a FECFRAME receiver [SHOULD]{.bcp14} continuously
observe the received FEC Repair Packets, since the NSS value carried in
the Repair FEC Payload ID will increase too, and adjust its ls_max_size
accordingly if need be. With a CBR flow, session start is expected to be
the only moment when the encoding window size will increase. Similarly,
with a CBR real-time flow, the session end is expected to be the only
moment when the encoding window size will progressively decrease. No
adjustment of the ls_max_size is required at the FECFRAME receiver in
that case.[¶](#section-c.1-17){.pilcrow}
:::
:::

::: {#param_derivation_other_realtime_flows}
::: {#section-c.2 .section}
## [C.2.](#section-c.2){.section-number .selfRef} [Other Types of Real-Time Flow](#name-other-types-of-real-time-fl){.section-name .selfRef} {#name-other-types-of-real-time-fl}

In the following, we consider a real-time source ADU flow with a max_lat
latency budget and a variable bitrate (VBR) measured at the entry of the
FECFRAME sender. A first approach consists in considering the smallest
instantaneous bitrate of the source ADU flow, when this parameter is
known, and to reuse the derivation of [Appendix
C.1](#param_derivation_cbr_realtime){.xref}. Considering the smallest
bitrate means that the encoding and decoding window maximum size
estimations are pessimistic: these windows have the smallest size
required to enable on-time decoding at a FECFRAME receiver. If the
instantaneous bitrate is higher than this smallest bitrate, this
approach leads to an encoding window that is unnecessarily small, which
reduces robustness in front of long erasure
bursts.[¶](#section-c.2-1){.pilcrow}

Another approach consists in using ADU timing information (e.g., using
the timestamp field of an RTP packet header, or registering the time
upon receiving a new ADU). From the global FEC-related latency budget,
the FECFRAME sender can derive a practical maximum latency budget for
encoding operations, max_lat_for_encoding. For the FEC schemes specified
in this document, this latency budget [SHOULD]{.bcp14} be computed
with:[¶](#section-c.2-2){.pilcrow}

-   [max_lat_for_encoding = max_lat \* WSR /
    255[¶](#section-c.2-3.1){.pilcrow}]{#section-c.2-3.1}

It follows that any source symbols associated to an ADU that has
timed-out with respect to max_lat_for_encoding [SHOULD]{.bcp14} be
removed from the encoding window. With this approach there is no
pre-determined ew_size value: this value fluctuates over the time
according to the instantaneous source ADU flow bitrate. For practical
reasons, a FECFRAME sender may still require that ew_size does not
increase beyond a maximum value ([Appendix
C.3](#param_derivation_non_realtime){.xref}).[¶](#section-c.2-4){.pilcrow}

With both approaches, and no matter the choice of the FECFRAME sender, a
FECFRAME receiver can still easily evaluate the ew_max_size by observing
the maximum Number of Source Symbols (NSS) value contained in the Repair
FEC Payload ID of received FEC Repair Packets. A receiver can then
compute dw_max_size and derive an appropriate ls_max_size as explained
in [Appendix
C.1](#param_derivation_cbr_realtime){.xref}.[¶](#section-c.2-5){.pilcrow}

When the observed NSS fluctuates significantly, a FECFRAME receiver may
want to adapt its ls_max_size accordingly. In particular when the NSS is
significantly reduced, a FECFRAME receiver may want to reduce the
ls_max_size too in order to limit computation complexity. A balance must
be found between using an ls_max_size \"too large\" (which increases
computation complexity and memory requirements) and the opposite (which
reduces recovery performance).[¶](#section-c.2-6){.pilcrow}
:::
:::

::: {#param_derivation_non_realtime}
::: {#section-c.3 .section}
## [C.3.](#section-c.3){.section-number .selfRef} [Case of a Non-Real-Time Flow](#name-case-of-a-non-real-time-flo){.section-name .selfRef} {#name-case-of-a-non-real-time-flo}

Finally there are configurations where a source ADU flow has no
real-time constraints. FECFRAME and the FEC schemes defined in this
document can still be used. The choice of appropriate parameter values
can be directed by practical considerations. For instance, it can derive
from an estimation of the maximum memory amount that could be dedicated
to the linear system at a FECFRAME receiver, or the maximum computation
complexity at a FECFRAME receiver, both of them depending on the
ls_max_size parameter. The same considerations also apply to the
FECFRAME sender, where the maximum memory amount and computation
complexity depend on the ew_max_size
parameter.[¶](#section-c.3-1){.pilcrow}
...
ew_max_size by observing its maximum value over the
time.[¶](#section-c.3-2){.pilcrow}
:::
:::
:::
:::

::: {#decodingBeyondMaxLatency}
::: {#section-appendix.d .section}
## [Appendix D.](#section-appendix.d){.section-number .selfRef} [Decoding Beyond Maximum Latency Optimization (Informational)](#name-decoding-beyond-maximum-lat){.section-name .selfRef} {#name-decoding-beyond-maximum-lat}

This annex introduces non-normative considerations. It is provided as
suggestions, without any impact on interoperability. For more
information see
\[[Roca16](#Roca16){.xref}\].[¶](#section-appendix.d-1){.pilcrow}

With a real-time source ADU flow, it is possible to improve the decoding
performance of Sliding Window Codes without impacting maximum latency,
at the cost of extra memory and CPU overhead. The optimization consists,
for a FECFRAME receiver, to extend the linear system beyond the decoding
window maximum size, by keeping a certain number of old source symbols
whereas their associated ADUs
timed-out:[¶](#section-appendix.d-2){.pilcrow}

-   [ls_max_size >
    dw_max_size[¶](#section-appendix.d-3.1){.pilcrow}]{#section-appendix.d-3.1}

Usually the following choice is a good trade-off between decoding
performance and extra CPU overhead:[¶](#section-appendix.d-4){.pilcrow}

-   [ls_max_size = 2 \*
    dw_max_size[¶](#section-appendix.d-5.1){.pilcrow}]{#section-appendix.d-5.1}
...
Going below this threshold will not save a significant amount of memory
nor CPU cycles. Therefore:[¶](#section-appendix.d-6){.pilcrow}

-   [ls_max_size = max(2 \* dw_max_size,
    LS_MIN_SIZE_DEFAULT)[¶](#section-appendix.d-7.1){.pilcrow}]{#section-appendix.d-7.1}
...
packet losses, can choose to reduce the ls_max_size. In that case lost
ADUs will be recovered without relying on this
optimization.[¶](#section-appendix.d-8){.pilcrow}

[]{#name-relationship-between-parame}

::: {#fig_decoding_beyond_max_latency}
::: {#section-appendix.d-9.1 .artwork .art-text .alignLeft}
                                 ls_max_size
    /---------------------------------^-------------------------------\
...
[Figure 11](#figure-11){.selfRef}: [Relationship between Parameters to
Decode beyond Maximum
Latency](#name-relationship-between-parame){.selfRef}
:::

It means that source symbols, and therefore ADUs, may be decoded even if
the added latency exceeds the maximum value permitted by the application
(the \"late source symbols\" of [Figure
11](#fig_decoding_beyond_max_latency){.xref}). It follows that the
corresponding ADUs will not be useful to the application. However,
decoding these \"late symbols\" significantly improves the global
robustness in bad reception conditions and is therefore recommended for
receivers experiencing bad communication conditions
\[[Roca16](#Roca16){.xref}\]. In any case whether or not to use this
optimization and what exact value to use for the ls_max_size parameter
are local decisions made by each receiver independently, without any
impact on the other receivers nor on the
source.[¶](#section-appendix.d-10){.pilcrow}
:::
:::
...
The authors would like to thank the three TSVWG chairs, Wesley Eddy (our
shepherd), David Black, and Gorry Fairhurst; as well as Spencer Dawkins,
our responsible AD; and all those who provided comments \-- namely (in
alphabetical order), Alan DeKok, Jonathan Detchart, Russ Housley,
Emmanuel Lochin, Marie-Jose Montpetit, and Greg Skinner. Last but not
least, the authors are really grateful to the IESG members, in
particular Benjamin Kaduk, Mirja Kuehlewind, Eric Rescorla, Adam Roach,
and Roman Danyliw for their highly valuable feedback that greatly
contributed to improving this
specification.[¶](#section-appendix.e-1){.pilcrow}
:::
