...
A *context* is an implementation-specific structure that encodes the
...
The `psk` and `psk_id` fields [MUST]{.bcp14} appear together or not at
...
The secret export parameters consist of:[¶](#section-5.2-4){.pilcrow}
...
that define new KDFs [MUST]{.bcp14} specify bounds for these
variable-length parameters.[¶](#section-7.2.1-3){.pilcrow}

The [RECOMMENDED]{.bcp14} limit for these values is 64 bytes. This would
enable interoperability with implementations that statically allocate
memory for these inputs to avoid memory
allocations.[¶](#section-7.2.1-4){.pilcrow}

The values for `psk`, `psk_id`, `info`, and `ikm`, which are inputs to
`LabeledExtract()`, were computed with the following
expression:[¶](#section-7.2.1-5){.pilcrow}

::: {#section-7.2.1-6 .alignLeft .art-pseudocode .art-text .artwork}
    max_size_hash_input - Nb - size_version_label -
        size_suite_id - size_input_label

[¶](#section-7.2.1-6){.pilcrow}
:::

The value for `exporter_context`, which is an input to
`LabeledExpand()`, was computed with the following
expression:[¶](#section-7.2.1-7){.pilcrow}

::: {#section-7.2.1-8 .alignLeft .art-pseudocode .art-text .artwork}
    max_size_hash_input - Nb - Nh - size_version_label -
        size_suite_id - size_input_label - 2 - 1

[¶](#section-7.2.1-8){.pilcrow}
:::

In these equations, `max_size_hash_input` is the maximum input length of
the underlying hash function in bytes, `Nb` is the block size of the
...
maximum of which is 13 across all labels in this
document.[¶](#section-7.2.1-9){.pilcrow}
:::
:::
:::
:::
...
::: {#aead-ids}
::: {#section-7.3 .section}
### [7.3.](#section-7.3){.section-number .selfRef} [Authenticated Encryption with Associated Data (AEAD) Functions](#name-authenticated-encryption-wi){.section-name .selfRef} {#name-authenticated-encryption-wi}
...
::: {#aeadid-values}
  ...
  0x0001   AES-128-GCM        16    12    16    \[[GCM](#GCM){.xref}\]
  0x0002   AES-256-GCM        32    12    16    \[[GCM](#GCM){.xref}\]
  0x0003   ChaCha20Poly1305   32    12    16    \[[RFC8439](#RFC8439){.xref}\]
  0xFFFF   Export-only        N/A   N/A   N/A   RFC 9180

  : [Table 5](#table-5){.selfRef}: [AEAD IDs](#name-aead-ids){.selfRef}
:::

The `0xFFFF` AEAD ID is reserved for applications that only use the
Export interface; see [Section 5.3](#hpke-export){.xref} for more
details.[¶](#section-7.3-2){.pilcrow}
:::
:::
:::
:::
...
::: {#api-considerations}
::: {#section-8 .section}
## [8.](#section-8){.section-number .selfRef} [API Considerations](#name-api-considerations){.section-name .selfRef} {#name-api-considerations}

This section documents considerations for interfaces to implementations
of HPKE. This includes error handling considerations and recommendations
that improve interoperability when HPKE is used in
applications.[¶](#section-8-1){.pilcrow}

::: {#auxiliary-authenticated-application-information}
::: {#section-8.1 .section}
### [8.1.](#section-8.1){.section-number .selfRef} [Auxiliary Authenticated Application Information](#name-auxiliary-authenticated-app){.section-name .selfRef} {#name-auxiliary-authenticated-app}

HPKE has two places at which applications can specify auxiliary
authenticated information: (1) during context construction via the Setup
`info` parameter, and (2) during Context operations, i.e., with the
`aad` parameter for `Open()` and `Seal()`, and the `exporter_context`
parameter for `Export()`. Application information applicable to multiple
operations on a single Context should use the Setup `info` parameter.
This avoids redundantly processing this information for each Context
operation. In contrast, application information that varies on a
per-message basis should be specified via the Context APIs (`Seal()`,
`Open()`, or `Export()`).[¶](#section-8.1-1){.pilcrow}

Applications that only use the single-shot APIs described in [Section
6](#single-shot-apis){.xref} should use the Setup `info` parameter for
specifying auxiliary authenticated information. Implementations which
only expose single-shot APIs should not allow applications to use both
Setup `info` and Context `aad` or `exporter_context` auxiliary
information parameters.[¶](#section-8.1-2){.pilcrow}
:::
:::

::: {#api-errors}
::: {#section-8.2 .section}
### [8.2.](#section-8.2){.section-number .selfRef} [Errors](#name-errors){.section-name .selfRef} {#name-errors}

The high-level, public HPKE APIs specified in this document are all
fallible. These include the Setup functions and all encryption context
functions. For example, `Decap()` can fail if the encapsulated key `enc`
is invalid, and `Open()` may fail if ciphertext decryption fails. The
explicit errors generated throughout this specification, along with the
conditions that lead to each error, are as
follows:[¶](#section-8.2-1){.pilcrow}

-   [`ValidationError`: KEM input or output validation failure; [Section
    4.1](#dhkem){.xref}.[¶](#section-8.2-2.1){.pilcrow}]{#section-8.2-2.1}
-   [`DeserializeError`: Public or private key deserialization failure;
    [Section
    4](#base-crypto){.xref}.[¶](#section-8.2-2.2){.pilcrow}]{#section-8.2-2.2}
-   [`EncapError`: `Encap()` failure; [Section
    4](#base-crypto){.xref}.[¶](#section-8.2-2.3){.pilcrow}]{#section-8.2-2.3}
-   [`DecapError`: `Decap()` failure; [Section
    4](#base-crypto){.xref}.[¶](#section-8.2-2.4){.pilcrow}]{#section-8.2-2.4}
-   [`OpenError`: Context AEAD `Open()` failure; Sections
    [4](#base-crypto){.xref} and
    [5.2](#hpke-dem){.xref}.[¶](#section-8.2-2.5){.pilcrow}]{#section-8.2-2.5}
-   [`MessageLimitReachedError`: Context AEAD sequence number overflow;
    Sections [4](#base-crypto){.xref} and
    [5.2](#hpke-dem){.xref}.[¶](#section-8.2-2.6){.pilcrow}]{#section-8.2-2.6}
-   [`DeriveKeyPairError`: Key pair derivation failure; [Section
    7.1.3](#derive-key-pair){.xref}.[¶](#section-8.2-2.7){.pilcrow}]{#section-8.2-2.7}
...
errors. For example, for the DHKEM variant described in this
specification, the `Encap()` algorithm fails when given an invalid
...
produce invalid output if given the wrong sender public key. This error
is not detectable until subsequent AEAD
decryption.[¶](#section-8.2-3){.pilcrow}

The errors in this document are meant as a guide for implementors. They
are not an exhaustive list of all the errors an implementation might
emit. For example, future KEMs might have internal failure cases, or an
implementation might run out of memory.[¶](#section-8.2-4){.pilcrow}

How these errors are expressed in an API or handled by applications is
an implementation-specific detail. For example, some implementations may
abort or panic upon a `DeriveKeyPairError` failure given that it only
occurs with negligible probability, whereas other implementations may
retry the failed DeriveKeyPair operation. See [Section
7.1.3](#derive-key-pair){.xref} for more information. As another
example, some implementations of the DHKEM specified in this document
may choose to transform `ValidationError` from `DH()` into an
`EncapError` or `DecapError` from `Encap()` or `Decap()`, respectively,
whereas others may choose to raise `ValidationError`
unmodified.[¶](#section-8.2-5){.pilcrow}

Applications using HPKE APIs should not assume that the errors here are
complete, nor should they assume certain classes of errors will always
manifest the same way for all ciphersuites. For example, the DHKEM
specified in this document will emit a `DeserializationError` or
`ValidationError` if a KEM public key is invalid. However, a new KEM
might not have an efficient algorithm for determining whether or not a
public key is valid. In this case, an invalid public key might instead
yield an `OpenError` when trying to decrypt a
ciphertext.[¶](#section-8.2-6){.pilcrow}
:::
:::
:::
:::
...
::: {#sec-properties}
::: {#section-9.1 .section}
### [9.1.](#section-9.1){.section-number .selfRef} [Security Properties](#name-security-properties){.section-name .selfRef} {#name-security-properties}
...
detailed below:[¶](#section-9.1-1){.pilcrow}

-   [Message secrecy: Confidentiality of the sender\'s messages against
    chosen ciphertext
    attacks[¶](#section-9.1-2.1){.pilcrow}]{#section-9.1-2.1}
-   [Export key secrecy: Indistinguishability of each export secret from
    a uniformly random bitstring of equal length, i.e., `Context.Export`
    is a variable-length
    PRF[¶](#section-9.1-2.2){.pilcrow}]{#section-9.1-2.2}
-   [Sender authentication: Proof of sender origin for PSK, Auth, and
    AuthPSK modes[¶](#section-9.1-2.3){.pilcrow}]{#section-9.1-2.3}
...
recipient keys are the same.[¶](#section-9.1-3){.pilcrow}

HPKE mitigates malleability problems (called benign malleability
\[[SECG](#SECG){.xref}\]) in prior public key encryption standards based
on ECIES by including all public keys in the context of the key
schedule.[¶](#section-9.1-4){.pilcrow}

HPKE does not provide forward secrecy with respect to recipient
compromise. In the Base and Auth modes, the secrecy properties are only
expected to hold if the recipient private key `skR` is not compromised
at any point in time. In the PSK and AuthPSK modes, the secrecy
properties are expected to hold if the recipient private key `skR` and
the pre-shared key are not both compromised at any point in time. See
[Section 9.7](#non-goals){.xref} for more
details.[¶](#section-9.1-5){.pilcrow}
...
expected to hold if, at the time of message reception, the sender
private key skS and the pre-shared key are not both
compromised.[¶](#section-9.1-6){.pilcrow}
...
importance, HPKE has other non-goals that are described in [Section
9.7](#non-goals){.xref}: no tolerance of message reordering or loss, no
downgrade or replay prevention, no hiding of the plaintext length, and
no protection against bad ephemeral randomness. [Section
9.7](#non-goals){.xref} suggests application-level mitigations for some
of them.[¶](#section-9.1-7){.pilcrow}

::: {#kci}
::: {#section-9.1.1 .section}
#### [9.1.1.](#section-9.1.1){.section-number .selfRef} [Key-Compromise Impersonation](#name-key-compromise-impersonatio){.section-name .selfRef} {#name-key-compromise-impersonatio}

The DHKEM variants defined in this document are vulnerable to
key-compromise impersonation attacks \[[BJM97](#BJM97){.xref}\], which
means that sender authentication cannot be expected to hold in the Auth
mode if the recipient private key `skR` is compromised, and in the
AuthPSK mode if the pre-shared key and the recipient private key `skR`
are both compromised. NaCl\'s `box` interface \[[NaCl](#NaCl){.xref}\]
has the same issue. At the same time, this enables
repudiability.[¶](#section-9.1.1-1){.pilcrow}

As shown by \[[ABHKLR20](#ABHKLR20){.xref}\], key-compromise
impersonation attacks are generally possible on HPKE because KEM
ciphertexts are not bound to HPKE messages. An adversary who knows a
recipient\'s private key can decapsulate an observed KEM ciphertext,
compute the key schedule, and encrypt an arbitrary message that the
recipient will accept as coming from the original sender. Importantly,
this is possible even with a KEM that is resistant to key-compromise
impersonation attacks. As a result, mitigating this issue requires
fundamental changes that are out of scope of this
specification.[¶](#section-9.1.1-2){.pilcrow}

Applications that require resistance against key-compromise
impersonation [SHOULD]{.bcp14} take extra steps to prevent this attack.
One possibility is to produce a digital signature over `(enc, ct)`
tuples using a sender\'s private key \-- where `ct` is an AEAD
ciphertext produced by the single-shot or multi-shot API and `enc` is
the corresponding KEM encapsulated key.[¶](#section-9.1.1-3){.pilcrow}

Given these properties, pre-shared keys strengthen both the
authentication and the secrecy properties in certain adversary models.
One particular example in which this can be useful is a hybrid quantum
setting: if a non-quantum-resistant KEM used with HPKE is broken by a
quantum computer, the security properties are preserved through the use
of a pre-shared key. As described in [Section
7](https://www.rfc-editor.org/rfc/rfc8696#section-7){.relref} of
\[[RFC8696](#RFC8696){.xref}\] this assumes that the pre-shared key has
not been compromised.[¶](#section-9.1.1-4){.pilcrow}
:::
:::

::: {#computational-analysis}
::: {#section-9.1.2 .section}
#### [9.1.2.](#section-9.1.2){.section-number .selfRef} [Computational Analysis](#name-computational-analysis){.section-name .selfRef} {#name-computational-analysis}

It is shown in \[[CS01](#CS01){.xref}\] that a hybrid public key
encryption scheme of essentially the same form as the Base mode
described here is IND-CCA2-secure as long as the underlying KEM and AEAD
schemes are IND-CCA2-secure. Moreover, it is shown in
\[[HHK06](#HHK06){.xref}\] that IND-CCA2 security of the KEM and the
data encapsulation mechanism are necessary conditions to achieve
IND-CCA2 security for hybrid public key encryption. The main difference
between the scheme proposed in \[[CS01](#CS01){.xref}\] and the Base
mode in this document (both named HPKE) is that we interpose some KDF
calls between the KEM and the AEAD. Analyzing the HPKE Base mode
instantiation in this document therefore requires verifying that the
additional KDF calls do not cause the IND-CCA2 property to fail, as well
as verifying the additional export key secrecy
property.[¶](#section-9.1.2-1){.pilcrow}
...
HPKE can be viewed and analyzed as flavors of signcryption
\[[SigncryptionDZ10](#SigncryptionDZ10){.xref}\].[¶](#section-9.1.2-2){.pilcrow}

A preliminary computational analysis of all HPKE modes has been done in
\[[HPKEAnalysis](#HPKEAnalysis){.xref}\], indicating asymptotic security
for the case where the KEM is DHKEM, the AEAD is any IND-CPA-secure and
INT-CTXT-secure scheme, and the DH group and KDF satisfy the following
conditions:[¶](#section-9.1.2-3){.pilcrow}

-   [DH group: The gap Diffie-Hellman (GDH) problem is hard in the
    appropriate subgroup
    \[[GAP](#GAP){.xref}\].[¶](#section-9.1.2-4.1){.pilcrow}]{#section-9.1.2-4.1}
-   [`Extract()` and `Expand()`: `Extract()` can be modeled as a random
    oracle. `Expand()` can be modeled as a pseudorandom function,
    wherein the first argument is the
    key.[¶](#section-9.1.2-4.2){.pilcrow}]{#section-9.1.2-4.2}

In particular, the KDFs and DH groups defined in this document (see
Sections [7.2](#kdf-ids){.xref} and [7.1](#kem-ids){.xref}) satisfy
these properties when used as specified. The analysis in
\[[HPKEAnalysis](#HPKEAnalysis){.xref}\] demonstrates that under these
...
secrets using the secret export
interface.[¶](#section-9.1.2-5){.pilcrow}

The table below summarizes the main results from
\[[HPKEAnalysis](#HPKEAnalysis){.xref}\]. N/A means that a property does
not apply for the given mode, whereas `Y` means the given mode satisfies
the property.[¶](#section-9.1.2-6){.pilcrow}

[]{#name-hpke-mode-security-properti}
  ...
  : [Table 6](#table-6){.selfRef}: [HPKE Mode Security
  Properties](#name-hpke-mode-security-properti){.selfRef}

If non-DH-based KEMs are to be used with HPKE, further analysis will be
necessary to prove their security. The results from
\[[CS01](#CS01){.xref}\] provide some indication that any
IND-CCA2-secure KEM will suffice here, but are not conclusive given the
differences in the schemes.[¶](#section-9.1.2-8){.pilcrow}

A detailed computational analysis of HPKE\'s Auth mode single-shot
encryption API has been done in \[[ABHKLR20](#ABHKLR20){.xref}\]. The
paper defines security notions for authenticated KEMs and for
authenticated public key encryption, using the outsider and insider
security terminology known from signcryption
\[[SigncryptionDZ10](#SigncryptionDZ10){.xref}\]. The analysis proves
that DHKEM\'s `AuthEncap()`/`AuthDecap()` interface fulfills these
notions for all Diffie-Hellman groups specified in this document. The
analysis also provides exact security bounds, under the assumptions that
the gap Diffie-Hellman (GDH) problem is hard in the appropriate subgroup
\[[GAP](#GAP){.xref}\], and that HKDF can be modeled as a random
oracle.[¶](#section-9.1.2-9){.pilcrow}

Further, \[[ABHKLR20](#ABHKLR20){.xref}\] proves composition theorems,
showing that HPKE\'s Auth mode fulfills the security notions of
authenticated public key encryption for all KDFs and AEAD schemes
specified in this document, given any authenticated KEM satisfying the
previously defined security notions for authenticated KEMs. The theorems
assume that the KEM is perfectly correct; they could easily be adapted
to work with KEMs that have a nonzero but negligible probability for
decryption failure. The assumptions on the KDF are that `Extract()` and
`Expand()` can be modeled as pseudorandom functions wherein the first
argument is the key, respectively. The assumption for the AEAD is
IND-CPA and IND-CTXT security.[¶](#section-9.1.2-10){.pilcrow}

In summary, the analysis in \[[ABHKLR20](#ABHKLR20){.xref}\] proves that
the single-shot encryption API of HPKE\'s Auth mode satisfies the
desired message confidentiality and sender authentication properties
listed at the beginning of this section; it does not consider multiple
messages, nor the secret export API.[¶](#section-9.1.2-11){.pilcrow}
:::
:::

::: {#post-quantum-security}
::: {#section-9.1.3 .section}
#### [9.1.3.](#section-9.1.3){.section-number .selfRef} [Post-Quantum Security](#name-post-quantum-security){.section-name .selfRef} {#name-post-quantum-security}

All of \[[CS01](#CS01){.xref}\],
\[[HPKEAnalysis](#HPKEAnalysis){.xref}\], and
\[[ABHKLR20](#ABHKLR20){.xref}\] are premised on classical security
models and assumptions, and do not consider adversaries capable of
quantum computation. A full proof of post-quantum security would need to
take appropriate security models and assumptions into account, in
addition to simply using a post-quantum KEM. However, the composition
theorems from \[[ABHKLR20](#ABHKLR20){.xref}\] for HPKE\'s Auth mode
only make standard assumptions (i.e., no random oracle assumption) that
are expected to hold against quantum adversaries (although with slightly
worse bounds). Thus, these composition theorems, in combination with a
post-quantum-secure authenticated KEM, guarantee the post-quantum
security of HPKE\'s Auth mode.[¶](#section-9.1.3-1){.pilcrow}

In future work, the analysis from \[[ABHKLR20](#ABHKLR20){.xref}\] can
be extended to cover HPKE\'s other modes and desired security
properties. The hybrid quantum-resistance property described above,
which is achieved by using the PSK or AuthPSK mode, is not proven in
\[[HPKEAnalysis](#HPKEAnalysis){.xref}\] because this analysis requires
the random oracle model; in a quantum setting, this model needs adaption
to, for example, the quantum random oracle
model.[¶](#section-9.1.3-2){.pilcrow}
:::
:::
:::
:::
...
::: {#kem-security}
::: {#section-9.2 .section}
### [9.2.](#section-9.2){.section-number .selfRef} [Security Requirements on a KEM Used within HPKE](#name-security-requirements-on-a-){.section-name .selfRef} {#name-security-requirements-on-a-}

A KEM used within HPKE [MUST]{.bcp14} allow HPKE to satisfy its desired
security properties described in [Section 9.1](#sec-properties){.xref}.
[Section 9.6](#domain-separation){.xref} lists requirements concerning
domain separation.[¶](#section-9.2-1){.pilcrow}

In particular, the KEM shared secret [MUST]{.bcp14} be a uniformly
random byte string of length `Nsecret`. This means, for instance, that
it would not be sufficient if the KEM shared secret is only uniformly
random as an element of some set prior to its encoding as a byte
string.[¶](#section-9.2-2){.pilcrow}

::: {#encapdecap-interface}
::: {#section-9.2.1 .section}
#### [9.2.1.](#section-9.2.1){.section-number .selfRef} [Encap/Decap Interface](#name-encap-decap-interface){.section-name .selfRef} {#name-encap-decap-interface}

As mentioned in [Section 9](#sec-considerations){.xref},
\[[CS01](#CS01){.xref}\] provides some indications that if the KEM\'s
`Encap()`/`Decap()` interface (which is used in the Base and PSK modes)
is IND-CCA2-secure, HPKE is able to satisfy its desired security
properties. An appropriate definition of IND-CCA2 security for KEMs can
be found in \[[CS01](#CS01){.xref}\] and
\[[BHK09](#BHK09){.xref}\].[¶](#section-9.2.1-1){.pilcrow}
:::
:::

::: {#authencapauthdecap-interface}
::: {#section-9.2.2 .section}
#### [9.2.2.](#section-9.2.2){.section-number .selfRef} [AuthEncap/AuthDecap Interface](#name-authencap-authdecap-interfa){.section-name .selfRef} {#name-authencap-authdecap-interfa}

The analysis of HPKE\'s Auth mode single-shot encryption API in
\[[ABHKLR20](#ABHKLR20){.xref}\] provides composition theorems that
guarantee that HPKE\'s Auth mode achieves its desired security
properties if the KEM\'s `AuthEncap()`/`AuthDecap()` interface satisfies
multi-user Outsider-CCA, Outsider-Auth, and Insider-CCA security, as
defined in the same paper.[¶](#section-9.2.2-1){.pilcrow}
...
compromised. Insider-CCA security formalizes confidentiality of the KEM
shared secret in case the sender private key is known or chosen by the
adversary. (If the recipient private key is known or chosen by the
adversary, confidentiality is trivially broken, because then the
adversary knows all secrets on the recipient\'s
side).[¶](#section-9.2.2-2){.pilcrow}
...
notion of HPKE described in [Section 9.1](#sec-properties){.xref}, a
definition of Insider-Auth security for KEMs used within HPKE is not
useful.[¶](#section-9.2.2-3){.pilcrow}
:::
:::

::: {#kem-key-reuse}
::: {#section-9.2.3 .section}
#### [9.2.3.](#section-9.2.3){.section-number .selfRef} [KEM Key Reuse](#name-kem-key-reuse){.section-name .selfRef} {#name-kem-key-reuse}

An `ikm` input to `DeriveKeyPair()` ([Section
7.1.3](#derive-key-pair){.xref}) [MUST NOT]{.bcp14} be reused elsewhere,
in particular not with `DeriveKeyPair()` of a different
KEM.[¶](#section-9.2.3-1){.pilcrow}

The randomness used in `Encap()` and `AuthEncap()` to generate the KEM
shared secret or its encapsulation [MUST NOT]{.bcp14} be reused
elsewhere.[¶](#section-9.2.3-2){.pilcrow}

Since a KEM key pair belonging to a sender or recipient works with all
modes, it can be used with multiple modes in parallel. HPKE is
constructed to be secure in such settings due to domain separation using
the `suite_id` variable. However, there is no formal proof of security
at the time of writing for using multiple modes in parallel;
\[[HPKEAnalysis](#HPKEAnalysis){.xref}\] and
\[[ABHKLR20](#ABHKLR20){.xref}\] only analyze isolated
modes.[¶](#section-9.2.3-3){.pilcrow}
:::
:::
:::
:::
...
::: {#kdf-choice}
::: {#section-9.3 .section}
### [9.3.](#section-9.3){.section-number .selfRef} [Security Requirements on a KDF](#name-security-requirements-on-a-k){.section-name .selfRef} {#name-security-requirements-on-a-k}

The choice of the KDF for HPKE [SHOULD]{.bcp14} be made based on the
security level provided by the KEM and, if applicable, by the PSK. The
KDF [SHOULD]{.bcp14} at least have the security level of the KEM and
[SHOULD]{.bcp14} at least have the security level provided by the
PSK.[¶](#section-9.3-1){.pilcrow}
:::
:::

::: {#aead-security}
::: {#section-9.4 .section}
### [9.4.](#section-9.4){.section-number .selfRef} [Security Requirements on an AEAD](#name-security-requirements-on-an){.section-name .selfRef} {#name-security-requirements-on-an}

All AEADs [MUST]{.bcp14} be IND-CCA2-secure, as is currently true for
all AEADs listed in [Section
7.3](#aead-ids){.xref}.[¶](#section-9.4-1){.pilcrow}
:::
:::

::: {#security-psk}
::: {#section-9.5 .section}
### [9.5.](#section-9.5){.section-number .selfRef} [Pre-Shared Key Recommendations](#name-pre-shared-key-recommendati){.section-name .selfRef} {#name-pre-shared-key-recommendati}

In the PSK and AuthPSK modes, the PSK [MUST]{.bcp14} have at least 32
bytes of entropy and [SHOULD]{.bcp14} be of length `Nh` bytes or longer.
Using a PSK longer than 32 bytes but shorter than `Nh` bytes is
permitted.[¶](#section-9.5-1){.pilcrow}

HPKE is specified to use HKDF as its key derivation function. HKDF is
not designed to slow down dictionary attacks (see
\[[RFC5869](#RFC5869){.xref}\]). Thus, HPKE\'s PSK mechanism is not
suitable for use with a low-entropy password as the PSK: In scenarios in
which the adversary knows the KEM shared secret `shared_secret` and has
access to an oracle that distinguishes between a good and a wrong PSK,
it can perform PSK-recovering attacks. This oracle can be the decryption
operation on a captured HPKE ciphertext or any other recipient behavior
that is observably different when using a wrong PSK. The adversary knows
the KEM shared secret `shared_secret` if it knows all KEM private keys
of one participant. In the PSK mode, this is trivially the case if the
adversary acts as the sender.[¶](#section-9.5-2){.pilcrow}
...
values, the attacker generates an HPKE ciphertext for each value in `S`,
and submits the resulting ciphertexts to the oracle to learn which PSK
is being used by the recipient. Further, because HPKE uses AEAD schemes
that are not key-committing, an attacker can mount a partitioning oracle
attack \[[LGR20](#LGR20){.xref}\] that can recover the PSK from a set of
`S` possible PSK values, with \|S\| = m\*k, in roughly m + log k queries
to the oracle using ciphertexts of length proportional to k, the maximum
message length in blocks. (Applying the multi-collision algorithm from
\[[LGR20](#LGR20){.xref}\] requires a small adaptation to the algorithm
wherein the appropriate nonce is computed for each candidate key. This
modification adds one call to HKDF per key. The number of partitioning
oracle queries remains unchanged.) As a result, the PSK must therefore
be chosen with sufficient entropy so that m + log k is prohibitive for
attackers (e.g., 2\^128). Future specifications can define new AEAD
algorithms that are key-committing.[¶](#section-9.5-3){.pilcrow}
:::
:::

::: {#domain-separation}
::: {#section-9.6 .section}
### [9.6.](#section-9.6){.section-number .selfRef} [Domain Separation](#name-domain-separation){.section-name .selfRef} {#name-domain-separation}
...
independent functions even if instantiated by the same KDF. This domain
separation between DHKEM and the remainder of HPKE is achieved by using
prefix-free sets of `suite_id` values in `LabeledExtract()` and
...
another within the set.[¶](#section-9.6-1){.pilcrow}

Future KEM instantiations [MUST]{.bcp14} ensure, should `Extract()` and
...
and `LabeledExpand()` with a `suite_id` as defined in [Section
4](#base-crypto){.xref}, which will ensure input domain separation, as
outlined above. Particular attention needs to be paid if the KEM
directly invokes functions that are used internally in HPKE\'s
`Extract()` or `Expand()`, such as `Hash()` and `HMAC()` in the case of
HKDF. It [MUST]{.bcp14} be ensured that inputs to these invocations
cannot collide with inputs to the internal invocations of these
functions inside `Extract()` or `Expand()`. In HPKE\'s `KeySchedule()`
this is avoided by using `Extract()` instead of `Hash()` on the
arbitrary-length inputs `info` and
`psk_id`.[¶](#section-9.6-2){.pilcrow}

The string literal \"HPKE-v1\" used in `LabeledExtract()` and
`LabeledExpand()` ensures that any secrets derived in HPKE are bound to
the scheme\'s name and version, even when possibly derived from the same
Diffie-Hellman or KEM shared secret as in another scheme or
version.[¶](#section-9.6-3){.pilcrow}
:::
:::

::: {#non-goals}
::: {#section-9.7 .section}
### [9.7.](#section-9.7){.section-number .selfRef} [Application Embedding and Non-Goals](#name-application-embedding-and-n){.section-name .selfRef} {#name-application-embedding-and-n}

HPKE is designed to be a fairly low-level mechanism. As a result, it
assumes that certain properties are provided by the application in which
HPKE is embedded and leaves certain security properties to be provided
by other mechanisms. Otherwise said, certain properties are out of scope
for HPKE.[¶](#section-9.7-1){.pilcrow}

::: {#message-order-and-message-loss}
::: {#section-9.7.1 .section}
#### [9.7.1.](#section-9.7.1){.section-number .selfRef} [Message Order and Message Loss](#name-message-order-and-message-l){.section-name .selfRef} {#name-message-order-and-message-l}

The primary requirement that HPKE imposes on applications is the
requirement that ciphertexts [MUST]{.bcp14} be presented to
`ContextR.Open()` in the same order in which they were generated by
`ContextS.Seal()`. When the single-shot API is used (see [Section
6](#single-shot-apis){.xref}), this is trivially true (since there is
only ever one ciphertext). Applications that allow for multiple
invocations of `Open()` / `Seal()` on the same context [MUST]{.bcp14}
enforce the ordering property described
above.[¶](#section-9.7.1-1){.pilcrow}
...
Whatever information is used to determine the ordering of HPKE-encrypted
messages [SHOULD]{.bcp14} be included in the associated data passed to
`ContextS.Seal()` and `ContextR.Open()`. The specifics of this scheme
are up to the application.[¶](#section-9.7.1-2){.pilcrow}

HPKE is not tolerant of lost messages. Applications [MUST]{.bcp14} be
able to detect when a message has been lost. When an unrecoverable loss
is detected, the application [MUST]{.bcp14} discard any associated HPKE
context.[¶](#section-9.7.1-3){.pilcrow}
:::
:::

::: {#downgrade-prevention}
::: {#section-9.7.2 .section}
#### [9.7.2.](#section-9.7.2){.section-number .selfRef} [Downgrade Prevention](#name-downgrade-prevention){.section-name .selfRef} {#name-downgrade-prevention}

HPKE assumes that the sender and recipient agree on what algorithms to
use. Depending on how these algorithms are negotiated, it may be
possible for an intermediary to force the two parties to use suboptimal
algorithms.[¶](#section-9.7.2-1){.pilcrow}
:::
:::

::: {#replay-protection}
::: {#section-9.7.3 .section}
#### [9.7.3.](#section-9.7.3){.section-number .selfRef} [Replay Protection](#name-replay-protection){.section-name .selfRef} {#name-replay-protection}
...
protection.[¶](#section-9.7.3-1){.pilcrow}
:::
:::

::: {#forward-secrecy}
::: {#section-9.7.4 .section}
#### [9.7.4.](#section-9.7.4){.section-number .selfRef} [Forward Secrecy](#name-forward-secrecy){.section-name .selfRef} {#name-forward-secrecy}
...
encrypted under said secrets. This is because only long-term secrets are
used on the side of the recipient.[¶](#section-9.7.4-1){.pilcrow}

HPKE ciphertexts are forward secret with respect to sender compromise in
all modes. This is because ephemeral randomness is used on the sender\'s
side, which is supposed to be erased directly after computation of the
KEM shared secret and ciphertext.[¶](#section-9.7.4-2){.pilcrow}
:::
:::

::: {#bad-ephemeral-randomness}
::: {#section-9.7.5 .section}
#### [9.7.5.](#section-9.7.5){.section-number .selfRef} [Bad Ephemeral Randomness](#name-bad-ephemeral-randomness){.section-name .selfRef} {#name-bad-ephemeral-randomness}
...
completely; in the other modes, at least forward secrecy with respect to
sender compromise can be lost completely.[¶](#section-9.7.5-1){.pilcrow}

Such a situation could also lead to the reuse of the same KEM shared
secret and thus to the reuse of same key-nonce pairs for the AEAD. The
AEADs specified in this document are not secure in case of nonce reuse.
This attack vector is particularly relevant in authenticated modes
because knowledge of the ephemeral randomness is not enough to derive
`shared_secret` in these modes.[¶](#section-9.7.5-2){.pilcrow}
...
secret that has been generated securely, as described in
\[[RFC8937](#RFC8937){.xref}\].[¶](#section-9.7.5-3){.pilcrow}
:::
:::

::: {#hiding-plaintext-length}
::: {#section-9.7.6 .section}
#### [9.7.6.](#section-9.7.6){.section-number .selfRef} [Hiding Plaintext Length](#name-hiding-plaintext-length){.section-name .selfRef} {#name-hiding-plaintext-length}
...
padding mechanism. See \[[TLS-ECH](#I-D.ietf-tls-esni){.xref}\] and
\[[RFC8467](#RFC8467){.xref}\] for examples of protocol-specific padding
policies.[¶](#section-9.7.6-1){.pilcrow}
:::
:::
:::
:::
...
::: {#bidirectional}
::: {#section-9.8 .section}
### [9.8.](#section-9.8){.section-number .selfRef} [Bidirectional Encryption](#name-bidirectional-encryption){.section-name .selfRef} {#name-bidirectional-encryption}

As discussed in [Section 5.2](#hpke-dem){.xref}, HPKE encryption is
unidirectional from sender to recipient. Applications that require
bidirectional encryption can derive necessary keying material with the
secret export interface ([Section 5.3](#hpke-export){.xref}). The type
and length of such keying material depends on the application use
case.[¶](#section-9.8-1){.pilcrow}
...
corresponding HPKE context as follows:[¶](#section-9.8-2){.pilcrow}
...
[¶](#section-9.8-3){.pilcrow}
:::

In this example, the length of each secret is based on the AEAD
algorithm used for the corresponding HPKE
context.[¶](#section-9.8-4){.pilcrow}
...
particular, in the Base mode, there is no authentication of the remote
party at all. Even in the Auth mode, where the remote party has proven
that they hold a specific private key, this authentication is still
subject to key-compromise impersonation, as discussed in [Section
9.1.1](#kci){.xref}.[¶](#section-9.8-5){.pilcrow}
:::
:::

::: {#metadata-protection}
::: {#section-9.9 .section}
### [9.9.](#section-9.9){.section-number .selfRef} [Metadata Protection](#name-metadata-protection){.section-name .selfRef} {#name-metadata-protection}

The authenticated modes of HPKE (PSK, Auth, and AuthPSK) require that
the recipient know what key material to use for the sender. This can be
signaled in applications by sending the PSK ID (`psk_id` above) and/or
the sender\'s public key (`pkS`). However, these values themselves might
be considered sensitive, since, in a given application context, they
might identify the sender.[¶](#section-9.9-1){.pilcrow}

An application that wishes to protect these metadata values without
requiring further provisioning of keys can use an additional instance of
HPKE, using the unauthenticated Base mode. Where the application might
have sent `(psk_id, pkS, enc, ciphertext)` before, it would now send
`(enc2, ciphertext2, enc, ciphertext)`, where `(enc2, ciphertext2)`
represent the encryption of the `psk_id` and `pkS`
values.[¶](#section-9.9-2){.pilcrow}
...
schemes in \[[BNT19](#BNT19){.xref}\] could be extended to cover other
metadata. However, this construction would require further
analysis.[¶](#section-9.9-3){.pilcrow}
:::
:::
:::
:::
...
::: {#message-encoding}
::: {#section-10 .section}
## [10.](#section-10){.section-number .selfRef} [Message Encoding](#name-message-encoding){.section-name .selfRef} {#name-message-encoding}

This document does not specify a wire format encoding for HPKE messages.
Applications that adopt HPKE must therefore specify an unambiguous
encoding mechanism that includes, minimally: the encapsulated value
`enc`, ciphertext value(s) (and order if there are multiple), and any
info values that are not implicit. One example of a non-implicit value
is the recipient public key used for encapsulation, which may be needed
if a recipient has more than one public key.[¶](#section-10-1){.pilcrow}

The AEAD interface used in this document is based on
\[[RFC5116](#RFC5116){.xref}\], which produces and consumes a single
ciphertext value. As discussed in \[[RFC5116](#RFC5116){.xref}\], this
ciphertext value contains the encrypted plaintext as well as any
authentication data, encoded in a manner described by the individual
AEAD scheme. Some implementations are not structured in this way,
instead providing a separate ciphertext and authentication tag. When
such AEAD implementations are used in HPKE implementations, the HPKE
implementation must combine these inputs into a single ciphertext value
within `Seal()` and parse them out within `Open()`, where the parsing
details are defined by the AEAD scheme. For example, with the AES-GCM
schemes specified in this document, the GCM authentication tag is placed
in the last Nt bytes of the ciphertext
output.[¶](#section-10-2){.pilcrow}
:::
:::
...
IANA has created three new registries:[¶](#section-11-1){.pilcrow}

-   [HPKE KEM
    Identifiers[¶](#section-11-2.1){.pilcrow}]{#section-11-2.1}
-   [HPKE KDF
    Identifiers[¶](#section-11-2.2){.pilcrow}]{#section-11-2.2}
-   [HPKE AEAD
    Identifiers[¶](#section-11-2.3){.pilcrow}]{#section-11-2.3}

All these registries are under \"Hybrid Public Key Encryption\", and
administered under a Specification Required policy
\[[RFC8126](#RFC8126){.xref}\].[¶](#section-11-3){.pilcrow}

::: {#kem-template}
::: {#section-11.1 .section}
### [11.1.](#section-11.1){.section-number .selfRef} [KEM Identifiers](#name-kem-identifiers){.section-name .selfRef} {#name-kem-identifiers}

The \"HPKE KEM Identifiers\" registry lists identifiers for key
encapsulation algorithms defined for use with HPKE. These identifiers
are two-byte values, so the maximum possible value is 0xFFFF =
65535.[¶](#section-11.1-1){.pilcrow}
...
:   []{.break}
    ...
    Value:
    :   The two-byte identifier for the
        algorithm[¶](#section-11.1-2.2.1.2){.pilcrow}
    :   

    KEM:
    :   The name of the algorithm[¶](#section-11.1-2.2.1.4){.pilcrow}
    :   

    Nsecret:
    :   The length in bytes of a KEM shared secret produced by the
        algorithm[¶](#section-11.1-2.2.1.6){.pilcrow}
    :   

    Nenc:
    :   The length in bytes of an encoded encapsulated key produced by
        the algorithm[¶](#section-11.1-2.2.1.8){.pilcrow}
    :   

    Npk:
    :   The length in bytes of an encoded public key for the
        algorithm[¶](#section-11.1-2.2.1.10){.pilcrow}
    :   

    Nsk:
    :   The length in bytes of an encoded private key for the
        algorithm[¶](#section-11.1-2.2.1.12){.pilcrow}
    :   

    Auth:
    :   A boolean indicating if this algorithm provides the
        `AuthEncap()`/`AuthDecap()`
        interface[¶](#section-11.1-2.2.1.14){.pilcrow}
    :   

    Reference:
    :   Where this algorithm is
        defined[¶](#section-11.1-2.2.1.16){.pilcrow}
    :   
:   
...
Initial contents:
:   Provided in [Table
    2](#kemid-values){.xref}[¶](#section-11.1-3.2){.pilcrow}
:   
:::
:::
...
### [11.2.](#section-11.2){.section-number .selfRef} [KDF Identifiers](#name-kdf-identifiers){.section-name .selfRef} {#name-kdf-identifiers}

The \"HPKE KDF Identifiers\" registry lists identifiers for key
derivation functions defined for use with HPKE. These identifiers are
two-byte values, so the maximum possible value is 0xFFFF =
65535.[¶](#section-11.2-1){.pilcrow}
...
:   []{.break}
    ...
    Value:
    :   The two-byte identifier for the
        algorithm[¶](#section-11.2-2.2.1.2){.pilcrow}
    :   

    KDF:
    :   The name of the algorithm[¶](#section-11.2-2.2.1.4){.pilcrow}
    :   

    Nh:
    :   The output size of the Extract function in
        bytes[¶](#section-11.2-2.2.1.6){.pilcrow}
    :   

    Reference:
    :   Where this algorithm is
        defined[¶](#section-11.2-2.2.1.8){.pilcrow}
    :   
:   
...
Initial contents:
:   Provided in [Table
    3](#kdfid-values){.xref}[¶](#section-11.2-3.2){.pilcrow}
:   
:::
:::
...
### [11.3.](#section-11.3){.section-number .selfRef} [AEAD Identifiers](#name-aead-identifiers){.section-name .selfRef} {#name-aead-identifiers}

The \"HPKE AEAD Identifiers\" registry lists identifiers for
authenticated encryption with associated data (AEAD) algorithms defined
for use with HPKE. These identifiers are two-byte values, so the maximum
possible value is 0xFFFF = 65535.[¶](#section-11.3-1){.pilcrow}
...
:   []{.break}
    ...
    Value:
    :   The two-byte identifier for the
        algorithm[¶](#section-11.3-2.2.1.2){.pilcrow}
    :   

    AEAD:
    :   The name of the algorithm[¶](#section-11.3-2.2.1.4){.pilcrow}
    :   

    Nk:
    :   The length in bytes of a key for this
        algorithm[¶](#section-11.3-2.2.1.6){.pilcrow}
    :   

    Nn:
    :   The length in bytes of a nonce for this
        algorithm[¶](#section-11.3-2.2.1.8){.pilcrow}
    :   

    Nt:
    :   The length in bytes of an authentication tag for this
        algorithm[¶](#section-11.3-2.2.1.10){.pilcrow}
    :   

    Reference:
    :   Where this algorithm is
        defined[¶](#section-11.3-2.2.1.12){.pilcrow}
    :   
:   
...
Initial contents:
:   Provided in [Table
    5](#aeadid-values){.xref}[¶](#section-11.3-3.2){.pilcrow}
:   
:::
:::
:::
...
\[ABHKLR20\]
:   [Alwen, J.]{.refAuthor}, [Blanchet, B.]{.refAuthor},
    [Hauck, E.]{.refAuthor}, [Kiltz, E.]{.refAuthor},
    [Lipp, B.]{.refAuthor}, and [D. Riepel]{.refAuthor}, [\"Analysing
    the HPKE Standard\"]{.refTitle}, November 2020,
    \<<https://eprint.iacr.org/2020/1499>\>.
:   

\[ANSI\]
:   [American National Standards Institute (ANSI)]{.refAuthor},
    ...
    Cryptography\"]{.refTitle}, November 2001.
:   

\[BHK09\]
:   [Bellare, M.]{.refAuthor}, [Hofheinz, D.]{.refAuthor}, and [E.
    Kiltz]{.refAuthor}, [\"Subtleties in the Definition of IND-CCA: When
    and How Should Challenge-Decryption be Disallowed?\"]{.refTitle},
    August 2009, \<<https://eprint.iacr.org/2009/418>\>.
:   

\[BJM97\]
:   [Blake-Wilson, S.]{.refAuthor}, [Johnson, D.]{.refAuthor}, and [A.
    Menezes]{.refAuthor}, [\"Key agreement protocols and their security
    analysis: Extended Abstract\"]{.refTitle}, [Crytography and Coding,
    pp. 30-45]{.refContent}, [DOI 10.1007/bfb0024447]{.seriesInfo}, June
    2005, \<<https://doi.org/10.1007/bfb0024447>\>.
:   

\[BNT19\]
:   [Bellare, M.]{.refAuthor}, [Ng, R.]{.refAuthor}, and [B.
    Tackmann]{.refAuthor}, [\"Nonces Are Noticed: AEAD
    Revisited\"]{.refTitle}, August 2019,
    \<[http://dx.doi.org/10.1007/978-3-030-26948-7_9](https://dx.doi.org/10.1007/978-3-030-26948-7_9)\>.
:   

\[CS01\]
:   [Cramer, R.]{.refAuthor} and [V. Shoup]{.refAuthor}, [\"Design and
    Analysis of Practical Public-Key Encryption Schemes Secure against
    Adaptive Chosen Ciphertext Attack\"]{.refTitle}, December 2001,
    \<<https://eprint.iacr.org/2001/108>\>.
:   

\[GAP\]
:   [Okamoto, T.]{.refAuthor} and [D. Pointcheval]{.refAuthor}, [\"The
    Gap-Problems: A New Class of Problems for the Security of
    Cryptographic Schemes\"]{.refTitle}, [ISBN
    978-3-540-44586-9]{.seriesInfo}, June 2001,
...
:   [Dworkin, M.]{.refAuthor}, [\"Recommendation for Block Cipher Modes
    ...
    10.6028/nist.sp.800-38d]{.seriesInfo}, [SP 800-38D]{.seriesInfo},
    November 2007, \<<https://doi.org/10.6028/nist.sp.800-38d>\>.
:   

\[HHK06\]
:   [Herranz, J.]{.refAuthor}, [Hofheinz, D.]{.refAuthor}, and [E.
    Kiltz]{.refAuthor}, [\"Some (in)sufficient conditions for secure
    hybrid encryption.\"]{.refTitle}, August 2006,
    \<<https://eprint.iacr.org/2006/265>\>.
:   

\[HPKEAnalysis\]
:   [Lipp, B.]{.refAuthor}, [\"An Analysis of Hybrid Public Key
    Encryption\"]{.refTitle}, February 2020,
    \<<https://eprint.iacr.org/2020/243>\>.
:   

\[IEEE1363\]
:   [IEEE]{.refAuthor}, [\"IEEE Standard Specifications for Public-Key
    Cryptography - Amendment 1: Additional Techniques\"]{.refTitle},
    [IEEE Std 1363a-2004]{.seriesInfo}.
:   

\[IMB\]
:   [Diffie, W.]{.refAuthor}, [Van Oorschot, P.]{.refAuthor}, and [M.
    Wiener]{.refAuthor}, [\"Authentication and authenticated key
    exchanges\"]{.refTitle}, [Designs, Codes and Cryptography, Vol. 2,
    pp. 107-125]{.refContent}, [DOI 10.1007/bf00124891]{.seriesInfo},
    June 1992, \<<https://doi.org/10.1007/bf00124891>\>.
:   

\[ISO\]
:   [International Organization for Standardization]{.refAuthor},
    [\"Information technology - Security techniques - Encryption
    algorithms - Part 2: Asymmetric ciphers\"]{.refTitle}, [ISO/IEC
    18033-2:2006]{.seriesInfo}, May 2006.
:   
...
\[LGR20\]
:   [Len, J.]{.refAuthor}, [Grubbs, P.]{.refAuthor}, and [T.
    Ristenpart]{.refAuthor}, [\"Partitioning Oracle
    Attacks\"]{.refTitle}.
:   

\[MAEA10\]
:   [Gayoso Martinez, V.]{.refAuthor}, [Hernandez
    Alvarez, F.]{.refAuthor}, [Hernandez Encinas, L.]{.refAuthor}, and
    [C. Sanchez Avila]{.refAuthor}, [\"A comparison of the standardized
    versions of ECIES\"]{.refTitle}, October 2010,
    \<<https://ieeexplore.ieee.org/abstract/document/5604194/>\>.
:   

\[MLS-PROTOCOL\]
:   [Barnes, R.]{.refAuthor}, [Beurdouche, B.]{.refAuthor},
    [Robert, R.]{.refAuthor}, [Millican, J.]{.refAuthor},
    [Omara, E.]{.refAuthor}, and [K. Cohn-Gordon]{.refAuthor}, [\"The
    Messaging Layer Security (MLS) Protocol\"]{.refTitle}, [Work in
    Progress]{.refContent}, [Internet-Draft,
    draft-ietf-mls-protocol-12]{.seriesInfo}, 11 October 2021,
    \<<https://datatracker.ietf.org/doc/html/draft-ietf-mls-protocol-12>\>.
:   

\[NaCl\]
:   [\"Public-key authenticated encryption: crypto_box\"]{.refTitle},
    March 2019, \<<https://nacl.cr.yp.to/box.html>\>.
:   
...
\[RFC1421\]
:   [Linn, J.]{.refAuthor}, [\"Privacy Enhancement for Internet
    Electronic Mail: Part I: Message Encryption and Authentication
    Procedures\"]{.refTitle}, [RFC 1421]{.seriesInfo}, [DOI
    10.17487/RFC1421]{.seriesInfo}, February 1993,
...
\[RFC8696\]
:   [Housley, R.]{.refAuthor}, [\"Using Pre-Shared Key (PSK) in the
    Cryptographic Message Syntax (CMS)\"]{.refTitle}, [RFC
    8696]{.seriesInfo}, [DOI 10.17487/RFC8696]{.seriesInfo}, December
    2019, \<<https://www.rfc-editor.org/info/rfc8696>\>.
:   

\[RFC8937\]
:   [Cremers, C.]{.refAuthor}, [Garratt, L.]{.refAuthor},
    [Smyshlyaev, S.]{.refAuthor}, [Sullivan, N.]{.refAuthor}, and [C.
    Wood]{.refAuthor}, [\"Randomness Improvements for Security
    Protocols\"]{.refTitle}, [RFC 8937]{.seriesInfo}, [DOI
    10.17487/RFC8937]{.seriesInfo}, October 2020,
...
:   [Standards for Efficient Cryptography Group]{.refAuthor}, [\"SEC 1:
    ...
    2]{.refContent}, May 2009, \<<https://secg.org/sec1-v2.pdf>\>.
:   

\[SigncryptionDZ10\]
:   [Dent, A.]{.refAuthor} and [Y. Zheng]{.refAuthor}, [\"Practical
    Signcryption\"]{.refTitle}, [Information Security and
    Cryptography]{.refContent}, [DOI
    10.1007/978-3-540-89411-7]{.seriesInfo}, November 2010,
    \<<https://doi.org/10.1007/978-3-540-89411-7>\>.
:   

\[TestVectors\]
:   [\"HPKE Test Vectors\"]{.refTitle},
...
Each section below contains test vectors for a single HPKE ciphersuite
and contains the following values:[¶](#appendix-A-1){.pilcrow}
...
1.  [Configuration information and private key material: This includes
    ...
    material. For each role X, where X is one of S, R, or E, as sender,
    recipient, and ephemeral, respectively, key pairs are generated as
    `(skX, pkX) = DeriveKeyPair(ikmX)`. Each key pair `(skX,  pkX)` is
    written in its serialized form, where
    `skXm =  SerializePrivateKey(skX)` and
    `pkXm = SerializePublicKey(pkX)`. For applicable modes, the shared
    PSK and PSK identifier are also
    included.[¶](#appendix-A-2.1){.pilcrow}]{#appendix-A-2.1}
2.  [Context creation intermediate values and outputs: This includes the
    KEM outputs `enc` and `shared_secret` used to create the context,
    along with intermediate values `key_schedule_context` and `secret`
    computed in the KeySchedule function in [Section
    5.1](#encryption-context){.xref}. The outputs include the context
    values `key`, `base_nonce`, and
    `exporter_secret`.[¶](#appendix-A-2.2){.pilcrow}]{#appendix-A-2.2}
3.  [Encryption test vectors: A fixed plaintext message is encrypted
    ...
    `ct`.[¶](#appendix-A-2.3){.pilcrow}]{#appendix-A-2.3}
4.  [Export test vectors: Several exported values of the same length
    ...
    output length `L`, and resulting export
    value.[¶](#appendix-A-2.4){.pilcrow}]{#appendix-A-2.4}

These test vectors are also available in JSON format at
\[[TestVectors](#TestVectors){.xref}\].[¶](#appendix-A-3){.pilcrow}

::: {#dhkemx25519-hkdf-sha256-hkdf-sha256-aes-128-gcm}
::: {#appendix-A.1 .section}
### [A.1.](#appendix-A.1){.section-number .selfRef} [DHKEM(X25519, HKDF-SHA256), HKDF-SHA256, AES-128-GCM](#name-dhkemx25519-hkdf-sha256-hkd){.section-name .selfRef} {#name-dhkemx25519-hkdf-sha256-hkd}

::: {#base-setup-information}
::: {#appendix-A.1.1 .section}
#### [A.1.1.](#appendix-A.1.1){.section-number .selfRef} [Base Setup Information](#name-base-setup-information){.section-name .selfRef} {#name-base-setup-information}
...
##### [A.1.1.1.](#appendix-A.1.1.1){.section-number .selfRef} [Encryptions](#name-encryptions){.section-name .selfRef} {#name-encryptions}
...
##### [A.1.1.2.](#appendix-A.1.1.2){.section-number .selfRef} [Exported Values](#name-exported-values){.section-name .selfRef} {#name-exported-values}
...
#### [A.1.2.](#appendix-A.1.2){.section-number .selfRef} [PSK Setup Information](#name-psk-setup-information){.section-name .selfRef} {#name-psk-setup-information}
...
::: {#encryptions-1}
::: {#appendix-A.1.2.1 .section}
##### [A.1.2.1.](#appendix-A.1.2.1){.section-number .selfRef} [Encryptions](#name-encryptions-2){.section-name .selfRef} {#name-encryptions-2}
...
[¶](#appendix-A.1.2.1-1){.pilcrow}
:::
:::
:::

::: {#exported-values-1}
::: {#appendix-A.1.2.2 .section}
##### [A.1.2.2.](#appendix-A.1.2.2){.section-number .selfRef} [Exported Values](#name-exported-values-2){.section-name .selfRef} {#name-exported-values-2}
...
#### [A.1.3.](#appendix-A.1.3){.section-number .selfRef} [Auth Setup Information](#name-auth-setup-information){.section-name .selfRef} {#name-auth-setup-information}
...
[¶](#appendix-A.1.3-1){.pilcrow}
:::

::: {#encryptions-2}
::: {#appendix-A.1.3.1 .section}
##### [A.1.3.1.](#appendix-A.1.3.1){.section-number .selfRef} [Encryptions](#name-encryptions-3){.section-name .selfRef} {#name-encryptions-3}
...
##### [A.1.3.2.](#appendix-A.1.3.2){.section-number .selfRef} [Exported Values](#name-exported-values-3){.section-name .selfRef} {#name-exported-values-3}
...
#### [A.1.4.](#appendix-A.1.4){.section-number .selfRef} [AuthPSK Setup Information](#name-authpsk-setup-information){.section-name .selfRef} {#name-authpsk-setup-information}
...
[¶](#appendix-A.1.4-1){.pilcrow}
:::

::: {#encryptions-3}
::: {#appendix-A.1.4.1 .section}
##### [A.1.4.1.](#appendix-A.1.4.1){.section-number .selfRef} [Encryptions](#name-encryptions-4){.section-name .selfRef} {#name-encryptions-4}
...
[¶](#appendix-A.1.4.1-1){.pilcrow}
:::
:::
:::

::: {#exported-values-3}
::: {#appendix-A.1.4.2 .section}
##### [A.1.4.2.](#appendix-A.1.4.2){.section-number .selfRef} [Exported Values](#name-exported-values-4){.section-name .selfRef} {#name-exported-values-4}
...
::: {#dhkemx25519-hkdf-sha256-hkdf-sha256-chacha20poly1305}
::: {#appendix-A.2 .section}
### [A.2.](#appendix-A.2){.section-number .selfRef} [DHKEM(X25519, HKDF-SHA256), HKDF-SHA256, ChaCha20Poly1305](#name-dhkemx25519-hkdf-sha256-hkdf){.section-name .selfRef} {#name-dhkemx25519-hkdf-sha256-hkdf}

::: {#base-setup-information-1}
::: {#appendix-A.2.1 .section}
#### [A.2.1.](#appendix-A.2.1){.section-number .selfRef} [Base Setup Information](#name-base-setup-information-2){.section-name .selfRef} {#name-base-setup-information-2}
...
##### [A.2.1.1.](#appendix-A.2.1.1){.section-number .selfRef} [Encryptions](#name-encryptions-5){.section-name .selfRef} {#name-encryptions-5}
...
##### [A.2.1.2.](#appendix-A.2.1.2){.section-number .selfRef} [Exported Values](#name-exported-values-5){.section-name .selfRef} {#name-exported-values-5}
...
::: {#psk-setup-information-1}
::: {#appendix-A.2.2 .section}
#### [A.2.2.](#appendix-A.2.2){.section-number .selfRef} [PSK Setup Information](#name-psk-setup-information-2){.section-name .selfRef} {#name-psk-setup-information-2}
...
::: {#encryptions-5}
::: {#appendix-A.2.2.1 .section}
##### [A.2.2.1.](#appendix-A.2.2.1){.section-number .selfRef} [Encryptions](#name-encryptions-6){.section-name .selfRef} {#name-encryptions-6}
...
[¶](#appendix-A.2.2.1-1){.pilcrow}
:::
:::
:::

::: {#exported-values-5}
::: {#appendix-A.2.2.2 .section}
##### [A.2.2.2.](#appendix-A.2.2.2){.section-number .selfRef} [Exported Values](#name-exported-values-6){.section-name .selfRef} {#name-exported-values-6}
...
::: {#auth-setup-information-1}
::: {#appendix-A.2.3 .section}
#### [A.2.3.](#appendix-A.2.3){.section-number .selfRef} [Auth Setup Information](#name-auth-setup-information-2){.section-name .selfRef} {#name-auth-setup-information-2}
...
[¶](#appendix-A.2.3-1){.pilcrow}
:::

::: {#encryptions-6}
::: {#appendix-A.2.3.1 .section}
##### [A.2.3.1.](#appendix-A.2.3.1){.section-number .selfRef} [Encryptions](#name-encryptions-7){.section-name .selfRef} {#name-encryptions-7}
...
##### [A.2.3.2.](#appendix-A.2.3.2){.section-number .selfRef} [Exported Values](#name-exported-values-7){.section-name .selfRef} {#name-exported-values-7}
...
::: {#authpsk-setup-information-1}
::: {#appendix-A.2.4 .section}
#### [A.2.4.](#appendix-A.2.4){.section-number .selfRef} [AuthPSK Setup Information](#name-authpsk-setup-information-2){.section-name .selfRef} {#name-authpsk-setup-information-2}
...
[¶](#appendix-A.2.4-1){.pilcrow}
:::

::: {#encryptions-7}
::: {#appendix-A.2.4.1 .section}
##### [A.2.4.1.](#appendix-A.2.4.1){.section-number .selfRef} [Encryptions](#name-encryptions-8){.section-name .selfRef} {#name-encryptions-8}
...
[¶](#appendix-A.2.4.1-1){.pilcrow}
:::
:::
:::

::: {#exported-values-7}
::: {#appendix-A.2.4.2 .section}
##### [A.2.4.2.](#appendix-A.2.4.2){.section-number .selfRef} [Exported Values](#name-exported-values-8){.section-name .selfRef} {#name-exported-values-8}
...
::: {#dhkemp-256-hkdf-sha256-hkdf-sha256-aes-128-gcm}
::: {#appendix-A.3 .section}
### [A.3.](#appendix-A.3){.section-number .selfRef} [DHKEM(P-256, HKDF-SHA256), HKDF-SHA256, AES-128-GCM](#name-dhkemp-256-hkdf-sha256-hkdf){.section-name .selfRef} {#name-dhkemp-256-hkdf-sha256-hkdf}

::: {#base-setup-information-2}
::: {#appendix-A.3.1 .section}
#### [A.3.1.](#appendix-A.3.1){.section-number .selfRef} [Base Setup Information](#name-base-setup-information-3){.section-name .selfRef} {#name-base-setup-information-3}
...
[¶](#appendix-A.3.1-1){.pilcrow}
:::

::: {#encryptions-8}
::: {#appendix-A.3.1.1 .section}
##### [A.3.1.1.](#appendix-A.3.1.1){.section-number .selfRef} [Encryptions](#name-encryptions-9){.section-name .selfRef} {#name-encryptions-9}
...
##### [A.3.1.2.](#appendix-A.3.1.2){.section-number .selfRef} [Exported Values](#name-exported-values-9){.section-name .selfRef} {#name-exported-values-9}
...
::: {#psk-setup-information-2}
::: {#appendix-A.3.2 .section}
#### [A.3.2.](#appendix-A.3.2){.section-number .selfRef} [PSK Setup Information](#name-psk-setup-information-3){.section-name .selfRef} {#name-psk-setup-information-3}
...
[¶](#appendix-A.3.2-1){.pilcrow}
:::

::: {#encryptions-9}
::: {#appendix-A.3.2.1 .section}
##### [A.3.2.1.](#appendix-A.3.2.1){.section-number .selfRef} [Encryptions](#name-encryptions-10){.section-name .selfRef} {#name-encryptions-10}
...
[¶](#appendix-A.3.2.1-1){.pilcrow}
:::
:::
:::

::: {#exported-values-9}
::: {#appendix-A.3.2.2 .section}
##### [A.3.2.2.](#appendix-A.3.2.2){.section-number .selfRef} [Exported Values](#name-exported-values-10){.section-name .selfRef} {#name-exported-values-10}
...
::: {#auth-setup-information-2}
::: {#appendix-A.3.3 .section}
#### [A.3.3.](#appendix-A.3.3){.section-number .selfRef} [Auth Setup Information](#name-auth-setup-information-3){.section-name .selfRef} {#name-auth-setup-information-3}
...
[¶](#appendix-A.3.3-1){.pilcrow}
:::

::: {#encryptions-10}
::: {#appendix-A.3.3.1 .section}
##### [A.3.3.1.](#appendix-A.3.3.1){.section-number .selfRef} [Encryptions](#name-encryptions-11){.section-name .selfRef} {#name-encryptions-11}
...
##### [A.3.3.2.](#appendix-A.3.3.2){.section-number .selfRef} [Exported Values](#name-exported-values-11){.section-name .selfRef} {#name-exported-values-11}
...
::: {#authpsk-setup-information-2}
::: {#appendix-A.3.4 .section}
#### [A.3.4.](#appendix-A.3.4){.section-number .selfRef} [AuthPSK Setup Information](#name-authpsk-setup-information-3){.section-name .selfRef} {#name-authpsk-setup-information-3}
...
[¶](#appendix-A.3.4-1){.pilcrow}
:::

::: {#encryptions-11}
::: {#appendix-A.3.4.1 .section}
##### [A.3.4.1.](#appendix-A.3.4.1){.section-number .selfRef} [Encryptions](#name-encryptions-12){.section-name .selfRef} {#name-encryptions-12}
...
##### [A.3.4.2.](#appendix-A.3.4.2){.section-number .selfRef} [Exported Values](#name-exported-values-12){.section-name .selfRef} {#name-exported-values-12}
...
::: {#dhkemp-256-hkdf-sha256-hkdf-sha512-aes-128-gcm}
::: {#appendix-A.4 .section}
### [A.4.](#appendix-A.4){.section-number .selfRef} [DHKEM(P-256, HKDF-SHA256), HKDF-SHA512, AES-128-GCM](#name-dhkemp-256-hkdf-sha256-hkdf-){.section-name .selfRef} {#name-dhkemp-256-hkdf-sha256-hkdf-}

::: {#base-setup-information-3}
::: {#appendix-A.4.1 .section}
#### [A.4.1.](#appendix-A.4.1){.section-number .selfRef} [Base Setup Information](#name-base-setup-information-4){.section-name .selfRef} {#name-base-setup-information-4}
...
[¶](#appendix-A.4.1-1){.pilcrow}
:::

::: {#encryptions-12}
::: {#appendix-A.4.1.1 .section}
##### [A.4.1.1.](#appendix-A.4.1.1){.section-number .selfRef} [Encryptions](#name-encryptions-13){.section-name .selfRef} {#name-encryptions-13}
...
##### [A.4.1.2.](#appendix-A.4.1.2){.section-number .selfRef} [Exported Values](#name-exported-values-13){.section-name .selfRef} {#name-exported-values-13}
...
::: {#psk-setup-information-3}
::: {#appendix-A.4.2 .section}
#### [A.4.2.](#appendix-A.4.2){.section-number .selfRef} [PSK Setup Information](#name-psk-setup-information-4){.section-name .selfRef} {#name-psk-setup-information-4}
...
[¶](#appendix-A.4.2-1){.pilcrow}
:::

::: {#encryptions-13}
::: {#appendix-A.4.2.1 .section}
##### [A.4.2.1.](#appendix-A.4.2.1){.section-number .selfRef} [Encryptions](#name-encryptions-14){.section-name .selfRef} {#name-encryptions-14}
...
[¶](#appendix-A.4.2.1-1){.pilcrow}
:::
:::
:::

::: {#exported-values-13}
::: {#appendix-A.4.2.2 .section}
##### [A.4.2.2.](#appendix-A.4.2.2){.section-number .selfRef} [Exported Values](#name-exported-values-14){.section-name .selfRef} {#name-exported-values-14}
...
::: {#auth-setup-information-3}
::: {#appendix-A.4.3 .section}
#### [A.4.3.](#appendix-A.4.3){.section-number .selfRef} [Auth Setup Information](#name-auth-setup-information-4){.section-name .selfRef} {#name-auth-setup-information-4}
...
[¶](#appendix-A.4.3-1){.pilcrow}
:::

::: {#encryptions-14}
::: {#appendix-A.4.3.1 .section}
##### [A.4.3.1.](#appendix-A.4.3.1){.section-number .selfRef} [Encryptions](#name-encryptions-15){.section-name .selfRef} {#name-encryptions-15}
...
##### [A.4.3.2.](#appendix-A.4.3.2){.section-number .selfRef} [Exported Values](#name-exported-values-15){.section-name .selfRef} {#name-exported-values-15}
...
::: {#authpsk-setup-information-3}
::: {#appendix-A.4.4 .section}
#### [A.4.4.](#appendix-A.4.4){.section-number .selfRef} [AuthPSK Setup Information](#name-authpsk-setup-information-4){.section-name .selfRef} {#name-authpsk-setup-information-4}
...
[¶](#appendix-A.4.4-1){.pilcrow}
:::

::: {#encryptions-15}
::: {#appendix-A.4.4.1 .section}
##### [A.4.4.1.](#appendix-A.4.4.1){.section-number .selfRef} [Encryptions](#name-encryptions-16){.section-name .selfRef} {#name-encryptions-16}
...
##### [A.4.4.2.](#appendix-A.4.4.2){.section-number .selfRef} [Exported Values](#name-exported-values-16){.section-name .selfRef} {#name-exported-values-16}
...
::: {#dhkemp-256-hkdf-sha256-hkdf-sha256-chacha20poly1305}
::: {#appendix-A.5 .section}
### [A.5.](#appendix-A.5){.section-number .selfRef} [DHKEM(P-256, HKDF-SHA256), HKDF-SHA256, ChaCha20Poly1305](#name-dhkemp-256-hkdf-sha256-hkdf-s){.section-name .selfRef} {#name-dhkemp-256-hkdf-sha256-hkdf-s}

::: {#base-setup-information-4}
::: {#appendix-A.5.1 .section}
#### [A.5.1.](#appendix-A.5.1){.section-number .selfRef} [Base Setup Information](#name-base-setup-information-5){.section-name .selfRef} {#name-base-setup-information-5}
...
[¶](#appendix-A.5.1-1){.pilcrow}
:::

::: {#encryptions-16}
::: {#appendix-A.5.1.1 .section}
##### [A.5.1.1.](#appendix-A.5.1.1){.section-number .selfRef} [Encryptions](#name-encryptions-17){.section-name .selfRef} {#name-encryptions-17}
...
[¶](#appendix-A.5.1.1-1){.pilcrow}
:::
:::
:::

::: {#exported-values-16}
::: {#appendix-A.5.1.2 .section}
##### [A.5.1.2.](#appendix-A.5.1.2){.section-number .selfRef} [Exported Values](#name-exported-values-17){.section-name .selfRef} {#name-exported-values-17}
...
::: {#psk-setup-information-4}
::: {#appendix-A.5.2 .section}
#### [A.5.2.](#appendix-A.5.2){.section-number .selfRef} [PSK Setup Information](#name-psk-setup-information-5){.section-name .selfRef} {#name-psk-setup-information-5}
...
[¶](#appendix-A.5.2-1){.pilcrow}
:::

::: {#encryptions-17}
::: {#appendix-A.5.2.1 .section}
##### [A.5.2.1.](#appendix-A.5.2.1){.section-number .selfRef} [Encryptions](#name-encryptions-18){.section-name .selfRef} {#name-encryptions-18}
...
[¶](#appendix-A.5.2.1-1){.pilcrow}
:::
:::
:::

::: {#exported-values-17}
::: {#appendix-A.5.2.2 .section}
##### [A.5.2.2.](#appendix-A.5.2.2){.section-number .selfRef} [Exported Values](#name-exported-values-18){.section-name .selfRef} {#name-exported-values-18}
...
::: {#auth-setup-information-4}
::: {#appendix-A.5.3 .section}
#### [A.5.3.](#appendix-A.5.3){.section-number .selfRef} [Auth Setup Information](#name-auth-setup-information-5){.section-name .selfRef} {#name-auth-setup-information-5}
...
[¶](#appendix-A.5.3-1){.pilcrow}
:::

::: {#encryptions-18}
::: {#appendix-A.5.3.1 .section}
##### [A.5.3.1.](#appendix-A.5.3.1){.section-number .selfRef} [Encryptions](#name-encryptions-19){.section-name .selfRef} {#name-encryptions-19}
...
[¶](#appendix-A.5.3.1-1){.pilcrow}
:::
:::
:::

::: {#exported-values-18}
::: {#appendix-A.5.3.2 .section}
##### [A.5.3.2.](#appendix-A.5.3.2){.section-number .selfRef} [Exported Values](#name-exported-values-19){.section-name .selfRef} {#name-exported-values-19}
...
::: {#authpsk-setup-information-4}
::: {#appendix-A.5.4 .section}
#### [A.5.4.](#appendix-A.5.4){.section-number .selfRef} [AuthPSK Setup Information](#name-authpsk-setup-information-5){.section-name .selfRef} {#name-authpsk-setup-information-5}
...
[¶](#appendix-A.5.4-1){.pilcrow}
:::

::: {#encryptions-19}
::: {#appendix-A.5.4.1 .section}
##### [A.5.4.1.](#appendix-A.5.4.1){.section-number .selfRef} [Encryptions](#name-encryptions-20){.section-name .selfRef} {#name-encryptions-20}
...
[¶](#appendix-A.5.4.1-1){.pilcrow}
:::
:::
:::

::: {#exported-values-19}
::: {#appendix-A.5.4.2 .section}
##### [A.5.4.2.](#appendix-A.5.4.2){.section-number .selfRef} [Exported Values](#name-exported-values-20){.section-name .selfRef} {#name-exported-values-20}
...
::: {#dhkemp-521-hkdf-sha512-hkdf-sha512-aes-256-gcm}
::: {#appendix-A.6 .section}
### [A.6.](#appendix-A.6){.section-number .selfRef} [DHKEM(P-521, HKDF-SHA512), HKDF-SHA512, AES-256-GCM](#name-dhkemp-521-hkdf-sha512-hkdf){.section-name .selfRef} {#name-dhkemp-521-hkdf-sha512-hkdf}

::: {#base-setup-information-5}
::: {#appendix-A.6.1 .section}
#### [A.6.1.](#appendix-A.6.1){.section-number .selfRef} [Base Setup Information](#name-base-setup-information-6){.section-name .selfRef} {#name-base-setup-information-6}
...
[¶](#appendix-A.6.1-1){.pilcrow}
:::

::: {#encryptions-20}
::: {#appendix-A.6.1.1 .section}
##### [A.6.1.1.](#appendix-A.6.1.1){.section-number .selfRef} [Encryptions](#name-encryptions-21){.section-name .selfRef} {#name-encryptions-21}
...
[¶](#appendix-A.6.1.1-1){.pilcrow}
:::
:::
:::

::: {#exported-values-20}
::: {#appendix-A.6.1.2 .section}
##### [A.6.1.2.](#appendix-A.6.1.2){.section-number .selfRef} [Exported Values](#name-exported-values-21){.section-name .selfRef} {#name-exported-values-21}
...
::: {#psk-setup-information-5}
::: {#appendix-A.6.2 .section}
#### [A.6.2.](#appendix-A.6.2){.section-number .selfRef} [PSK Setup Information](#name-psk-setup-information-6){.section-name .selfRef} {#name-psk-setup-information-6}
...
[¶](#appendix-A.6.2-1){.pilcrow}
:::

::: {#encryptions-21}
::: {#appendix-A.6.2.1 .section}
##### [A.6.2.1.](#appendix-A.6.2.1){.section-number .selfRef} [Encryptions](#name-encryptions-22){.section-name .selfRef} {#name-encryptions-22}
...
[¶](#appendix-A.6.2.1-1){.pilcrow}
:::
:::
:::

::: {#exported-values-21}
::: {#appendix-A.6.2.2 .section}
##### [A.6.2.2.](#appendix-A.6.2.2){.section-number .selfRef} [Exported Values](#name-exported-values-22){.section-name .selfRef} {#name-exported-values-22}
...
::: {#auth-setup-information-5}
::: {#appendix-A.6.3 .section}
#### [A.6.3.](#appendix-A.6.3){.section-number .selfRef} [Auth Setup Information](#name-auth-setup-information-6){.section-name .selfRef} {#name-auth-setup-information-6}
...
[¶](#appendix-A.6.3-1){.pilcrow}
:::

::: {#encryptions-22}
::: {#appendix-A.6.3.1 .section}
##### [A.6.3.1.](#appendix-A.6.3.1){.section-number .selfRef} [Encryptions](#name-encryptions-23){.section-name .selfRef} {#name-encryptions-23}
...
[¶](#appendix-A.6.3.1-1){.pilcrow}
:::
:::
:::

::: {#exported-values-22}
::: {#appendix-A.6.3.2 .section}
##### [A.6.3.2.](#appendix-A.6.3.2){.section-number .selfRef} [Exported Values](#name-exported-values-23){.section-name .selfRef} {#name-exported-values-23}
...
::: {#authpsk-setup-information-5}
::: {#appendix-A.6.4 .section}
#### [A.6.4.](#appendix-A.6.4){.section-number .selfRef} [AuthPSK Setup Information](#name-authpsk-setup-information-6){.section-name .selfRef} {#name-authpsk-setup-information-6}
...
[¶](#appendix-A.6.4-1){.pilcrow}
:::

::: {#encryptions-23}
::: {#appendix-A.6.4.1 .section}
##### [A.6.4.1.](#appendix-A.6.4.1){.section-number .selfRef} [Encryptions](#name-encryptions-24){.section-name .selfRef} {#name-encryptions-24}
...
[¶](#appendix-A.6.4.1-1){.pilcrow}
:::
:::
:::

::: {#exported-values-23}
::: {#appendix-A.6.4.2 .section}
##### [A.6.4.2.](#appendix-A.6.4.2){.section-number .selfRef} [Exported Values](#name-exported-values-24){.section-name .selfRef} {#name-exported-values-24}
...
::: {#dhkemx25519-hkdf-sha256-hkdf-sha256-export-only-aead}
::: {#appendix-A.7 .section}
### [A.7.](#appendix-A.7){.section-number .selfRef} [DHKEM(X25519, HKDF-SHA256), HKDF-SHA256, Export-Only AEAD](#name-dhkemx25519-hkdf-sha256-hkdf-){.section-name .selfRef} {#name-dhkemx25519-hkdf-sha256-hkdf-}

::: {#base-setup-information-6}
::: {#appendix-A.7.1 .section}
#### [A.7.1.](#appendix-A.7.1){.section-number .selfRef} [Base Setup Information](#name-base-setup-information-7){.section-name .selfRef} {#name-base-setup-information-7}
...
[¶](#appendix-A.7.1-1){.pilcrow}
:::

::: {#exported-values-24}
::: {#appendix-A.7.1.1 .section}
##### [A.7.1.1.](#appendix-A.7.1.1){.section-number .selfRef} [Exported Values](#name-exported-values-25){.section-name .selfRef} {#name-exported-values-25}
...
::: {#psk-setup-information-6}
::: {#appendix-A.7.2 .section}
#### [A.7.2.](#appendix-A.7.2){.section-number .selfRef} [PSK Setup Information](#name-psk-setup-information-7){.section-name .selfRef} {#name-psk-setup-information-7}
...
[¶](#appendix-A.7.2-1){.pilcrow}
:::

::: {#exported-values-25}
::: {#appendix-A.7.2.1 .section}
##### [A.7.2.1.](#appendix-A.7.2.1){.section-number .selfRef} [Exported Values](#name-exported-values-26){.section-name .selfRef} {#name-exported-values-26}
...
::: {#auth-setup-information-6}
::: {#appendix-A.7.3 .section}
#### [A.7.3.](#appendix-A.7.3){.section-number .selfRef} [Auth Setup Information](#name-auth-setup-information-7){.section-name .selfRef} {#name-auth-setup-information-7}
...
[¶](#appendix-A.7.3-1){.pilcrow}
:::

::: {#exported-values-26}
::: {#appendix-A.7.3.1 .section}
##### [A.7.3.1.](#appendix-A.7.3.1){.section-number .selfRef} [Exported Values](#name-exported-values-27){.section-name .selfRef} {#name-exported-values-27}
...
::: {#authpsk-setup-information-6}
::: {#appendix-A.7.4 .section}
#### [A.7.4.](#appendix-A.7.4){.section-number .selfRef} [AuthPSK Setup Information](#name-authpsk-setup-information-7){.section-name .selfRef} {#name-authpsk-setup-information-7}
...
[¶](#appendix-A.7.4-1){.pilcrow}
:::

::: {#exported-values-27}
::: {#appendix-A.7.4.1 .section}
##### [A.7.4.1.](#appendix-A.7.4.1){.section-number .selfRef} [Exported Values](#name-exported-values-28){.section-name .selfRef} {#name-exported-values-28}
...
The authors would like to thank [Joel Alwen]{.contact-name},
[Jean-Philippe Aumasson]{.contact-name}, [David
Benjamin]{.contact-name}, [Benjamin Beurdouche]{.contact-name}, [Bruno
Blanchet]{.contact-name}, [Frank Denis]{.contact-name}, [Stephen
Farrell]{.contact-name}, [Scott Fluhrer]{.contact-name}, [Eduard
Hauck]{.contact-name}, [Scott Hollenbeck]{.contact-name}, [Kevin
Jacobs]{.contact-name}, [Burt Kaliski]{.contact-name}, [Eike
Kiltz]{.contact-name}, [Julia Len]{.contact-name}, [John
Mattsson]{.contact-name}, [Christopher Patton]{.contact-name}, [Doreen
Riepel]{.contact-name}, [Raphael Robert]{.contact-name}, [Michael
Rosenberg]{.contact-name}, [Michael Scott]{.contact-name}, [Martin
Thomson]{.contact-name}, [Steven Valdez]{.contact-name}, [Riad
Wahby]{.contact-name}, and other contributors in the CFRG for helpful
feedback that greatly improved this
