...
Where bit arithmetic or data types are explained, this document uses the
...
If any item between the indefinite-length string indicator (0b010_11111
...
::: {#major-type-indef-table}
  ...
  : [Table 2](#table-2){.selfRef}: [Overview of the Indefinite-Length
...
::: {#fpnoconttbl2}
  ...
  : [Table 4](#table-4){.selfRef}: [Simple
...
The tag definition usually defines which nested data items are valid for
...
::: {#tagvalues}
  ...
  36      text string        MIME message; see [Section 3.4.5.3](#encodedtext){.xref}
...
Implementation note: the self-delimiting nature of the CBOR encoding
...
Protocols that need to be compatible with the order specified in
\[[RFC7049](#RFC7049){.xref}\] can instead be specified in terms of this
specification\'s \"length-first core deterministic encoding
requirements\":[¶](#section-4.2.3-1){.pilcrow}

A CBOR encoding satisfies the \"length-first core deterministic encoding
requirements\" if it satisfies the core deterministic encoding
requirements except that the keys in every map [MUST]{.bcp14} be sorted
such that:[¶](#section-4.2.3-2){.pilcrow}

1.  [If two keys have different lengths, the shorter one sorts
    earlier;[¶](#section-4.2.3-3.1){.pilcrow}]{#section-4.2.3-3.1}
2.  [If two keys have the same length, the one with the lower value in
    (bytewise) lexical order sorts
    earlier.[¶](#section-4.2.3-3.2){.pilcrow}]{#section-4.2.3-3.2}

For example, under the length-first core deterministic encoding
requirements, the following keys are sorted
correctly:[¶](#section-4.2.3-4){.pilcrow}

1.  [10, encoded as
    0x0a.[¶](#section-4.2.3-5.1){.pilcrow}]{#section-4.2.3-5.1}
2.  [-1, encoded as
    0x20.[¶](#section-4.2.3-5.2){.pilcrow}]{#section-4.2.3-5.2}
3.  [false, encoded as
    0xf4.[¶](#section-4.2.3-5.3){.pilcrow}]{#section-4.2.3-5.3}
4.  [100, encoded as
    0x1864.[¶](#section-4.2.3-5.4){.pilcrow}]{#section-4.2.3-5.4}
5.  [\"z\", encoded as
    0x617a.[¶](#section-4.2.3-5.5){.pilcrow}]{#section-4.2.3-5.5}
6.  [\[-1\], encoded as
    0x8120.[¶](#section-4.2.3-5.6){.pilcrow}]{#section-4.2.3-5.6}
7.  [\"aa\", encoded as
    0x626161.[¶](#section-4.2.3-5.7){.pilcrow}]{#section-4.2.3-5.7}
8.  [\[100\], encoded as
    0x811864.[¶](#section-4.2.3-5.8){.pilcrow}]{#section-4.2.3-5.8}

Although \[[RFC7049](#RFC7049){.xref}\] used the term \"Canonical CBOR\"
for its form of requirements on deterministic encoding, this document
avoids this term because \"canonicalization\" is often associated with
specific uses of deterministic encoding only. The terms are essentially
interchangeable, however, and the set of core requirements in this
document could also be called \"Canonical CBOR\", while the
length-first-ordered version of that could be called \"Old Canonical
CBOR\".[¶](#section-4.2.3-6.1){.pilcrow}
:::
:::
:::
:::
...
::: {#section-5 .section}
## [5.](#section-5){.section-number .selfRef} [Creating CBOR-Based Protocols](#name-creating-cbor-based-protoco){.section-name .selfRef} {#name-creating-cbor-based-protoco}

Data formats such as CBOR are often used in environments where there is
no format negotiation. A specific design goal of CBOR is to not need any
included or assumed schema: a decoder can take a CBOR item and decode it
with no other knowledge.[¶](#section-5-1){.pilcrow}
...
example, an agreed-to format might be \"the item is an array whose first
value is a UTF-8 string, second value is an integer, and subsequent
values are zero or more floating-point numbers\" or \"the item is a map
that has byte strings for keys and contains a pair whose key is
0xab01\".[¶](#section-5-2){.pilcrow}

CBOR-based protocols [MUST]{.bcp14} specify how their decoders handle
invalid and other unexpected data. CBOR-based protocols [MAY]{.bcp14}
specify that they treat arbitrary valid data as unexpected. Encoders for
CBOR-based protocols [MUST]{.bcp14} produce only valid items, that is,
the protocol cannot be designed to make use of invalid items. An encoder
can be capable of encoding as many or as few types of values as is
required by the protocol in which it is used; a decoder can be capable
of understanding as many or as few types of values as is required by the
protocols in which it is used. This lack of restrictions allows CBOR to
be used in extremely constrained
environments.[¶](#section-5-3){.pilcrow}
...
explicitly excludes any language from BCP 14
\[[RFC2119](#RFC2119){.xref}\] \[[RFC8174](#RFC8174){.xref}\] other than
words that could be interpreted as \"[MAY]{.bcp14}\" in the sense of BCP
14. The exceptions aim at facilitating interoperability of CBOR-based
protocols while making use of a wide variety of both generic and
application-specific encoders and decoders.[¶](#section-5-4){.pilcrow}

::: {#cbor-in-streaming-applications}
::: {#section-5.1 .section}
### [5.1.](#section-5.1){.section-number .selfRef} [CBOR in Streaming Applications](#name-cbor-in-streaming-applicati){.section-name .selfRef} {#name-cbor-in-streaming-applicati}

In a streaming application, a data stream may be composed of a sequence
of CBOR data items concatenated back-to-back. In such an environment,
the decoder immediately begins decoding a new data item if data is found
after the end of a previous data item.[¶](#section-5.1-1){.pilcrow}

Not all of the bytes making up a data item may be immediately available
to the decoder; some decoders will buffer additional data until a
complete data item can be presented to the application. Other decoders
can present partial information about a top-level data item to an
application, such as the nested data items that could already be
decoded, or even parts of a byte string that hasn\'t completely arrived
yet. Such an application also [MUST]{.bcp14} have a matching streaming
security mechanism, where the desired protection is available for
incremental data presented to the
application.[¶](#section-5.1-2){.pilcrow}
...
encoder to not need to marshal all the data for counting, but it
...
applications but not others.[¶](#section-5.1-3){.pilcrow}
:::
:::

::: {#generic}
::: {#section-5.2 .section}
### [5.2.](#section-5.2){.section-number .selfRef} [Generic Encoders and Decoders](#name-generic-encoders-and-decode){.section-name .selfRef} {#name-generic-encoders-and-decode}

A generic CBOR decoder can decode all well-formed encoded CBOR data
items and present the data items to an application. See [Appendix
C](#pseudocode){.xref}. (The diagnostic notation, [Section
8](#diagnostic-notation){.xref}, may be used to present well-formed CBOR
values to humans.)[¶](#section-5.2-1){.pilcrow}
...
data item, including simple values and tags unknown to the
encoder.[¶](#section-5.2-2){.pilcrow}

Even though CBOR attempts to minimize these cases, not all well-formed
CBOR data is valid: for example, the encoded text string `0x62c0ae` does
not contain valid UTF-8 (because \[[RFC3629](#RFC3629){.xref}\] requires
always using the shortest form) and so is not a valid CBOR item. Also,
specific tags may make semantic constraints that may be violated, for
instance, by a bignum tag enclosing another tag or by an instance of tag
number 0 containing a byte string or containing a text string with
contents that do not match the `date-time` production of
\[[RFC3339](#RFC3339){.xref}\]. There is no requirement that generic
encoders and decoders make unnatural choices for their application
interface to enable the processing of invalid data. Generic encoders and
decoders are expected to forward simple values and tags even if their
specific codepoints are not registered at the time the encoder/decoder
is written ([Section
5.4](#validity-checking){.xref}).[¶](#section-5.2-3){.pilcrow}
:::
:::

::: {#semantic-errors}
::: {#section-5.3 .section}
### [5.3.](#section-5.3){.section-number .selfRef} [Validity of Items](#name-validity-of-items){.section-name .selfRef} {#name-validity-of-items}

A well-formed but invalid CBOR data item ([Section
1.2](#terminology){.xref}) presents a problem with interpreting the data
encoded in it in the CBOR data model. A CBOR-based protocol could be
specified in several layers, in which the lower layers don\'t process
the semantics of some of the CBOR data they forward. These layers can\'t
notice any validity errors in data they don\'t process and
[MUST]{.bcp14} forward that data as-is. The first layer that does
process the semantics of an invalid CBOR item [MUST]{.bcp14} pick one of
two choices:[¶](#section-5.3-1){.pilcrow}

1.  [Replace the problematic item with an error marker and continue with
    the next item, or[¶](#section-5.3-2.1){.pilcrow}]{#section-5.3-2.1}
2.  [Issue an error and stop processing
    altogether.[¶](#section-5.3-2.2){.pilcrow}]{#section-5.3-2.2}

A CBOR-based protocol [MUST]{.bcp14} specify which of these options its
decoders take for each kind of invalid item they might
encounter.[¶](#section-5.3-3){.pilcrow}

Such problems might occur at the basic validity level of CBOR or in the
context of tags (tag validity).[¶](#section-5.3-4){.pilcrow}

::: {#basic-validity}
::: {#section-5.3.1 .section}
#### [5.3.1.](#section-5.3.1){.section-number .selfRef} [Basic validity](#name-basic-validity){.section-name .selfRef} {#name-basic-validity}

Two kinds of validity errors can occur in the basic generic data
model:[¶](#section-5.3.1-1){.pilcrow}

[]{.break}

Duplicate keys in a map:
:   Generic decoders ([Section 5.2](#generic){.xref}) make data
    available to applications using the native CBOR data model. That
    data model includes maps (key-value mappings with unique keys), not
    multimaps (key-value mappings where multiple entries can have the
    same key). Thus, a generic decoder that gets a CBOR map item that
    has duplicate keys will decode to a map with only one instance of
    that key, or it might stop processing altogether. On the other hand,
    a \"streaming decoder\" may not even be able to notice. See [Section
    5.6](#map-keys){.xref} for more discussion of keys in
    maps.[¶](#section-5.3.1-2.2){.pilcrow}
:   
...
:   A decoder might or might not want to verify that the sequence of
    ...
    react appropriately.[¶](#section-5.3.1-2.4){.pilcrow}
:   
:::
:::
...
#### [5.3.2.](#section-5.3.2){.section-number .selfRef} [Tag validity](#name-tag-validity){.section-name .selfRef} {#name-tag-validity}

Two additional kinds of validity errors are introduced by adding tags to
the basic generic data model:[¶](#section-5.3.2-1){.pilcrow}

[]{.break}

Inadmissible type for tag content:
:   Tag numbers ([Section 3.4](#tags){.xref}) specify what type of data
    item is supposed to be used as their tag content; for example, the
    tag numbers for unsigned or negative bignums are supposed to be put
    on byte strings. A decoder that decodes the tagged data item into a
    native representation (a native big integer in this example) is
    expected to check the type of the data item being tagged. Even
    decoders that don\'t have such native representations available in
    their environment may perform the check on those tags known to them
    and react appropriately.[¶](#section-5.3.2-2.2){.pilcrow}
:   
...
:   The type of data item may be admissible for a tag\'s content, but
    ...
    number ([Section
    5.4](#validity-checking){.xref}).[¶](#section-5.3.2-2.4){.pilcrow}
:   
:::
:::
:::
...
::: {#validity-checking}
::: {#section-5.4 .section}
### [5.4.](#section-5.4){.section-number .selfRef} [Validity and Evolution](#name-validity-and-evolution){.section-name .selfRef} {#name-validity-and-evolution}
...
for a CBOR data item that contains any of the validity errors listed in
the previous subsection.[¶](#section-5.4-1){.pilcrow}

The set of tags defined in the \"Concise Binary Object Representation
(CBOR) Tags\" registry ([Section 9.2](#ianatags){.xref}), as well as the
set of simple values defined in the \"Concise Binary Object
Representation (CBOR) Simple Values\" registry ([Section
9.1](#ianasimple){.xref}), can grow at any time beyond the set
understood by a generic decoder. A validity-checking decoder can do one
of two things when it encounters such a case that it does not
recognize:[¶](#section-5.4-2){.pilcrow}
...
-   [It can report an error (and not return data). Note that treating
    ...
    encouraged. This error is not a validity error, per se. This kind of
    error is more likely to be raised by a decoder that would be
    performing validity checking if this were a known
    case.[¶](#section-5.4-3.1){.pilcrow}]{#section-5.4-3.1}
-   [It can emit the unknown item (type, value, and, for tags, the
    decoded tagged data item) to the application calling the decoder,
    and then give the application an indication that the decoder did not
    recognize that tag number or simple
    value.[¶](#section-5.4-3.2){.pilcrow}]{#section-5.4-3.2}

The latter approach, which is also appropriate for decoders that do not
support validity checking, provides forward compatibility with newly
registered tags and simple values without the requirement to update the
encoder at the same time as the calling application. (For this, the
decoder\'s API needs the ability to mark unknown items so that the
calling application can handle them in a manner appropriate for the
program.)[¶](#section-5.4-4){.pilcrow}
...
decoders.[¶](#section-5.4-5){.pilcrow}

Some encoders will rely on their applications to provide input data in
such a way that valid CBOR results from the encoder. A generic encoder
may also want to provide a validity-checking mode where it reliably
limits its output to valid CBOR, independent of whether or not its
application is indeed providing API-conformant
data.[¶](#section-5.4-6){.pilcrow}
:::
:::

::: {#numbers}
::: {#section-5.5 .section}
### [5.5.](#section-5.5){.section-number .selfRef} [Numbers](#name-numbers){.section-name .selfRef} {#name-numbers}
...
significant bits. Another example is that, since CBOR keeps the sign bit
for its integer representation in the major type, it has one bit more
for signed numbers of a certain length (e.g., -2^64^..2^64^-1 for
1+8-byte integers) than the typical platform signed integer
representation of the same length (-2^63^..2^63^-1 for 8-byte int64_t).
A protocol that uses numbers should define its expectations on the
handling of nontrivial numbers in decoders and receiving
applications.[¶](#section-5.5-1){.pilcrow}

A CBOR-based protocol that includes floating-point numbers can restrict
which of the three formats (half-precision, single-precision, and
double-precision) are to be supported. For an integer-only application,
a protocol may want to completely exclude the use of floating-point
values.[¶](#section-5.5-2){.pilcrow}
...
There is an expectation that encoders will use the most compact integer
representation that can represent a given value. However, a compact
application that does not require deterministic encoding should accept
values that use a longer-than-needed encoding (such as encoding \"0\" as
0b000_11001 followed by two bytes of 0x00) as long as the application
can decode an integer of the given size. Similar considerations apply to
floating-point values; decoding both preferred serializations and
longer-than-needed ones is recommended.[¶](#section-5.5-3){.pilcrow}

CBOR-based protocols for constrained applications that provide a choice
between representing a specific number as an integer and as a decimal
fraction or bigfloat (such as when the exponent is small and
nonnegative) might express a quality-of-implementation expectation that
the integer representation is used
directly.[¶](#section-5.5-4){.pilcrow}
:::
:::

::: {#map-keys}
::: {#section-5.6 .section}
### [5.6.](#section-5.6){.section-number .selfRef} [Specifying Keys for Maps](#name-specifying-keys-for-maps){.section-name .selfRef} {#name-specifying-keys-for-maps}
...
application, directly using the numbers for the keys is
useful.[¶](#section-5.6-1){.pilcrow}
...
environments that are to be used. For example, in JavaScript Maps
\[[ECMA262](#ECMA262){.xref}\], a key of integer 1 cannot be
distinguished from a key of floating-point 1.0. This means that, if
integer keys are used, the protocol needs to avoid the use of
floating-point keys the values of which happen to be integer numbers in
the same map.[¶](#section-5.6-2){.pilcrow}
...
want to reduce its overhead significantly by relying on its data source
to maintain uniqueness.[¶](#section-5.6-3){.pilcrow}

A CBOR-based protocol [MUST]{.bcp14} define what to do when a receiving
application sees multiple identical keys in a map. The resulting rule in
the protocol [MUST]{.bcp14} respect the CBOR data model: it cannot
...
error. When processing maps that exhibit entries with duplicate keys, a
generic decoder might do one of the
following:[¶](#section-5.6-4){.pilcrow}

-   [Not accept maps with duplicate keys (that is, enforce validity for
    maps, see also [Section 5.4](#validity-checking){.xref}). These
    generic decoders are universally useful. An application may still
    need to perform its own duplicate checking based on application
    rules (for instance, if the application equates integers and
    floating-point values in map key positions for specific
    maps).[¶](#section-5.6-5.1){.pilcrow}]{#section-5.6-5.1}
-   [Pass all map entries to the application, including ones with
    ...
    to the generic data model
    rules.[¶](#section-5.6-5.2){.pilcrow}]{#section-5.6-5.2}
-   [Lose some entries with duplicate keys, e.g., deliver only the final
    (or first) entry out of the entries with the same key. With such a
    generic decoder, applications may get different results for a
    specific key on different runs, and with different generic decoders,
    which value is returned is based on generic decoder implementation
    and the actual order of keys in the map. In particular, applications
    cannot validate key uniqueness on their own as they do not
    necessarily see all entries; they may not be able to use such a
    generic decoder if they need to validate key uniqueness. These
    generic decoders can only be used in situations where the data
    source and transfer always provide valid maps; this is not possible
    if the data source and transfer can be
    attacked.[¶](#section-5.6-5.3){.pilcrow}]{#section-5.6-5.3}

Generic decoders need to document which of these three approaches they
implement.[¶](#section-5.6-6){.pilcrow}
...
CBOR-based protocol [MUST NOT]{.bcp14} specify that changing the
key/value pair order in a map changes the semantics, except to specify
that some orders are disallowed, for example, where they would not meet
the requirements of a deterministic encoding ([Section
4.2](#det-enc){.xref}). (Any secondary effects of map ordering such as
on timing, cache usage, and other potential side channels are not
considered part of the semantics but may be enough reason on their own
for a protocol to require a deterministic encoding
format.)[¶](#section-5.6-7){.pilcrow}
...
used keys; for instance, a set of 24 or fewer keys can be encoded in a
single byte as unsigned integers, up to 48 if negative integers are also
used. Less frequently occurring keys can then use integers with longer
encodings.[¶](#section-5.6-8){.pilcrow}

::: {#equivalence-of-keys}
::: {#section-5.6.1 .section}
#### [5.6.1.](#section-5.6.1){.section-number .selfRef} [Equivalence of Keys](#name-equivalence-of-keys){.section-name .selfRef} {#name-equivalence-of-keys}

The specific data model that applies to a CBOR data item is used to
determine whether keys occurring in maps are duplicates or
distinct.[¶](#section-5.6.1-1){.pilcrow}

At the generic data model level, numerically equivalent integer and
floating-point values are distinct from each other, as they are from the
various big numbers (Tags 2 to 5). Similarly, text strings are distinct
from byte strings, even if composed of the same bytes. A tagged value is
distinct from an untagged value or from a value tagged with a different
tag number.[¶](#section-5.6.1-2){.pilcrow}

Within each of these groups, numeric values are distinct unless they are
numerically equal (specifically, -0.0 is equal to 0.0); for the purpose
of map key equivalence, NaN values are equivalent if they have the same
significand after zero-extending both significands at the right to 64
bits.[¶](#section-5.6.1-3){.pilcrow}

Both byte strings and text strings are compared byte by byte, arrays are
compared element by element, and are equal if they have the same number
of bytes/elements and the same values at the same positions. Two maps
are equal if they have the same set of pairs regardless of their order;
pairs are equal if both the key and value are
equal.[¶](#section-5.6.1-4){.pilcrow}

Tagged values are equal if both the tag number and the tag content are
equal. (Note that a generic decoder that provides processing for a
specific tag may not be able to distinguish some semantically equivalent
values, e.g., if leading zeroes occur in the content of tag 2 or tag 3
([Section 3.4.3](#bignums){.xref}).) Simple values are equal if they
simply have the same value. Nothing else is equal in the generic data
model; a simple value 2 is not equivalent to an integer 2, and an array
is never equivalent to a map.[¶](#section-5.6.1-5){.pilcrow}

As discussed in [Section 2.2](#specific-data-models){.xref}, specific
data models can make values equivalent for the purpose of comparing map
keys that are distinct in the generic data model. Note that this implies
that a generic decoder may deliver a decoded map to an application that
needs to be checked for duplicate map keys by that application
(alternatively, the decoder may provide a programming interface to
perform this service for the application). Specific data models are not
able to distinguish values for map keys that are equal for this purpose
at the generic data model level.[¶](#section-5.6.1-6){.pilcrow}
:::
:::
:::
:::
...
::: {#undefined-values}
::: {#section-5.7 .section}
### [5.7.](#section-5.7){.section-number .selfRef} [Undefined Values](#name-undefined-values){.section-name .selfRef} {#name-undefined-values}

In some CBOR-based protocols, the simple value ([Section
3.3](#fpnocont){.xref}) of `undefined` might be used by an encoder as a
substitute for a data item with an encoding problem, in order to allow
the rest of the enclosing data items to be encoded without
harm.[¶](#section-5.7-1){.pilcrow}
:::
:::
:::
:::
...
::: {#section-6 .section}
## [6.](#section-6){.section-number .selfRef} [Converting Data between CBOR and JSON](#name-converting-data-between-cbo){.section-name .selfRef} {#name-converting-data-between-cbo}

This section gives non-normative advice about converting between CBOR
and JSON. Implementations of converters [MAY]{.bcp14} use whichever
advice here they want.[¶](#section-6-1){.pilcrow}

It is worth noting that a JSON text is a sequence of characters, not an
encoded sequence of bytes, while a CBOR data item consists of bytes, not
characters.[¶](#section-6-2){.pilcrow}

::: {#converting-from-cbor-to-json}
::: {#section-6.1 .section}
### [6.1.](#section-6.1){.section-number .selfRef} [Converting from CBOR to JSON](#name-converting-from-cbor-to-jso){.section-name .selfRef} {#name-converting-from-cbor-to-jso}

Most of the types in CBOR have direct analogs in JSON. However, some do
not, and someone implementing a CBOR-to-JSON converter has to consider
what to do in those cases. The following non-normative advice deals with
these by converting them to a single substitute value, such as a JSON
null.[¶](#section-6.1-1){.pilcrow}

-   [An integer (major type 0 or 1) becomes a JSON
    number.[¶](#section-6.1-2.1){.pilcrow}]{#section-6.1-2.1}
-   [A byte string (major type 2) that is not embedded in a tag that
    ...
    padding and becomes a JSON
    string.[¶](#section-6.1-2.2){.pilcrow}]{#section-6.1-2.2}
-   [A UTF-8 string (major type 3) becomes a JSON string. Note that JSON
    requires escaping certain characters
    (\[[RFC8259](#RFC8259){.xref}\], [Section
    7](https://www.rfc-editor.org/rfc/rfc8259#section-7){.relref}):
    quotation mark (U+0022), reverse solidus (U+005C), and the \"C0
    control characters\" (U+0000 through U+001F). All other characters
    are copied unchanged into the JSON UTF-8
    string.[¶](#section-6.1-2.3){.pilcrow}]{#section-6.1-2.3}
-   [An array (major type 4) becomes a JSON
    array.[¶](#section-6.1-2.4){.pilcrow}]{#section-6.1-2.4}
-   [A map (major type 5) becomes a JSON object. This is possible
    ...
    will cause a key collision if the tags are different but the strings
    are the same.[¶](#section-6.1-2.5){.pilcrow}]{#section-6.1-2.5}
-   [False (major type 7, additional information 20) becomes a JSON
    false.[¶](#section-6.1-2.6){.pilcrow}]{#section-6.1-2.6}
-   [True (major type 7, additional information 21) becomes a JSON
    true.[¶](#section-6.1-2.7){.pilcrow}]{#section-6.1-2.7}
-   [Null (major type 7, additional information 22) becomes a JSON
    null.[¶](#section-6.1-2.8){.pilcrow}]{#section-6.1-2.8}
-   [A floating-point value (major type 7, additional information 25
    ...
    substitute value.[¶](#section-6.1-2.9){.pilcrow}]{#section-6.1-2.9}
-   [Any other simple value (major type 7, any additional information
    value not yet discussed) is represented by the substitute
    value.[¶](#section-6.1-2.10){.pilcrow}]{#section-6.1-2.10}
-   [A bignum (major type 6, tag number 2 or 3) is represented by
    encoding its byte string in base64url without padding and becomes a
    JSON string. For tag number 3 (negative bignum), a \"\~\" (ASCII
    tilde) is inserted before the base-encoded value. (The conversion to
    a binary blob instead of a number is to prevent a likely numeric
    overflow for the JSON
    decoder.)[¶](#section-6.1-2.11){.pilcrow}]{#section-6.1-2.11}
-   [A byte string with an encoding hint (major type 6, tag number 21
    ...
    string.[¶](#section-6.1-2.12){.pilcrow}]{#section-6.1-2.12}
-   [For all other tags (major type 6, any other tag number), the tag
    content is represented as a JSON value; the tag number is
    ignored.[¶](#section-6.1-2.13){.pilcrow}]{#section-6.1-2.13}
-   [Indefinite-length items are made definite before
    conversion.[¶](#section-6.1-2.14){.pilcrow}]{#section-6.1-2.14}

A CBOR-to-JSON converter may want to keep to the JSON profile I-JSON
\[[RFC7493](#RFC7493){.xref}\], to maximize interoperability and
increase confidence that the JSON output can be processed with
predictable results. For example, this has implications on the range of
integers that can be represented reliably, as well as on the top-level
items that may be supported by older JSON
implementations.[¶](#section-6.1-3){.pilcrow}
:::
:::

::: {#converting-from-json}
::: {#section-6.2 .section}
### [6.2.](#section-6.2){.section-number .selfRef} [Converting from JSON to CBOR](#name-converting-from-json-to-cbo){.section-name .selfRef} {#name-converting-from-json-to-cbo}

All JSON values, once decoded, directly map into one or more CBOR
values. As with any kind of CBOR generation, decisions have to be made
with respect to number representation. In a suggested
conversion:[¶](#section-6.2-1){.pilcrow}
...
-   [JSON numbers without fractional parts (integer numbers) are
    ...
    tag number 2 and 3), choosing the shortest form; integers longer
    than an implementation-defined threshold may instead be represented
    as floating-point values. The default range that is represented as
    integer is -2^53^+1..2^53^-1 (fully exploiting the range for exact
    integers in the binary64 representation often used for decoding JSON
    \[[RFC7493](#RFC7493){.xref}\]). A CBOR-based protocol, or a generic
    ...
    maximum.)[¶](#section-6.2-2.1){.pilcrow}]{#section-6.2-2.1}
-   [Numbers with fractional parts are represented as floating-point
    ...
    precision provided by IEEE 754 binary64. The mathematical value of
    the JSON number is converted to binary64 using the roundTiesToEven
    procedure in Section 4.3.1 of \[[IEEE754](#IEEE754){.xref}\]. Then,
    when encoding in CBOR, the preferred serialization uses the shortest
    floating-point representation exactly representing this conversion
    result; for instance, 1.5 is represented in a 16-bit floating-point
    value (not all implementations will be capable of efficiently
    finding the minimum form, though). Instead of using the default
    binary64 precision, there may be an implementation-defined limit to
    the precision of the conversion that will affect the precision of
    the represented values. Decimal representation should only be used
    on the CBOR side if that is specified in a
    protocol.[¶](#section-6.2-2.2){.pilcrow}]{#section-6.2-2.2}

CBOR has been designed to generally provide a more compact encoding than
JSON. One implementation strategy that might come to mind is to perform
a JSON-to-CBOR encoding in place in a single buffer. This strategy would
need to carefully consider a number of pathological cases, such as that
some strings represented with no or very few escapes and longer (or much
...
JSON. This may be hard to get right, and any ensuing vulnerabilities may
be exploited by an attacker.[¶](#section-6.2-3){.pilcrow}
:::
:::
:::
:::
...
::: {#section-7 .section}
## [7.](#section-7){.section-number .selfRef} [Future Evolution of CBOR](#name-future-evolution-of-cbor){.section-name .selfRef} {#name-future-evolution-of-cbor}

Successful protocols evolve over time. New ideas appear, implementation
platforms improve, related protocols are developed and evolve, and new
requirements from applications and protocols are added. Facilitating
protocol evolution is therefore an important design consideration for
any protocol development.[¶](#section-7-1){.pilcrow}

For protocols that will use CBOR, CBOR provides some useful mechanisms
to facilitate their evolution. Best practices for this are well known,
particularly from JSON format development of JSON-based protocols.
Therefore, such best practices are outside the scope of this
specification.[¶](#section-7-2){.pilcrow}
...
successful protocol may live for decades, CBOR needs to be designed for
decades of use and evolution. This section provides some guidance for
the evolution of CBOR. It is necessarily more subjective than other
parts of this document. It is also necessarily incomplete, lest it turn
into a textbook on protocol development.[¶](#section-7-3){.pilcrow}

::: {#extension-points}
::: {#section-7.1 .section}
### [7.1.](#section-7.1){.section-number .selfRef} [Extension Points](#name-extension-points){.section-name .selfRef} {#name-extension-points}

In a protocol design, opportunities for evolution are often included in
the form of extension points. For example, there may be a codepoint
space that is not fully allocated from the outset, and the protocol is
designed to tolerate and embrace implementations that start using more
codepoints than initially allocated.[¶](#section-7.1-1){.pilcrow}
...
intended lifetime of the protocol.[¶](#section-7.1-2){.pilcrow}

CBOR has three major extension points:[¶](#section-7.1-3){.pilcrow}

[]{.break}

the \"simple\" space (values in major type 7):
:   Of the 24 efficient (and 224 slightly less efficient) values, only a
    small number have been allocated. Implementations receiving an
    ...
    registry in [Section 9.1](#ianasimple){.xref} is the appropriate way
    to address the extensibility of this codepoint
    space.[¶](#section-7.1-4.2){.pilcrow}
:   
...
:   The total codepoint space is abundant; only a tiny part of it has
    ...
    consume two (\"1+1\") bytes, with nearly a quarter already
    allocated. These subspaces need some curation to last for a few more
    decades. Implementations receiving an unknown tag number can choose
    to process just the enclosed tag content or, preferably, to process
    the tag as an unknown tag number wrapping the tag content. The IANA
    registry in [Section 9.2](#ianatags){.xref} is the appropriate way
    to address the extensibility of this codepoint
    space.[¶](#section-7.1-4.4){.pilcrow}
:   

the \"additional information\" space:
:   An implementation receiving an unknown additional information value
    has no way to continue decoding, so allocating codepoints in this
    space is a major step beyond just exercising an extension point.
    There are also very few codepoints left. See also [Section
    7.2](#curating){.xref}.[¶](#section-7.1-4.6){.pilcrow}
:   
:::
:::
...
::: {#curating}
::: {#section-7.2 .section}
### [7.2.](#section-7.2){.section-number .selfRef} [Curating the Additional Information Space](#name-curating-the-additional-inf){.section-name .selfRef} {#name-curating-the-additional-inf}

The human mind is sometimes drawn to filling in little perceived gaps to
make something neat. We expect the remaining gaps in the codepoint space
for the additional information values to be an attractor for new ideas,
just because they are there.[¶](#section-7.2-1){.pilcrow}
...
space can only be done by updating this
specification.[¶](#section-7.2-2){.pilcrow}
...
information values 28 and 29 should be viewed as candidates for 128-bit
and 256-bit quantities, in case a need arises to add them to the
protocol. Additional information value 30 is then the only additional
information value available for general allocation, and there should be
a very good reason for allocating it before assigning it through an
update of the present specification.[¶](#section-7.2-3){.pilcrow}
:::
:::
:::
:::
...
::: {#diagnostic-notation}
::: {#section-8 .section}
## [8.](#section-8){.section-number .selfRef} [Diagnostic Notation](#name-diagnostic-notation){.section-name .selfRef} {#name-diagnostic-notation}
...
happens in the binary format.[¶](#section-8-1){.pilcrow}
...
document. (Implementers looking for a text-based format for representing
CBOR data items in configuration files may also want to consider YAML
\[[YAML](#YAML){.xref}\].)[¶](#section-8-2){.pilcrow}

The diagnostic notation is loosely based on JSON as it is defined in RFC
8259, extending it where needed.[¶](#section-8-3){.pilcrow}
...
key position). Undefined is written \>undefined\< as in JavaScript. The
non-finite floating-point numbers Infinity, -Infinity, and NaN are
...
written as an integer number for the tag number, followed by the tag
content in parentheses; for instance, a date in the format specified by
RFC 3339 (ISO 8601) could be notated as:[¶](#section-8-4){.pilcrow}

0(\"2013-03-21T20:04:00Z\")[¶](#section-8-5){.pilcrow}

or the equivalent relative time as the
following:[¶](#section-8-6){.pilcrow}

1(1363896240)[¶](#section-8-7){.pilcrow}

Byte strings are notated in one of the base encodings, without padding,
enclosed in single quotes, prefixed by \>h\< for base16, \>b32\< for
base32, \>h32\< for base32hex, \>b64\< for base64 or base64url (the
actual encodings do not overlap, so the string remains unambiguous). For
example, the byte string 0x12345678 could be written h\'12345678\',
b32\'CI2FM6A\', or b64\'EjRWeA\'.[¶](#section-8-8){.pilcrow}
...
type 7, value 42.[¶](#section-8-9){.pilcrow}
...
\[[RFC8610](#RFC8610){.xref}\], \"Extended Diagnostic Notation\" (EDN).
Similarly, this notation could be extended in a separate document to
provide documentation for NaN payloads, which are not covered in this
document.[¶](#section-8-10){.pilcrow}

::: {#encoding-indicators}
::: {#section-8.1 .section}
### [8.1.](#section-8.1){.section-number .selfRef} [Encoding Indicators](#name-encoding-indicators){.section-name .selfRef} {#name-encoding-indicators}
...
encoded as a half-, single-, or double-precision
float.[¶](#section-8.1-1){.pilcrow}

The convention for encoding indicators is that anything starting with an
underscore and all following characters that are alphanumeric or
...
indicators are always optional.[¶](#section-8.1-2){.pilcrow}
...
contains an indicator that an indefinite-length representation was used
to represent the data item \[1, 2\].[¶](#section-8.1-3){.pilcrow}

An underscore followed by a decimal digit n indicates that the preceding
item (or, for arrays and maps, the item starting with the preceding
bracket or brace) was encoded with an additional information value of
24+n. For example, 1.5_1 is a half-precision floating-point number,
while 1.5_3 is encoded as double precision. This encoding indicator is
not shown in [Appendix A](#examples){.xref}. (Note that the encoding
indicator \"\_\" is thus an abbreviation of the full form \"\_7\", which
is not used.)[¶](#section-8.1-4){.pilcrow}

The detailed chunk structure of byte and text strings of indefinite
length can be notated in the form (\_ h\'0123\', h\'4567\') and (\_
\"foo\", \"bar\"). However, for an indefinite-length string with no
chunks inside, (\_ ) would be ambiguous as to whether a byte string
(0x5fff) or a text string (0x7fff) is meant and is therefore not used.
The basic forms \'\'\_ and \"\"\_ can be used instead and are reserved
for the case of no chunks only \-- not as short forms for the
(permitted, but not really useful) encodings with only empty chunks,
which need to be notated as (\_ \'\'), (\_ \"\"), etc., to preserve the
chunk structure.[¶](#section-8.1-5){.pilcrow}
:::
:::
:::
:::
...
in \[[RFC8126](#RFC8126){.xref}\]. IANA has also assigned a new media
type, an associated CoAP Content-Format entry, and a structured syntax
suffix.[¶](#section-9-1){.pilcrow}

::: {#ianasimple}
::: {#section-9.1 .section}
### [9.1.](#section-9.1){.section-number .selfRef} [CBOR Simple Values Registry](#name-cbor-simple-values-registry){.section-name .selfRef} {#name-cbor-simple-values-registry}

IANA has created the \"Concise Binary Object Representation (CBOR)
Simple Values\" registry at
\[[IANA.cbor-simple-values](#IANA.cbor-simple-values){.xref}\]. The
initial values are shown in [Table
4](#fpnoconttbl2){.xref}.[¶](#section-9.1-1){.pilcrow}

New entries in the range 0 to 19 are assigned by Standards Action
\[[RFC8126](#RFC8126){.xref}\]. It is suggested that IANA allocate
values starting with the number 16 in order to reserve the lower numbers
for contiguous blocks (if any).[¶](#section-9.1-2){.pilcrow}

New entries in the range 32 to 255 are assigned by Specification
Required.[¶](#section-9.1-3){.pilcrow}
:::
:::

::: {#ianatags}
::: {#section-9.2 .section}
### [9.2.](#section-9.2){.section-number .selfRef} [CBOR Tags Registry](#name-cbor-tags-registry){.section-name .selfRef} {#name-cbor-tags-registry}

IANA has created the \"Concise Binary Object Representation (CBOR)
Tags\" registry at \[[IANA.cbor-tags](#IANA.cbor-tags){.xref}\]. The
tags that were defined in \[[RFC7049](#RFC7049){.xref}\] are described
in detail in [Section 3.4](#tags){.xref}, and other tags have already
been defined since then.[¶](#section-9.2-1){.pilcrow}
...
The template for registration requests is:[¶](#section-9.2-2){.pilcrow}

-   [Data item[¶](#section-9.2-3.1){.pilcrow}]{#section-9.2-3.1}
-   [Semantics (short
    form)[¶](#section-9.2-3.2){.pilcrow}]{#section-9.2-3.2}

In addition, First Come First Served requests should
include:[¶](#section-9.2-4){.pilcrow}

-   [Point of contact[¶](#section-9.2-5.1){.pilcrow}]{#section-9.2-5.1}
-   [Description of semantics (URL) \-- This description is optional;
    the URL can point to something like an Internet-Draft or a web
    page.[¶](#section-9.2-5.2){.pilcrow}]{#section-9.2-5.2}

Applicants exercising the First Come First Served range and making a
suggestion for a tag number that is not representable in 32 bits (i.e.,
larger than 4294967295) should be aware that this could reduce
interoperability with implementations that do not support 64-bit
numbers.[¶](#section-9.2-6){.pilcrow}
:::
:::

::: {#media-type-mime-type}
::: {#section-9.3 .section}
### [9.3.](#section-9.3){.section-number .selfRef} [Media Types Registry](#name-media-types-registry){.section-name .selfRef} {#name-media-types-registry}

The Internet media type \[[RFC6838](#RFC6838){.xref}\] (\"MIME type\")
for a single encoded CBOR data item is \"application/cbor\" as defined
in the \"Media Types\" registry
\[[IANA.media-types](#IANA.media-types){.xref}\]:[¶](#section-9.3-1){.pilcrow}
...
Subtype name:
:   cbor[¶](#section-9.3-2.4){.pilcrow}
:   

Required parameters:
:   n/a[¶](#section-9.3-2.6){.pilcrow}
:   

Optional parameters:
:   n/a[¶](#section-9.3-2.8){.pilcrow}
:   

Encoding considerations:
:   Binary[¶](#section-9.3-2.10){.pilcrow}
:   

Security considerations:
:   See [Section 10](#securitycons){.xref} of RFC
    8949.[¶](#section-9.3-2.12){.pilcrow}
:   

Interoperability considerations:
:   n/a[¶](#section-9.3-2.14){.pilcrow}
:   

Published specification:
:   RFC 8949[¶](#section-9.3-2.16){.pilcrow}
:   

Applications that use this media type:
:   Many[¶](#section-9.3-2.18){.pilcrow}
:   
...
:   \
    ...
    Magic number(s):
    :   n/a[¶](#section-9.3-2.20.2.2){.pilcrow}
    :   

    File extension(s):
    :   .cbor[¶](#section-9.3-2.20.2.4){.pilcrow}
    :   

    Macintosh file type code(s):
    :   n/a[¶](#section-9.3-2.20.2.6){.pilcrow}
    :   
...
:   IETF CBOR Working Group (cbor\@ietf.org) or IETF Applications and
    Real-Time Area (art\@ietf.org)[¶](#section-9.3-2.22){.pilcrow}
:   

Intended usage:
:   COMMON[¶](#section-9.3-2.24){.pilcrow}
:   

Restrictions on usage:
:   none[¶](#section-9.3-2.26){.pilcrow}
:   

Author:
:   IETF CBOR Working Group
    (cbor\@ietf.org)[¶](#section-9.3-2.28){.pilcrow}
:   

Change controller:
:   The IESG (iesg\@ietf.org)[¶](#section-9.3-2.30){.pilcrow}
:   
:::
:::
...
::: {#coap-content-format}
::: {#section-9.4 .section}
### [9.4.](#section-9.4){.section-number .selfRef} [CoAP Content-Format Registry](#name-coap-content-format-registr){.section-name .selfRef} {#name-coap-content-format-registr}
...
\[[IANA.core-parameters](#IANA.core-parameters){.xref}\]:[¶](#section-9.4-1){.pilcrow}

[]{.break}

Media Type:
:   application/cbor[¶](#section-9.4-2.2){.pilcrow}
:   

Encoding:
:   \-[¶](#section-9.4-2.4){.pilcrow}
:   

ID:
:   60[¶](#section-9.4-2.6){.pilcrow}
:   

Reference:
:   RFC 8949[¶](#section-9.4-2.8){.pilcrow}
:   
:::
:::
...
::: {#section-9.5 .section}
### [9.5.](#section-9.5){.section-number .selfRef} [Structured Syntax Suffix Registry](#name-structured-syntax-suffix-re){.section-name .selfRef} {#name-structured-syntax-suffix-re}

The structured syntax suffix \[[RFC6838](#RFC6838){.xref}\] for media
types based on a single encoded CBOR data item is +cbor, which IANA has
registered in the \"Structured Syntax Suffixes\" registry
\[[IANA.structured-suffix](#IANA.structured-suffix){.xref}\]:[¶](#section-9.5-1){.pilcrow}

[]{.break}

Name:
:   Concise Binary Object Representation
    (CBOR)[¶](#section-9.5-2.2){.pilcrow}
:   

+suffix:
:   +cbor[¶](#section-9.5-2.4){.pilcrow}
:   

References:
:   RFC 8949[¶](#section-9.5-2.6){.pilcrow}
:   

Encoding Considerations:
:   CBOR is a binary format.[¶](#section-9.5-2.8){.pilcrow}
:   

Interoperability Considerations:
:   n/a[¶](#section-9.5-2.10){.pilcrow}
:   

Fragment Identifier Considerations:

:   The syntax and semantics of fragment identifiers specified for +cbor
    [SHOULD]{.bcp14} be as specified for \"application/cbor\". (At
    publication of RFC 8949, there is no fragment identification syntax
    defined for \"application/cbor\".)[¶](#section-9.5-2.12.1){.pilcrow}

    The syntax and semantics for fragment identifiers for a specific
    \"xxx/yyy+cbor\" [SHOULD]{.bcp14} be processed as
    follows:[¶](#section-9.5-2.12.2){.pilcrow}

    -   [For cases defined in +cbor, where the fragment identifier
        resolves per the +cbor rules, then process as specified in
        +cbor.[¶](#section-9.5-2.12.3.1){.pilcrow}]{#section-9.5-2.12.3.1}
    -   [For cases defined in +cbor, where the fragment identifier does
        not resolve per the +cbor rules, then process as specified in
        \"xxx/yyy+cbor\".[¶](#section-9.5-2.12.3.2){.pilcrow}]{#section-9.5-2.12.3.2}
    -   [For cases not defined in +cbor, then process as specified in
        \"xxx/yyy+cbor\".[¶](#section-9.5-2.12.3.3){.pilcrow}]{#section-9.5-2.12.3.3}

:   

Security Considerations:
:   See [Section 10](#securitycons){.xref} of RFC
    8949.[¶](#section-9.5-2.14){.pilcrow}
:   

Contact:
:   IETF CBOR Working Group (cbor\@ietf.org) or IETF Applications and
    Real-Time Area (art\@ietf.org)[¶](#section-9.5-2.16){.pilcrow}
:   

Author/Change Controller:
:   IETF[¶](#section-9.5-2.18){.pilcrow}
:   
:::
:::
:::
...
processing logic for incoming data. Complex parsers are well known as a
likely source of such vulnerabilities, such as the ability to remotely
crash a node, or even remotely execute arbitrary code on it. CBOR
attempts to narrow the opportunities for introducing such
vulnerabilities by reducing parser complexity, by giving the entire
range of encodable values a meaning where
possible.[¶](#section-10-1){.pilcrow}
...
control of the system decoding the CBOR data item. A CBOR decoder needs
to assume that all input may be hostile even if it has been checked by a
firewall, has come over a secure channel such as TLS, is encrypted or
signed, or has come from some other source that is presumed
trusted.[¶](#section-10-2){.pilcrow}

[Section 4.1](#preferred){.xref} gives examples of limitations in
interoperability when using a constrained CBOR decoder with input from a
CBOR encoder that uses a non-preferred serialization. When a single data
item is consumed both by such a constrained decoder and a full decoder,
it can lead to security issues that can be exploited by an attacker who
can inject or manipulate content.[¶](#section-10-3){.pilcrow}

As discussed throughout this document, there are many values that can be
considered \"equivalent\" in some circumstances and \"not equivalent\"
in others. As just one example, the numeric value for the number \"one\"
might be expressed as an integer or a bignum. A system interpreting CBOR
input might accept either form for the number \"one\", or might reject
one (or both) forms. Such acceptance or rejection can have security
implications in the program that is using the interpreted
input.[¶](#section-10-4){.pilcrow}
...
extremely large or too short. Resource exhaustion attacks might attempt
to lure a decoder into allocating very big data items (strings, arrays,
maps, or even arbitrary precision numbers) or exhaust the stack depth by
setting up deeply nested items. Decoders need to have appropriate
resource management to mitigate these attacks. (Items for which very
large sizes are given can also attempt to exploit integer overflow
vulnerabilities.)[¶](#section-10-5){.pilcrow}
...
well-formedness was detected. If possible, any data decoded up to this
point should have no impact on the application using the CBOR
decoder.[¶](#section-10-6){.pilcrow}
...
those checks to the application using the decoder. This choice needs to
be clearly documented in the decoder. Beyond the validity at the CBOR
level, an application also needs to ascertain that the input is in
alignment with the application protocol that is serialized in
CBOR.[¶](#section-10-7){.pilcrow}

The input check itself may consume resources. This is usually linear in
the size of the input, which means that an attacker has to spend
resources that are commensurate to the resources spent by the defender
on input validation. However, an attacker might be able to craft inputs
that will take longer for a target decoder to process than for the
attacker to produce. Processing for arbitrary-precision numbers may
...
attacked to spend quadratic effort, unless a secret key (see Section 7
of \[[SIPHASH_LNCS](#SIPHASH_LNCS){.xref}\], also
\[[SIPHASH_OPEN](#SIPHASH_OPEN){.xref}\]) or some other mitigation is
employed. Such superlinear efforts can be exploited by an attacker to
exhaust resources at or before the input validator; they therefore need
to be avoided in a CBOR decoder implementation. Note that tag number
definitions and their implementations can add security considerations of
this kind; this should then be discussed in the security considerations
of the tag number definition.[¶](#section-10-8){.pilcrow}

CBOR encoders do not receive input directly from the network and are
thus not directly attackable in the same way as CBOR decoders. However,
CBOR encoders often have an API that takes input from another level in
the implementation and can be attacked through that API. The design and
implementation of that API should assume the behavior of its caller may
be based on hostile input or on coding mistakes. It should check inputs
for buffer overruns, overflow and underflow of integer arithmetic, and
other such errors that are aimed to disrupt the
encoder.[¶](#section-10-9){.pilcrow}
...
example, an attacker could make use of invalid input such as duplicate
keys in maps, or exploit different precision in processing numbers to
make one application base its decisions on a different interpretation
than the one that will be used by a second application. To facilitate
consistent interpretation, encoder and decoder implementations should
provide a validity-checking mode of operation ([Section
5.4](#validity-checking){.xref}). Note, however, that a generic decoder
...
performing its own input checking. Also, since the set of defined tag
numbers evolves, the application may employ a tag number that is not yet
supported for validity checking by the generic decoder it uses. Generic
decoders therefore need to document which tag numbers they support and
what validity checking they provide for those tag numbers as well as for
basic CBOR (UTF-8 checking, duplicate map key
checking).[¶](#section-10-10){.pilcrow}

[Section 3.4.3](#bignums){.xref} notes that using the non-preferred
choice of a bignum representation instead of a basic integer for
encoding a number is not intended to have application semantics, but it
can have such semantics if an application receiving CBOR data is using a
decoder in the basic generic data model. This disparity causes a
security issue if the two sets of semantics differ. Thus, applications
using CBOR need to specify the data model that they are using for each
use of CBOR data.[¶](#section-10-11){.pilcrow}

It is common to convert CBOR data to other formats. In many cases, CBOR
has more expressive types than other formats; this is particularly true
for the common conversion to JSON. The loss of type information can
cause security issues for the systems that are processing the
less-expressive data.[¶](#section-10-12){.pilcrow}

[Section 6.2](#converting-from-json){.xref} describes a possibly common
usage scenario of converting between CBOR and JSON that could allow an
attack if the attacker knows that the application is performing the
conversion.[¶](#section-10-13){.pilcrow}

Security considerations for the use of base16 and base64 from
\[[RFC4648](#RFC4648){.xref}\], and the use of UTF-8 from
\[[RFC3629](#RFC3629){.xref}\], are relevant to CBOR as
well.[¶](#section-10-14){.pilcrow}
:::
:::
...
:   [International Organization for Standardization]{.refAuthor},
    ...
    [Fourth Edition]{.refContent}, [ISO/IEC 9899:2018]{.seriesInfo},
    June 2018, \<<https://www.iso.org/standard/74528.html>\>.
:   

\[Cplusplus20\]
:   [International Organization for Standardization]{.refAuthor},
    [\"Programming languages - C++\"]{.refTitle}, [Sixth
    Edition]{.refContent}, [ISO/IEC DIS 14882]{.seriesInfo}, [ISO/IEC
    ISO/IEC JTC1 SC22 WG21 N 4860]{.seriesInfo}, March 2020,
    \<<https://isocpp.org/files/papers/N4860.pdf>\>.
:   

\[IEEE754\]
:   [IEEE]{.refAuthor}, [\"IEEE Standard for Floating-Point
    Arithmetic\"]{.refTitle}, [IEEE Std 754-2019]{.seriesInfo}, [DOI
...
\[TIME_T\]
:   [The Open Group]{.refAuthor}, [\"The Open Group Base
    Specifications\"]{.refTitle}, [Section 4.16, \'Seconds Since the
    Epoch\']{.refContent}, [Issue 7, 2018 Edition]{.refContent}, [IEEE
    Std 1003.1]{.seriesInfo}, 2018,
...
:   [International Telecommunication Union]{.refAuthor}, [\"Information
    ...
    X.690]{.seriesInfo}, 2015,
    \<<https://www.itu.int/rec/T-REC-X.690-201508-I/en>\>.
:   

\[BSON\]
:   [Various]{.refAuthor}, [\"BSON - Binary JSON\"]{.refTitle},
    \<<http://bsonspec.org/>\>.
:   

\[CBOR-TAGS\]
:   [Bormann, C.]{.refAuthor}, [\"Notable CBOR Tags\"]{.refTitle}, [Work
    in Progress]{.refContent}, [Internet-Draft,
    draft-bormann-cbor-notable-tags-02]{.seriesInfo}, 25 June 2020,
    \<<https://tools.ietf.org/html/draft-bormann-cbor-notable-tags-02>\>.
:   

\[ECMA262\]
:   [Ecma International]{.refAuthor}, [\"ECMAScript 2020 Language
    Specification\"]{.refTitle}, [Standard ECMA-262, 11th
    Edition]{.refContent}, June 2020,
    \<<https://www.ecma-international.org/publications/standards/Ecma-262.htm>\>.
:   

\[Err3764\]
:   [RFC Errata]{.refAuthor}, [Erratum ID 3764]{.refTitle}, [RFC
    7049]{.refContent}, \<<https://www.rfc-editor.org/errata/eid3764>\>.
:   

\[Err3770\]
:   [RFC Errata]{.refAuthor}, [Erratum ID 3770]{.refTitle}, [RFC
    7049]{.refContent}, \<<https://www.rfc-editor.org/errata/eid3770>\>.
:   

\[Err4294\]
:   [RFC Errata]{.refAuthor}, [Erratum ID 4294]{.refTitle}, [RFC
    7049]{.refContent}, \<<https://www.rfc-editor.org/errata/eid4294>\>.
:   

\[Err4409\]
:   [RFC Errata]{.refAuthor}, [Erratum ID 4409]{.refTitle}, [RFC
    7049]{.refContent}, \<<https://www.rfc-editor.org/errata/eid4409>\>.
:   

\[Err4963\]
:   [RFC Errata]{.refAuthor}, [Erratum ID 4963]{.refTitle}, [RFC
    7049]{.refContent}, \<<https://www.rfc-editor.org/errata/eid4963>\>.
:   

\[Err4964\]
:   [RFC Errata]{.refAuthor}, [Erratum ID 4964]{.refTitle}, [RFC
    7049]{.refContent}, \<<https://www.rfc-editor.org/errata/eid4964>\>.
:   

\[Err5434\]
:   [RFC Errata]{.refAuthor}, [Erratum ID 5434]{.refTitle}, [RFC
    7049]{.refContent}, \<<https://www.rfc-editor.org/errata/eid5434>\>.
:   

\[Err5763\]
:   [RFC Errata]{.refAuthor}, [Erratum ID 5763]{.refTitle}, [RFC
    7049]{.refContent}, \<<https://www.rfc-editor.org/errata/eid5763>\>.
:   

\[Err5917\]
:   [RFC Errata]{.refAuthor}, [Erratum ID 5917]{.refTitle}, [RFC
    7049]{.refContent}, \<<https://www.rfc-editor.org/errata/eid5917>\>.
:   

\[IANA.cbor-simple-values\]
:   [IANA]{.refAuthor}, [\"Concise Binary Object Representation (CBOR)
    Simple Values\"]{.refTitle},
...
:   [IANA]{.refAuthor}, [\"Structured Syntax Suffixes\"]{.refTitle},
    \<<https://www.iana.org/assignments/media-type-structured-suffix>\>.
:   

\[MessagePack\]
:   [Furuhashi, S.]{.refAuthor}, [\"MessagePack\"]{.refTitle},
    \<<https://msgpack.org/>\>.
:   

\[PCRE\]
:   [Hazel, P.]{.refAuthor}, [\"PCRE - Perl Compatible Regular
    Expressions\"]{.refTitle}, \<<https://www.pcre.org/>\>.
:   

\[RFC0713\]
:   [Haverty, J.]{.refAuthor}, [\"MSDTP-Message Services Data
    Transmission Protocol\"]{.refTitle}, [RFC 713]{.seriesInfo}, [DOI
    10.17487/RFC0713]{.seriesInfo}, April 1976,
...
\[RFC8746\]
:   [Bormann, C., Ed.]{.refAuthor}, [\"Concise Binary Object
    Representation (CBOR) Tags for Typed Arrays\"]{.refTitle}, [RFC
    8746]{.seriesInfo}, [DOI 10.17487/RFC8746]{.seriesInfo}, February
    2020, \<<https://www.rfc-editor.org/info/rfc8746>\>.
:   

\[SIPHASH_LNCS\]
:   [Aumasson, J.]{.refAuthor}[ and D. Bernstein]{.refAuthor},
    [\"SipHash: A Fast Short-Input PRF\"]{.refTitle}, [Progress in
    Cryptology - INDOCRYPT 2012, pp. 489-508]{.refContent}, [DOI
    10.1007/978-3-642-34931-7_28]{.seriesInfo}, 2012,
    \<<https://doi.org/10.1007/978-3-642-34931-7_28>\>.
:   

\[SIPHASH_OPEN\]
:   [Aumasson, J.]{.refAuthor}[ and D.J. Bernstein]{.refAuthor},
    [\"SipHash: a fast short-input PRF\"]{.refTitle},
    \<<https://www.aumasson.jp/siphash/siphash.pdf>\>.
:   

\[YAML\]
:   [Ben-Kiki, O.]{.refAuthor}[, Evans, C.]{.refAuthor}[, and I.d.
    Net]{.refAuthor}, [\"YAML Ain\'t Markup Language (YAML\[TM\])
    Version 1.2\"]{.refTitle}, [3rd Edition]{.refContent}, October 2009,
...
## [Appendix A.](#section-appendix.a){.section-number .selfRef} [Examples of Encoded CBOR Data Items](#name-examples-of-encoded-cbor-da){.section-name .selfRef} {#name-examples-of-encoded-cbor-da}
...
notation for a UTF-8 string containing the single Unicode character
[U+00FC (LATIN SMALL LETTER U WITH DIAERESIS, \"ü\")]{.unicode}.
Similarly, \"\\u6c34\" is a UTF-8 string in diagnostic notation with a
single character [U+6C34 (CJK UNIFIED IDEOGRAPH-6C34,
\"水\")]{.unicode}, often representing \"water\", and \"\\ud800\\udd51\"
is a UTF-8 string in diagnostic notation with a single character
[U+10151 (GREEK ACROPHONIC ATTIC FIFTY STATERS, \"𐅑\")]{.unicode}. (Note
that all these single-character strings could also be represented in
native UTF-8 in diagnostic notation, just not if an ASCII-only
specification is required.) In the diagnostic notation provided for
...
2(h\'010000000000000000\')).[¶](#section-appendix.a-1){.pilcrow}

[]{#name-examples-of-encoded-cbor-dat}
...
::: {#table_examples}
  ...
  : [Table 6](#table-6){.selfRef}: [Examples of Encoded CBOR Data
  Items](#name-examples-of-encoded-cbor-dat){.selfRef}
:::
:::
:::
...
## [Appendix B.](#section-appendix.b){.section-number .selfRef} [Jump Table for Initial Byte](#name-jump-table-for-initial-byte){.section-name .selfRef} {#name-jump-table-for-initial-byte}
...
integers are in network byte order.)[¶](#section-appendix.b-1){.pilcrow}

[]{#name-jump-table-for-initial-byte-2}
...
::: {#jumptable}
  ...
  0xc0         text-based date/time (data item follows; see [Section 3.4.1](#stringdatetimesect){.xref})
  0xc1         epoch-based date/time (data item follows; see [Section 3.4.2](#epochdatetimesect){.xref})
  0xc2         unsigned bignum (data item \"byte string\" follows)
  0xc3         negative bignum (data item \"byte string\" follows)
  0xc4         decimal Fraction (data item \"array\" follows; see [Section 3.4.4](#fractions){.xref})
  0xc5         bigfloat (data item \"array\" follows; see [Section 3.4.4](#fractions){.xref})
  0xc6..0xd4   (tag)
  0xd5..0xd7   expected conversion (data item follows; see [Section 3.4.5.2](#convexpect){.xref})
  ...
  : [Table 7](#table-7){.selfRef}: [Jump Table for Initial
  Byte](#name-jump-table-for-initial-byte-2){.selfRef}
:::
:::
:::
...
## [Appendix C.](#section-appendix.c){.section-number .selfRef} [Pseudocode](#name-pseudocode){.section-name .selfRef} {#name-pseudocode}

The well-formedness of a CBOR item can be checked by the pseudocode in
[Figure 1](#pseudo){.xref}. The data is well-formed if and only
if:[¶](#section-appendix.c-1){.pilcrow}

-   [the pseudocode does not
    \"fail\";[¶](#section-appendix.c-2.1){.pilcrow}]{#section-appendix.c-2.1}
-   [after execution of the pseudocode, no bytes are left in the input
    (except in streaming
    applications).[¶](#section-appendix.c-2.2){.pilcrow}]{#section-appendix.c-2.2}

The pseudocode has the following
prerequisites:[¶](#section-appendix.c-3){.pilcrow}

-   [take(n) reads n bytes from the input data and returns them as a
    byte string. If n bytes are no longer available, take(n)
    fails.[¶](#section-appendix.c-4.1){.pilcrow}]{#section-appendix.c-4.1}
-   [uint() converts a byte string into an unsigned integer by
    interpreting the byte string in network byte
    order.[¶](#section-appendix.c-4.2){.pilcrow}]{#section-appendix.c-4.2}
-   [Arithmetic works as in
    C.[¶](#section-appendix.c-4.3){.pilcrow}]{#section-appendix.c-4.3}
-   [All variables are unsigned integers of sufficient
    range.[¶](#section-appendix.c-4.4){.pilcrow}]{#section-appendix.c-4.4}

Note that `well_formed` returns the major type for well-formed
definite-length items, but 99 for an indefinite-length item (or -1 for a
\"break\" stop code, only if `breakable` is set). This is used in
`well_formed_indefinite` to ascertain that indefinite-length strings
only contain definite-length strings as
chunks.[¶](#section-appendix.c-5){.pilcrow}

[]{#name-pseudocode-for-well-formedn}
...
[Figure 1](#figure-1){.selfRef}: [Pseudocode for Well-Formedness
Check](#name-pseudocode-for-well-formedn){.selfRef}
:::
...
appropriate form.[¶](#section-appendix.c-7){.pilcrow}

Major types 0 and 1 are designed in such a way that they can be encoded
in C from a signed integer without actually doing an if-then-else for
positive/negative ([Figure 2](#branchless){.xref}). This uses the fact
that (-1-n), the transformation for major type 1, is the same as \~n
(bitwise complement) in C unsigned arithmetic; \~n can then be expressed
as (-1)\^n for the negative case, while 0\^n leaves n unchanged for
nonnegative. The sign of a number can be converted to -1 for negative
and 0 for nonnegative (0 or positive) by arithmetic-shifting the number
by one bit less than the bit length of the number (for example, by 63
for 64-bit numbers).[¶](#section-appendix.c-8){.pilcrow}

[]{#name-pseudocode-for-encoding-a-s}

::: {#branchless}
::: {#section-appendix.c-9.1}
``` {.sourcecode .lang-pseudocode}
...
[Figure 2](#figure-2){.selfRef}: [Pseudocode for Encoding a Signed
Integer](#name-pseudocode-for-encoding-a-s){.selfRef}
:::

See [Section 1.2](#terminology){.xref} for some specific assumptions
about the profile of the C language used in these pieces of
code.[¶](#section-appendix.c-10){.pilcrow}
:::
:::

::: {#half-precision}
::: {#section-appendix.d .section}
## [Appendix D.](#section-appendix.d){.section-number .selfRef} [Half-Precision](#name-half-precision){.section-name .selfRef} {#name-half-precision}

As half-precision floating-point numbers were only added to IEEE 754 in
2008 \[[IEEE754](#IEEE754){.xref}\], today\'s programming platforms
often still only have limited support for them. It is very easy to
include at least decoding support for them even without such support. An
example of a small decoder for half-precision floating-point numbers in
the C language is shown in [Figure 3](#decode-half-c){.xref}. A similar
program for Python is in [Figure 4](#decode-half-py){.xref}; this code
assumes that the 2-byte value has already been decoded as an (unsigned
short) integer in network byte order (as would be done by the pseudocode
in [Appendix
C](#pseudocode){.xref}).[¶](#section-appendix.d-1){.pilcrow}

[]{#name-c-code-for-a-half-precision}

::: {#decode-half-c}
::: {#section-appendix.d-2.1}
``` {.sourcecode .lang-c}
#include <math.h>
...
[Figure 3](#figure-3){.selfRef}: [C Code for a Half-Precision
Decoder](#name-c-code-for-a-half-precision){.selfRef}
:::

[]{#name-python-code-for-a-half-prec}
...
[Figure 4](#figure-4){.selfRef}: [Python Code for a Half-Precision
Decoder](#name-python-code-for-a-half-prec){.selfRef}
:::
:::
:::
...
## [Appendix E.](#section-appendix.e){.section-number .selfRef} [Comparison of Other Binary Formats to CBOR\'s Design Objectives](#name-comparison-of-other-binary-){.section-name .selfRef} {#name-comparison-of-other-binary-}

The proposal for CBOR follows a history of binary formats that is as
long as the history of computers themselves. Different formats have had
different objectives. In most cases, the objectives of the format were
never stated, although they can sometimes be implied by the context
where the format was first used. Some formats were meant to be
universally usable, although history has proven that no binary format
meets the needs of all protocols and
applications.[¶](#section-appendix.e-1){.pilcrow}

CBOR differs from many of these formats due to it starting with a set of
objectives and attempting to meet just those. This section compares a
few of the dozens of formats with CBOR\'s objectives in order to help
the reader decide if they want to use CBOR or a different format for a
particular protocol or application.[¶](#section-appendix.e-2){.pilcrow}

Note that the discussion here is not meant to be a criticism of any
format: to the best of our knowledge, no format before CBOR was meant to
cover CBOR\'s objectives in the priority we have assigned them. A brief
recap of the objectives from [Section 1.1](#objectives){.xref}
is:[¶](#section-appendix.e-3){.pilcrow}

1.  [unambiguous encoding of most common data formats from Internet
    standards[¶](#section-appendix.e-4.1){.pilcrow}]{#section-appendix.e-4.1}
2.  [code compactness for encoder or
    decoder[¶](#section-appendix.e-4.2){.pilcrow}]{#section-appendix.e-4.2}
3.  [no schema description
    needed[¶](#section-appendix.e-4.3){.pilcrow}]{#section-appendix.e-4.3}
4.  [reasonably compact
    serialization[¶](#section-appendix.e-4.4){.pilcrow}]{#section-appendix.e-4.4}
5.  [applicability to constrained and unconstrained
    applications[¶](#section-appendix.e-4.5){.pilcrow}]{#section-appendix.e-4.5}
6.  [good JSON
    conversion[¶](#section-appendix.e-4.6){.pilcrow}]{#section-appendix.e-4.6}
7.  [extensibility[¶](#section-appendix.e-4.7){.pilcrow}]{#section-appendix.e-4.7}

A discussion of CBOR and other formats with respect to a different set
of design objectives is provided in Section
[5](https://www.rfc-editor.org/rfc/rfc8618#section-5){.relref} and
Appendix [C](https://www.rfc-editor.org/rfc/rfc8618#appendix-C){.relref}
of \[[RFC8618](#RFC8618){.xref}\].[¶](#section-appendix.e-5){.pilcrow}

::: {#asn1-der-ber-and-per}
::: {#section-e.1 .section}
## [E.1.](#section-e.1){.section-number .selfRef} [ASN.1 DER, BER, and PER](#name-asn1-der-ber-and-per){.section-name .selfRef} {#name-asn1-der-ber-and-per}

\[[ASN.1](#ASN.1){.xref}\] has many serializations. In the IETF, DER and
BER are the most common. The serialized output is not particularly
compact for many items, and the code needed to decode numeric items can
be complex on a constrained device.[¶](#section-e.1-1){.pilcrow}

Few (if any) IETF protocols have adopted one of the several variants of
Packed Encoding Rules (PER). There could be many reasons for this, but
one that is commonly stated is that PER makes use of the schema even for
parsing the surface structure of the data item, requiring significant
tool support. There are different versions of the ASN.1 schema language
in use, which has also hampered adoption.[¶](#section-e.1-2){.pilcrow}
:::
:::

::: {#messagepack}
::: {#section-e.2 .section}
## [E.2.](#section-e.2){.section-number .selfRef} [MessagePack](#name-messagepack){.section-name .selfRef} {#name-messagepack}

\[[MessagePack](#MessagePack){.xref}\] is a concise, widely implemented
counted binary serialization format, similar in many properties to CBOR,
although somewhat less regular. While the data model can be used to
represent JSON data, MessagePack has also been used in many remote
procedure call (RPC) applications and for long-term storage of
data.[¶](#section-e.2-1){.pilcrow}
...
still available for extension. Repeated requests over the years from the
MessagePack user community to separate out binary and text strings in
the encoding recently have led to an extension proposal that would leave
MessagePack\'s \"raw\" data ambiguous between its usages for binary and
text data. The extension mechanism for MessagePack remains
unclear.[¶](#section-e.2-2){.pilcrow}
:::
:::

::: {#bson}
::: {#section-e.3 .section}
## [E.3.](#section-e.3){.section-number .selfRef} [BSON](#name-bson){.section-name .selfRef} {#name-bson}

\[[BSON](#BSON){.xref}\] is a data format that was developed for the
storage of JSON-like maps (JSON objects) in the MongoDB database. Its
major distinguishing feature is the capability for in-place update,
...
database application and has become somewhat baroque. The status of how
BSON extensions will be implemented remains
unclear.[¶](#section-e.3-1){.pilcrow}
:::
:::

::: {#msdtp-rfc-713}
::: {#section-e.4 .section}
## [E.4.](#section-e.4){.section-number .selfRef} [MSDTP: RFC 713](#name-msdtp-rfc-713){.section-name .selfRef} {#name-msdtp-rfc-713}

Message Services Data Transmission (MSDTP) is a very early example of a
compact message format; it is described in
\[[RFC0713](#RFC0713){.xref}\], written in 1976. It is included here for
its historical value, not because it was ever widely
used.[¶](#section-e.4-1){.pilcrow}
:::
:::

::: {#conciseness-on-the-wire}
::: {#section-e.5 .section}
## [E.5.](#section-e.5){.section-number .selfRef} [Conciseness on the Wire](#name-conciseness-on-the-wire){.section-name .selfRef} {#name-conciseness-on-the-wire}
...
wire, many people focus on the wire size. [Table 8](#concise){.xref}
shows some encoding examples for the simple nested array \[1, \[2,
3\]\]; where some form of indefinite-length encoding is supported by the
encoding, \[\_ 1, \[2, 3\]\] (indefinite length on the outer array) is
also shown.[¶](#section-e.5-1){.pilcrow}

[]{#name-examples-for-different-leve}
...
::: {#concise}
  ...
  : [Table 8](#table-8){.selfRef}: [Examples for Different Levels of
  Conciseness](#name-examples-for-different-leve){.selfRef}
:::
:::
:::
:::
...
## [Appendix F.](#section-appendix.f){.section-number .selfRef} [Well-Formedness Errors and Examples](#name-well-formedness-errors-and-){.section-name .selfRef} {#name-well-formedness-errors-and-}

There are three basic kinds of well-formedness errors that can occur in
decoding a CBOR data item:[¶](#section-appendix.f-1){.pilcrow}

[]{.break}

Too much data:
:   There are input bytes left that were not consumed. This is only an
    error if the application assumed that the input bytes would span
    exactly one data item. Where the application uses the
    self-delimiting nature of CBOR encoding to permit additional data
    after the data item, as is done in CBOR sequences
    \[[RFC8742](#RFC8742){.xref}\], for example, the CBOR decoder can
    simply indicate which part of the input has not been
    consumed.[¶](#section-appendix.f-2.2){.pilcrow}
:   

Too little data:
:   The input data available would need additional bytes added at their
    end for a complete CBOR data item. This may indicate the input is
    truncated; it is also a common error when trying to decode random
    data as CBOR. For some applications, however, this may not actually
    be an error, as the application may not be certain it has all the
    data yet and can obtain or wait for additional input bytes. Some of
    ...
    encoded CBOR data item cannot be completed within this
    limit.[¶](#section-appendix.f-2.4){.pilcrow}
:   

Syntax error:
:   The input data are not consistent with the requirements of the CBOR
    encoding, and this cannot be remedied by adding (or removing) data
    at the end.[¶](#section-appendix.f-2.6){.pilcrow}
:   

In [Appendix C](#pseudocode){.xref}, errors of the first kind are
addressed in the first paragraph and bullet list (requiring \"no bytes
are left\"), and errors of the second kind are addressed in the second
paragraph/bullet list (failing \"if n bytes are no longer available\").
Errors of the third kind are identified in the pseudocode by specific
instances of calling fail(), in
order:[¶](#section-appendix.f-3){.pilcrow}

-   [a reserved value is used for additional information (28, 29,
    30)[¶](#section-appendix.f-4.1){.pilcrow}]{#section-appendix.f-4.1}
-   [major type 7, additional information 24, value \< 32
    (incorrect)[¶](#section-appendix.f-4.2){.pilcrow}]{#section-appendix.f-4.2}
-   [incorrect substructure of indefinite-length byte string or text
    ...
    type)[¶](#section-appendix.f-4.3){.pilcrow}]{#section-appendix.f-4.3}
-   [\"break\" stop code (major type 7, additional information 31)
    occurs in a value position of a map or except at a position directly
    in an indefinite-length item where also another enclosed data item
    could
    occur[¶](#section-appendix.f-4.4){.pilcrow}]{#section-appendix.f-4.4}
-   [additional information 31 used with major type 0, 1, or
    6[¶](#section-appendix.f-4.5){.pilcrow}]{#section-appendix.f-4.5}

::: {#examples-for-cbor-data-items-that-are-not-well-formed}
::: {#section-f.1 .section}
## [F.1.](#section-f.1){.section-number .selfRef} [Examples of CBOR Data Items That Are Not Well-Formed](#name-examples-of-cbor-data-items){.section-name .selfRef} {#name-examples-of-cbor-data-items}
...
hexadecimal; multiple examples in a list are separated by
commas.[¶](#section-f.1-1){.pilcrow}

Examples for well-formedness error kind 1 (too much data) can easily be
formed by adding data to a well-formed encoded CBOR data
item.[¶](#section-f.1-2){.pilcrow}
...
data items that would require large amounts of addition to be completed
(for instance by starting the encoding of a string of a very large
size).[¶](#section-f.1-3){.pilcrow}
...
counted or should have been ended by a \"break\" stop
code.[¶](#section-f.1-4){.pilcrow}

[]{.break}

End of input in a head:
:   18, 19, 1a, 1b, 19 01, 1a 01 02, 1b 01 02 03 04 05 06 07, 38, 58,
    78, 98, 9a 01 ff 00, b8, d8, f8, f9 00, fa 00 00, fb 00 00
    00[¶](#section-f.1-5.2){.pilcrow}
:   

Definite-length strings with short data:
:   41, 61, 5a ff ff ff ff 00, 5b ff ff ff ff ff ff ff ff 01 02 03, 7a
    ff ff ff ff 00, 7b 7f ff ff ff ff ff ff ff 01 02
    03[¶](#section-f.1-5.4){.pilcrow}
:   

Definite-length maps and arrays not closed with enough items:
:   81, 81 81 81 81 81 81 81 81 81, 82 00, a1, a2 01 02, a1 00, a2 00 00
    00[¶](#section-f.1-5.6){.pilcrow}
:   

Tag number not followed by tag content:
:   c0[¶](#section-f.1-5.8){.pilcrow}
:   

Indefinite-length strings not closed by a \"break\" stop code:
:   5f 41 00, 7f 61 00[¶](#section-f.1-5.10){.pilcrow}
:   
...
:   9f, 9f 01 02, bf, bf 01 02 01 02, 81 9f, 9f 80 00, 9f 9f 9f 9f 9f ff
    ff ff ff, 9f 81 9f 81 9f 9f ff ff ff[¶](#section-f.1-5.12){.pilcrow}
:   

A few examples for the five subkinds of well-formedness error kind 3
(syntax error) are shown below.[¶](#section-f.1-6){.pilcrow}
...
:   []{.break}
    ...
    :   1c, 1d, 1e, 3c, 3d, 3e, 5c, 5d, 5e, 7c, 7d, 7e, 9c, 9d, 9e, bc,
        bd, be, dc, dd, de, fc, fd,
        fe,[¶](#section-f.1-7.2.2.2){.pilcrow}
    :   
:   
...
:   []{.break}
    ...
    :   f8 00, f8 01, f8 18, f8 1f[¶](#section-f.1-7.4.2.2){.pilcrow}
    :   
:   
...
:   []{.break}
    ...
    :   5f 00 ff, 5f 21 ff, 5f 61 00 ff, 5f 80 ff, 5f a0 ff, 5f c0 00
        ff, 5f e0 ff, 7f 41 00 ff[¶](#section-f.1-7.6.2.2){.pilcrow}
    :   

    Indefinite-length string chunks not definite length:
    :   5f 5f 41 00 ff ff, 7f 7f 61 00 ff
        ff[¶](#section-f.1-7.6.2.4){.pilcrow}
    :   
:   
...
:   []{.break}
    ...
    :   1f, 3f, df[¶](#section-f.1-7.10.2.2){.pilcrow}
    :   
:   
:::
:::
...
## [Appendix G.](#section-appendix.g){.section-number .selfRef} [Changes from RFC 7049](#name-changes-from-rfc-7049){.section-name .selfRef} {#name-changes-from-rfc-7049}

As discussed in the introduction, this document formally obsoletes RFC
7049 while keeping full compatibility with the interchange format from
RFC 7049. This document provides editorial improvements, added detail,
and fixed errata. This document does not create a new version of the
format.[¶](#section-appendix.g-1){.pilcrow}

::: {#errata-processing-clerical-changes}
::: {#section-g.1 .section}
## [G.1.](#section-g.1){.section-number .selfRef} [Errata Processing and Clerical Changes](#name-errata-processing-and-cleri){.section-name .selfRef} {#name-errata-processing-and-cleri}

The two verified errata on RFC 7049, \[[Err3764](#Err3764){.xref}\] and
\[[Err3770](#Err3770){.xref}\], concerned two encoding examples in the
text that have been corrected ([Section 3.4.3](#bignums){.xref}: \"29\"
-\> \"49\", [Section 5.5](#numbers){.xref}: \"0b000_11101\" -\>
\"0b000_11001\"). Also, RFC 7049 contained an example using the numeric
value 24 for a simple value \[[Err5917](#Err5917){.xref}\], which is not
well-formed; this example has been removed. Errata report 5763
\[[Err5763](#Err5763){.xref}\] pointed to an error in the wording of the
definition of tags; this was resolved during a rewrite of [Section
3.4](#tags){.xref}. Errata report 5434 \[[Err5434](#Err5434){.xref}\]
pointed out that the Universal Binary JSON (UBJSON) example in [Appendix
E](#comparison-app){.xref} no longer complied with the version of UBJSON
current at the time of the errata report submission. It turned out that
the UBJSON specification had completely changed since 2013; this example
therefore was removed. Other errata reports
\[[Err4409](#Err4409){.xref}\] \[[Err4963](#Err4963){.xref}\]
\[[Err4964](#Err4964){.xref}\] complained that the map key sorting rules
for canonical encoding were onerous; these led to a reconsideration of
the canonical encoding suggestions and replacement by the deterministic
encoding suggestions (described below). An editorial suggestion in
errata report 4294 \[[Err4294](#Err4294){.xref}\] was also implemented
(improved symmetry by adding \"Second value\" to a comment to the last
example in [Section 3.2.2](#indef){.xref}).[¶](#section-g.1-1){.pilcrow}

Other clerical changes include:[¶](#section-g.1-2){.pilcrow}

-   [the use of new xml2rfc functionality
    \[[RFC7991](#RFC7991){.xref}\];[¶](#section-g.1-3.1){.pilcrow}]{#section-g.1-3.1}
-   [more explanation of the notation
    used;[¶](#section-g.1-3.2){.pilcrow}]{#section-g.1-3.2}
-   [the update of references, e.g., from RFC 4627 to
    \[[RFC8259](#RFC8259){.xref}\], from CNN-TERMS to
    \[[RFC7228](#RFC7228){.xref}\], and from the 5.1 edition to the 11th
    edition of \[[ECMA262](#ECMA262){.xref}\]; the addition of a
    reference to \[[IEEE754](#IEEE754){.xref}\] and importation of
    required definitions; the addition of references to
    \[[C](#C){.xref}\] and \[[Cplusplus20](#Cplusplus20){.xref}\]; and
    the addition of a reference to \[[RFC8618](#RFC8618){.xref}\] that
    further illustrates the discussion in [Appendix
    E](#comparison-app){.xref};[¶](#section-g.1-3.3){.pilcrow}]{#section-g.1-3.3}
-   [in the discussion of diagnostic notation ([Section
    8](#diagnostic-notation){.xref}), the \"Extended Diagnostic
    Notation\" (EDN) defined in \[[RFC8610](#RFC8610){.xref}\] is now
    mentioned, the gap in representing NaN payloads is now highlighted,
    and an explanation of representing indefinite-length strings with no
    chunks has been added ([Section
    8.1](#encoding-indicators){.xref});[¶](#section-g.1-3.4){.pilcrow}]{#section-g.1-3.4}
-   [the addition of this
    appendix.[¶](#section-g.1-3.5){.pilcrow}]{#section-g.1-3.5}
:::
:::

::: {#changes-in-iana-considerations}
::: {#section-g.2 .section}
## [G.2.](#section-g.2){.section-number .selfRef} [Changes in IANA Considerations](#name-changes-in-iana-considerati){.section-name .selfRef} {#name-changes-in-iana-considerati}

The IANA considerations were generally updated (clerical changes, e.g.,
now pointing to the CBOR Working Group as the author of the
specification). References to the respective IANA registries were added
to the informative references.[¶](#section-g.2-1){.pilcrow}

In the \"Concise Binary Object Representation (CBOR) Tags\" registry
\[[IANA.cbor-tags](#IANA.cbor-tags){.xref}\], tags in the space from 256
to 32767 (lower half of \"1+2\") are no longer assigned by First Come
First Served; this range is now Specification
Required.[¶](#section-g.2-2){.pilcrow}
:::
:::

::: {#changes-in-suggestions-and-other-informational-components}
::: {#section-g.3 .section}
## [G.3.](#section-g.3){.section-number .selfRef} [Changes in Suggestions and Other Informational Components](#name-changes-in-suggestions-and-){.section-name .selfRef} {#name-changes-in-suggestions-and-}
...
emphasizing some aspects and de-emphasizing
others.[¶](#section-g.3-1){.pilcrow}

A significant addition is [Section 2](#cbor-data-models){.xref}, which
discusses the CBOR data model and its small variations involved in the
processing of CBOR. The introduction of terms for those variations
(basic generic, extended generic, specific) enables more concise
language in other places of the document and also helps to clarify
expectations of implementations and of the extensibility features of the
format.[¶](#section-g.3-2){.pilcrow}

As a format derived from the JSON ecosystem, RFC 7049 was influenced by
the JSON number system that was in turn inherited from JavaScript at the
time. JSON does not provide distinct integers and floating-point values
(and the latter are decimal in the format). CBOR provides binary
representations of numbers, which do differ between integers and
floating-point values. Experience from implementation and use suggested
that the separation between these two number domains should be more
clearly drawn in the document; language that suggested an integer could
seamlessly stand in for a floating-point value was removed. Also, a
suggestion (based on I-JSON \[[RFC7493](#RFC7493){.xref}\]) was added
for handling these types when converting JSON to CBOR, and the use of a
specific rounding mechanism has been
recommended.[¶](#section-g.3-3){.pilcrow}

For a single value in the data model, CBOR often provides multiple
encoding options. A new section ([Section
4](#serialization-considerations){.xref}) introduces the term
\"preferred serialization\" ([Section 4.1](#preferred){.xref}) and
defines it for various kinds of data items. On the basis of this
terminology, the section then discusses how a CBOR-based protocol can
define \"deterministic encoding\" ([Section 4.2](#det-enc){.xref}),
which avoids terms \"canonical\" and \"canonicalization\" from RFC 7049.
The suggestion of \"Core Deterministic Encoding Requirements\" ([Section
4.2.1](#core-det){.xref}) enables generic support for such
protocol-defined encoding requirements. This document further eases the
implementation of deterministic encoding by simplifying the map ordering
suggested in RFC 7049 to a simple lexicographic ordering of encoded
keys. A description of the older suggestion is kept as an alternative,
now termed \"length-first map key ordering\" ([Section
4.2.3](#length-first){.xref}).[¶](#section-g.3-4){.pilcrow}
...
\"syntax error\", \"decoding error\", and \"strict mode\" outside of
examples. Also, a third level of requirements that an application has on
its input data beyond CBOR-level validity is now explicitly called out.
Well-formed (processable at all), valid (checked by a validity-checking
generic decoder), and expected input (as checked by the application) are
treated as a hierarchy of layers of
acceptability.[¶](#section-g.3-5){.pilcrow}

The handling of non-well-formed simple values was clarified in text and
pseudocode. [Appendix F](#errors){.xref} was added to discuss
well-formedness errors and provide examples for them. The pseudocode was
updated to be more portable, and some portability considerations were
added.[¶](#section-g.3-6){.pilcrow}

The discussion of validity has been sharpened in two areas. Map validity
(handling of duplicate keys) was clarified, and the domain of
applicability of certain implementation choices explained. Also, while
streamlining the terminology for tags, tag numbers, and tag content,
discussion was added on tag validity, and the restrictions were
clarified on tag content, in general and specifically for tag
1.[¶](#section-g.3-7){.pilcrow}

An implementation note (and note for future tag definitions) was added
to [Section 3.4](#tags){.xref} about defining tags with semantics that
depend on serialization order.[¶](#section-g.3-8){.pilcrow}

Tag 35 is not defined by this document; the registration based on the
definition in RFC 7049 remains in place.[¶](#section-g.3-9){.pilcrow}

Terminology was introduced in [Section 3](#encoding){.xref} for
\"argument\" and \"head\", simplifying further
discussion.[¶](#section-g.3-10){.pilcrow}

The security considerations ([Section 10](#securitycons){.xref}) were
mostly rewritten and significantly expanded; in multiple other places,
the document is now more explicit that a decoder cannot simply condone
well-formedness errors.[¶](#section-g.3-11){.pilcrow}
:::
:::
:::
:::
...
CBOR was inspired by MessagePack. MessagePack was developed and promoted
by [Sadayuki Furuhashi]{.contact-name} (\"frsyuki\"). This reference to
MessagePack is solely for attribution; CBOR is not intended as a version
of, or replacement for, MessagePack, as it has different design goals
and requirements.[¶](#section-appendix.h-1){.pilcrow}

The need for functionality beyond the original MessagePack specification
became obvious to many people at about the same time around the year
2012. BinaryPack is a minor derivation of MessagePack that was developed
by [Eric Zhang]{.contact-name} for the binaryjs project. A similar, but
different, extension was made by [Tim Caswell]{.contact-name} for his
msgpack-js and msgpack-js-browser projects. Many people have contributed
to the discussion about extending MessagePack to separate text string
representation from byte string
representation.[¶](#section-appendix.h-2){.pilcrow}

The encoding of the additional information in CBOR was inspired by the
encoding of length information designed by [Klaus Hartke]{.contact-name}
for CoAP.[¶](#section-appendix.h-3){.pilcrow}

This document also incorporates suggestions made by many people, notably
[Dan Frost]{.contact-name}, [James Manger]{.contact-name}, [Jeffrey
Yasskin]{.contact-name}, [Joe Hildebrand]{.contact-name}, [Keith
Moore]{.contact-name}, [Laurence Lundblade]{.contact-name}, [Matthew
Lepinski]{.contact-name}, [Michael Richardson]{.contact-name}, [Nico
Williams]{.contact-name}, [Peter Occil]{.contact-name}, [Phillip
Hallam-Baker]{.contact-name}, [Ray Polk]{.contact-name}, [Stuart
Cheshire]{.contact-name}, [Tim Bray]{.contact-name}, [Tony
Finch]{.contact-name}, [Tony Hansen]{.contact-name}, and [Yaron
Sheffer]{.contact-name}. [Benjamin Kaduk]{.contact-name} provided an
extensive review during IESG processing. [Éric Vyncke]{.contact-name},
[Erik Kline]{.contact-name}, [Robert Wilton]{.contact-name}, and [Roman
Danyliw]{.contact-name} provided further IESG comments, which included
an IoT directorate review by [Eve
Schooler]{.contact-name}.[¶](#section-appendix.h-4){.pilcrow}
:::
:::
