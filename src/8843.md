  RFC 8843           Bundled Media     January 2021
  ------------------ ----------------- --------------
...
The example shows an initial BUNDLE offer. The offer includes two \"m=\"
...
-   ::: {#section-9.2-18.7}
    ...
    -   [[]{.break}]{#section-9.2-18.7.2.1}

        Generic NACK:
        :   (PT=RTPFB, FMT=1)
            \[[RFC4585](#RFC4585){.xref}\].[¶](#section-9.2-18.7.2.1.1.2){.pilcrow}
        :   

        Picture Loss Indication (PLI):
        :   (PT=PSFB, FMT=1)
            \[[RFC4585](#RFC4585){.xref}\].[¶](#section-9.2-18.7.2.1.1.4){.pilcrow}
        :   

        Slice Loss Indication (SLI):
        :   (PT=PSFB, FMT=2)
            \[[RFC4585](#RFC4585){.xref}\].[¶](#section-9.2-18.7.2.1.1.6){.pilcrow}
        :   

        Reference Picture Selection Indication (RPSI):
        :   (PT=PSFB, FMT=3)
            \[[RFC4585](#RFC4585){.xref}\].[¶](#section-9.2-18.7.2.1.1.8){.pilcrow}
        :   
    :::
...
-   ::: {#section-9.2-19.2}
    ...
    -   [[]{.break}]{#section-9.2-19.2.2.1}

        Full Intra Request (FIR):
        :   (PT=PSFB, FMT=4)
            \[[RFC5104](#RFC5104){.xref}\].[¶](#section-9.2-19.2.2.1.1.2){.pilcrow}
        :   

        Temporal-Spatial Trade-off Request (TSTR):
        :   (PT=PSFB, FMT=5)
            \[[RFC5104](#RFC5104){.xref}\].[¶](#section-9.2-19.2.2.1.1.4){.pilcrow}
        :   

        H.271 Video Back Channel Message (VBCM):
        :   (PT=PSFB, FMT=7)
            \[[RFC5104](#RFC5104){.xref}\].[¶](#section-9.2-19.2.2.1.1.6){.pilcrow}
        :   

        Temporary Maximum Media Stream Bit Rate Request (TMMBR):
        :   (PT=RTPFB, FMT=3)
            \[[RFC5104](#RFC5104){.xref}\].[¶](#section-9.2-19.2.2.1.1.8){.pilcrow}
        :   

        Layer Refresh Request (LRR):
        :   (PT=PSFB, FMT=10)
            \[[LLR-RTCP](#I-D.ietf-avtext-lrr){.xref}\].[¶](#section-9.2-19.2.2.1.1.10){.pilcrow}
        :   
    :::
...
-   ::: {#section-9.2-20.1}
    ...
    -   [[]{.break}]{#section-9.2-20.1.2.1}

        Temporal-Spatial Trade-off Notification (TSTN):
        :   (PT=PSFB, FMT=6) \[[RFC5104](#RFC5104){.xref}\]. This
            message is a notification in response to a prior
            TSTR.[¶](#section-9.2-20.1.2.1.1.2){.pilcrow}
        :   

        Temporary Maximum Media Stream Bit Rate Notification (TMMBN):
        :   (PT=RTPFB, FMT=4) \[[RFC5104](#RFC5104){.xref}\]. This
            message is a notification in response to a prior TMMBR, but
            it can also be sent
            unsolicited.[¶](#section-9.2-20.1.2.1.1.4){.pilcrow}
        :   
    :::
...
\"m=\" section is the offerer-tagged \"m=\" section or answerer-tagged
\"m=\" section).[¶](#section-9.3.1-2){.pilcrow}

::: {#sec-rtprtcp-mux-oa-ino}
::: {#section-9.3.1.1 .section}
##### [9.3.1.1.](#section-9.3.1.1){.section-number .selfRef} [Generating the Initial SDP BUNDLE Offer](#name-generating-the-initial-sdp-b){.section-name .selfRef} {#name-generating-the-initial-sdp-b}
...
section in the created BUNDLE group, and the answerer [MUST]{.bcp14}
move the \"m=\" section out of the BUNDLE group ([Section
7.3.2](#sec-sdp-oa-ans-mov){.xref}); include the attribute in the moved
\"m=\" section and enable RTP/RTCP multiplexing for the media associated
with the \"m=\" section; or reject the \"m=\" section ([Section
7.3.3](#sec-sdp-oa-ans-rej){.xref}).[¶](#section-9.3.1.2-2){.pilcrow}
...
-   [The generic SDP attribute offer/answer considerations ([Section
    ...
    contains a BUNDLE group, and when an offerer sends a subsequent
    offer that contains a BUNDLE group, ICE-related media-level
    attributes are only included in the tagged \"m=\" section (suggested
    offerer-tagged \"m=\" section or answerer-tagged \"m=\" section),
    and the ICE properties are applied to each bundled \"m=\" section
    within the BUNDLE
    group.[¶](#section-10-3.2){.pilcrow}]{#section-10-3.2}
...
One or more media streams within a BUNDLE group might use the Datagram
Transport Layer Security (DTLS) protocol \[[RFC6347](#RFC6347){.xref}\]
in order to encrypt the data or negotiate encryption keys if another
encryption mechanism is used to encrypt
media.[¶](#section-11-1){.pilcrow}
...
-   [Each usage of the DTLS association within the BUNDLE group
    ...
    association or an existing DTLS association will be used;
...
::: {#sec-3264}
::: {#section-13 .section}
## [13.](#section-13){.section-number .selfRef} [Update to RFC 3264](#name-update-to-rfc-3264){.section-name .selfRef} {#name-update-to-rfc-3264}

This section updates RFC 3264, in order to allow extensions to define
the usage of a zero port value in offers and answers for purposes other
than removing or disabling media streams. The following sections are
being updated:[¶](#section-13-1){.pilcrow}
...
### [13.1.](#section-13.1){.section-number .selfRef} [Original Text from RFC 3264, Section 5.1, 2nd Paragraph](#name-original-text-from-rfc-3264){.section-name .selfRef} {#name-original-text-from-rfc-3264}
...
> rejected stream (Section 6). Furthermore, existing streams can be
> terminated by setting the port to zero (Section 8). In general, a port
> number of zero indicates that the media stream is not
...
### [13.2.](#section-13.2){.section-number .selfRef} [New Text Replacing RFC 3264, Section 5.1, 2nd Paragraph](#name-new-text-replacing-rfc-3264){.section-name .selfRef} {#name-new-text-replacing-rfc-3264}

For recvonly and sendrecv streams, the port number and address in the
offer indicate where the offerer would like to receive the media stream.
For sendonly RTP streams, the address and port number indirectly
indicate where the offerer wants to receive RTCP reports. Unless there
is an explicit indication otherwise, reports are sent to the port number
one higher than the number indicated. The IP address and port present in
the offer indicate nothing about the source IP address and source port
of the RTP and RTCP packets that will be sent by the offerer. By
default, a port number of zero in the offer indicates that the stream is
offered but [MUST NOT]{.bcp14} be used, but an extension mechanism might
specify different semantics for the usage of a zero port value.
...
that the media stream is not wanted.[¶](#section-13.2-1){.pilcrow}
:::
:::

::: {#sec-3264-old-8_4}
::: {#section-13.3 .section}
### [13.3.](#section-13.3){.section-number .selfRef} [Original Text from RFC 3264, Section 8.4, 6th Paragraph](#name-original-text-from-rfc-3264-){.section-name .selfRef} {#name-original-text-from-rfc-3264-}
...
### [13.4.](#section-13.4){.section-number .selfRef} [New Text Replacing RFC 3264, Section 8.4, 6th Paragraph](#name-new-text-replacing-rfc-3264-){.section-name .selfRef} {#name-new-text-replacing-rfc-3264-}

RFC 2543 \[[RFC2543](#RFC2543){.xref}\] specifies that placing a user on
hold was accomplished by setting the connection address to 0.0.0.0. Its
usage for putting a call on hold is no longer recommended, since it
doesn\'t allow for RTCP to be used with held streams, doesn\'t work with
IPv6, and breaks with connection oriented media. However, it can be
useful in an initial offer when the offerer knows it wants to use a
particular set of media streams and formats, but doesn\'t know the
addresses and ports at the time of the offer. Of course, when used, the
port number [MUST NOT]{.bcp14} be zero, if it would specify that the
stream has been disabled. However, an extension mechanism might specify
different semantics of the zero port number usage. An agent
[MUST]{.bcp14} be capable of receiving SDP with a connection address of
0.0.0.0, in which case it means that neither RTP nor RTCP is to be sent
to the peer.[¶](#section-13.4-1){.pilcrow}
:::
:::
:::
:::
...
### [14.1.](#section-14.1){.section-number .selfRef} [Original Text from RFC 5888, Section 9.2, 3rd Paragraph](#name-original-text-from-rfc-5888){.section-name .selfRef} {#name-original-text-from-rfc-5888}
...
### [14.2.](#section-14.2){.section-number .selfRef} [New Text Replacing RFC 5888, Section 9.2, 3rd Paragraph](#name-new-text-replacing-rfc-5888){.section-name .selfRef} {#name-new-text-replacing-rfc-5888}

SIP entities refuse media streams by setting the port to zero in the
corresponding \"m\" line. \"a=group\" lines [MUST NOT]{.bcp14} contain
identification-tags that correspond to \"m\" lines with the port set to
zero, but an extension mechanism might specify different semantics for
including identification-tags that correspond to such \"m=\"
lines.[¶](#section-14.2-1){.pilcrow}
:::
:::
:::
:::
...
identification-tags with \"m=\" sections within offers and answers,
using the procedures in \[[RFC5888](#RFC5888){.xref}\]. Each
...
associated with an \"m=\" section through the use of a \'id\' attribute
...
::: {#section-15.1-1 .artwork .art-text .alignLeft}
        0                   1                   2                   3
...
### [16.1.](#section-16.1){.section-number .selfRef} [New SDES Item](#name-new-sdes-item){.section-name .selfRef} {#name-new-sdes-item}

This document adds the MID SDES item to the IANA \"RTP SDES Item Types\"
registry as follows:[¶](#section-16.1-1){.pilcrow}

-   [[]{.break}]{#section-16.1-2.1}

    Value:
    :   15[¶](#section-16.1-2.1.1.2){.pilcrow}
    :   

    Abbrev.:
    :   MID[¶](#section-16.1-2.1.1.4){.pilcrow}
    :   

    Name:
    :   Media Identification[¶](#section-16.1-2.1.1.6){.pilcrow}
    :   

    Reference:
    :   RFC 8843[¶](#section-16.1-2.1.1.8){.pilcrow}
    :   
:::
:::
...
### [16.2.](#section-16.2){.section-number .selfRef} [New RTP SDES Header Extension URI](#name-new-rtp-sdes-header-extensi){.section-name .selfRef} {#name-new-rtp-sdes-header-extensi}

This document defines a new extension URI in the RTP SDES Compact Header
Extensions sub-registry of the RTP Compact Header Extensions registry
sub-registry, according to the following
data:[¶](#section-16.2-1){.pilcrow}

-   [[]{.break}]{#section-16.2-2.1}

    Extension URI:
    :   urn:ietf:params:rtp-hdrext:sdes:mid[¶](#section-16.2-2.1.1.2){.pilcrow}
    :   

    Description:
    :   Media identification[¶](#section-16.2-2.1.1.4){.pilcrow}
    :   

    Contact:
    :   IESG (iesg\@ietf.org)[¶](#section-16.2-2.1.1.6){.pilcrow}
    :   

    Reference:
    :   RFC 8843[¶](#section-16.2-2.1.1.8){.pilcrow}
    :   
...
### [16.3.](#section-16.3){.section-number .selfRef} [New SDP Attribute](#name-new-sdp-attribute){.section-name .selfRef} {#name-new-sdp-attribute}

This document defines a new SDP media-level attribute, \'bundle-only\',
according to the following data:[¶](#section-16.3-1){.pilcrow}

-   [[]{.break}]{#section-16.3-2.1}

    Attribute name:
    :   bundle-only[¶](#section-16.3-2.1.1.2){.pilcrow}
    :   

    Type of attribute:
    :   media[¶](#section-16.3-2.1.1.4){.pilcrow}
    :   

    Subject to charset:
    :   No[¶](#section-16.3-2.1.1.6){.pilcrow}
    :   

    Purpose:
    :   Request a media description to be accepted in the answer only if
        kept within a BUNDLE group by the
        answerer.[¶](#section-16.3-2.1.1.8){.pilcrow}
    :   

    Appropriate values:
    :   N/A[¶](#section-16.3-2.1.1.10){.pilcrow}
    :   

    Contact name:
    :   IESG[¶](#section-16.3-2.1.1.12){.pilcrow}
    :   

    Contact e-mail:
    :   iesg\@ietf.org[¶](#section-16.3-2.1.1.14){.pilcrow}
    :   

    Reference:
    :   RFC 8843[¶](#section-16.3-2.1.1.16){.pilcrow}
    :   

    Mux category:
    :   NORMAL[¶](#section-16.3-2.1.1.18){.pilcrow}
    :   
:::
:::
...
### [16.4.](#section-16.4){.section-number .selfRef} [New SDP Group Semantics](#name-new-sdp-group-semantics){.section-name .selfRef} {#name-new-sdp-group-semantics}
...
registry):[¶](#section-16.4-1){.pilcrow}
...
network, with the exception of the usage of the MID SDES item as
discussed below. Primarily, it changes which addresses and ports, and
thus in which (RTP) sessions, the information flows to. This affects the
security contexts being used and can cause previously separated
information flows to share the same security context. This has very
...
needed before selecting to apply BUNDLE.[¶](#section-17-1){.pilcrow}
...
bytes or less to allow them to efficiently fit into the MID RTP header
...
protection. Therefore, this specification updates RFC 7941 by adding the
exception that this requirement [MAY]{.bcp14} be ignored for the MID RTP
header extension. Security mechanisms for RTP/RTCP are discussed in
\"Options for Securing RTP Sessions\" \[[RFC7201](#RFC7201){.xref}\],
for example, SRTP \[[RFC3711](#RFC3711){.xref}\] can provide the
...
:   [Lennox, J.]{.refAuthor}[, Hong, D.]{.refAuthor}[,
    ...
    2017, \<<https://tools.ietf.org/html/draft-ietf-avtext-lrr-07>\>.
:   
...
## [Appendix A.](#section-appendix.a){.section-number .selfRef} [Design Considerations](#name-design-considerations){.section-name .selfRef} {#name-design-considerations}
...
in the Appendix, have been raised. The outcome was to specify a
mechanism that uses offers with both different and identical port
values.[¶](#section-appendix.a-1){.pilcrow}

Below are the primary issues that have been considered when defining the
\"BUNDLE\" grouping extension:[¶](#section-appendix.a-2){.pilcrow}

[]{.break}

1\)
:   Interoperability with existing User Agents
    (UAs).[¶](#section-appendix.a-3.1){.pilcrow}
:   

2\)
:   Interoperability with intermediary Back-to-Back User Agent (B2BUA)
    and proxy entities.[¶](#section-appendix.a-3.2){.pilcrow}
:   

3\)
:   Time to gather, and the number of, ICE
    candidates.[¶](#section-appendix.a-3.3){.pilcrow}
:   

4\)
:   Different error scenarios and when they
    occur.[¶](#section-appendix.a-3.4){.pilcrow}
:   

5\)
:   SDP offer/answer impacts, including usage of port number value
    zero.[¶](#section-appendix.a-3.5){.pilcrow}
:   

::: {#section-a.1 .section}
## [A.1.](#section-a.1){.section-number .selfRef} [UA Interoperability](#name-ua-interoperability){.section-name .selfRef} {#name-ua-interoperability}

Consider the following SDP offer/answer exchange, where Alice sends an
offer to Bob:[¶](#section-a.1-1){.pilcrow}

SDP Offer[¶](#section-a.1-2){.pilcrow}
...
SDP Answer[¶](#section-a.1-4){.pilcrow}
...
way that Bob knows if the packet is to be passed to the video or audio
codec is by looking at the port it was received on. This prompted some
SDP implementations to use a port number as an index to find the correct
\"m=\" line in the SDP, since each \"m\"= section contains a different
port number. As a result, some implementations that do support symmetric
RTP and ICE still use an SDP data structure where SDP with \"m=\"
sections with the same port such as:[¶](#section-a.1-6){.pilcrow}

SDP Offer[¶](#section-a.1-7){.pilcrow}
...
line.[¶](#section-a.1-9){.pilcrow}
:::

::: {#section-a.2 .section}
## [A.2.](#section-a.2){.section-number .selfRef} [Usage of Port Number Value Zero](#name-usage-of-port-number-value-){.section-name .selfRef} {#name-usage-of-port-number-value-}
...
section.[¶](#section-a.2-1){.pilcrow}

If each \"m=\" section associated with a BUNDLE group would contain
different port values, and one of those port values would be used for a
...
the BUNDLE address:port. In addition, it is unclear what would happen to
the ICE candidates associated with the \"m=\" section, as they are also
used for the BUNDLE address:port.[¶](#section-a.2-2){.pilcrow}
:::

::: {#section-a.3 .section}
## [A.3.](#section-a.3){.section-number .selfRef} [B2BUA and Proxy Interoperability](#name-b2bua-and-proxy-interoperab){.section-name .selfRef} {#name-b2bua-and-proxy-interoperab}
...
SDP \'rtcp\' attribute, defined in RFC 3605, yet acted this way. Further
assume that the B2BUA was configured to tear down any call where it did
not see any RTCP for 5 minutes. In this case, if the B2BUA received an
Offer like:[¶](#section-a.3-1){.pilcrow}

SDP Offer[¶](#section-a.3-2){.pilcrow}
...
specifications.[¶](#section-a.3-4){.pilcrow}

::: {#section-a.3.1 .section}
### [A.3.1.](#section-a.3.1){.section-number .selfRef} [Traffic Policing](#name-traffic-policing){.section-name .selfRef} {#name-traffic-policing}
...
ongoing.[¶](#section-a.3.1-1){.pilcrow}
:::

::: {#section-a.3.2 .section}
### [A.3.2.](#section-a.3.2){.section-number .selfRef} [Bandwidth Allocation](#name-bandwidth-allocation){.section-name .selfRef} {#name-bandwidth-allocation}
...
bandwidth. That may simply lead to either bad user experience or
termination of the call.[¶](#section-a.3.2-1){.pilcrow}
:::
:::

::: {#section-a.4 .section}
## [A.4.](#section-a.4){.section-number .selfRef} [Candidate Gathering](#name-candidate-gathering){.section-name .selfRef} {#name-candidate-gathering}
...
proposals.[¶](#section-a.5-1){.pilcrow}

The SDP examples are also modified versions from the ones in the
Alvestrand proposal.[¶](#section-a.5-2){.pilcrow}
...
feedback.[¶](#section-a.5-3){.pilcrow}
...
RTP/RTCP stream association.[¶](#section-a.5-4){.pilcrow}
...
::: {.left dir="auto"}
[400 3rd Avenue SW, Suite 350]{.street-address}
:::
